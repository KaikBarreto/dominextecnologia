-- =============================================================================
-- get_public_os: ADICIONA freq_code em cada activity do payload
-- =============================================================================
-- POR QUÊ ("Tipo de Visita" por equipamento no relatório público):
--   service_order_activities tem a coluna freq_code (M/T/S/A — mensal, trimestral,
--   semestral, anual). A tela pública da OS (modo cliente, anon) lê tudo via
--   get_public_os porque o RLS bloqueia leitura direta de service_order_activities
--   no anônimo. O relatório público precisa do freq_code pra exibir o
--   "Tipo de Visita" de cada equipamento no checklist PMOC.
--
-- O QUE MUDA:
--   Apenas acrescenta a chave 'freq_code' (a.freq_code) ao jsonb_build_object de
--   cada item de `activities`. Nada mais muda — copiei a definição CORRENTE
--   (20260620190000) integralmente.
--
-- SEGURANÇA: get_public_os é anon. freq_code é só um código de frequência
--   (M/T/S/A), sem dado sensível e sem cruzar tenant (a atividade já pertence à OS).
--
