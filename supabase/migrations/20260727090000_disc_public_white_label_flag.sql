-- =============================================================================
-- Perfil Comportamental (DISC): expor white_label_enabled no payload publico.
-- POR QUE: a tela publica do DISC (deslogada) precisa saber se a empresa e
--   white-label para decidir se mostra o rodape Dominex no PDF publico
--   (mostra quando NAO e white-label; some quando e).
-- O QUE MUDA (SO ISSO — funcao identica a 20260725100000 fora destes 2 pontos):
--   1. SELECT ... INTO v_cs: adiciona cs.white_label_enabled AS white_label_enabled.
--   2. jsonb_build_object: adiciona 'white_label_enabled', COALESCE(v_cs.white_label_enabled, false).
-- SEGURANCA: white_label_enabled e status booleano nao-sensivel (o logo/cor do
--   payload ja refletem isso). Allowlist, SECURITY DEFINER, search_path, STABLE,
--   GRANTs e COALESCE 3-niveis preservados. Assinatura inalterada (nao regenera types.ts).
-- =============================================================================
...
BEGIN
  ...
  -- white_label_enabled: status booleano (nao sensivel) usado pelo rodape do PDF publico.
  SELECT
    ...
    cs.white_label_enabled AS white_label_enabled
  INTO v_cs
  ...
  v_result := jsonb_build_object(
    ...
    'white_label_enabled',     COALESCE(v_cs.white_label_enabled, false),
...
COMMENT ON FUNCTION public.get_disc_public(text) IS
  'Payload publico do teste DISC (link /avaliacao/:token). SECURITY DEFINER: recebe so o short_code. Allowlist rigida — expoe branding + white_label_enabled + company_locale + 1o nome (employee_first_name) + nome completo (employee_name) + status; scores/profile so quando completed E visivel ao funcionario. Visibilidade efetiva: COALESCE(avaliacao.show_result_to_employee, empresa.disc_show_result_to_employee, false). NUNCA vaza salario/CPF/email/telefone nem outro tenant.';
