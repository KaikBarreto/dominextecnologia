-- =============================================================================
-- get_public_os: ADICIONA form_template_id em cada activity do payload
-- =============================================================================
-- POR QUÊ (feature "checklists personalizados por máquina" — Fases 1-3):
--   service_order_activities ganhou a coluna form_template_id (uuid null).
--   Atividades de "checklist personalizado" têm esse campo setado. A tela
--   pública da OS (modo cliente, anon) lê tudo via get_public_os porque o RLS
--   bloqueia leitura direta de service_order_activities no anônimo.
--   O frontend público precisa do form_template_id pra:
--     (a) renderizar as perguntas do checklist personalizado em vez de um
--         item de conformidade vazio;
--     (b) calcular o relatório parcial da OS pausada (só mostra equipamento
--         100% completo).
--
-- O QUE MUDA:
--   Apenas acrescenta a chave 'form_template_id' (a.form_template_id) ao
--   jsonb_build_object de cada item de `activities`. Nada mais muda — copiei a
--   definição CORRENTE (20260620140000) integralmente.
--
-- SEGURANÇA: get_public_os é anon. form_template_id é só um uuid de template,
--   sem dado sensível e sem cruzar tenant (a atividade já pertence à OS).
--
