-- Estende get_public_os para incluir as flags de conformidade PMOC no objeto
-- `contract` (is_pmoc + pmoc_legal_compliance_text). O link público da OS
-- (?modo=cliente) lê pelo cliente anônimo via este RPC SECURITY DEFINER; o
-- helper useIsPmocOrder usa o cliente autenticado e é barrado pelo RLS no modo
-- anônimo, então a flag precisa vir no payload deste RPC.
-- SEGURANÇA: o objeto `contract` continua expondo APENAS id/name + as duas
-- colunas PMOC. Nada de valor, frequência, datas ou responsável do contrato.
