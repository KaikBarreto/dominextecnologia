-- =============================================================================
-- regenerate_contract_visits: persistência atômica da regeneração de OSs
--
-- Corrige o 57014 (statement_timeout) da regeneração client-side: em vez de
-- centenas de round-trips (1 OS + filhos por vez), o client manda TODAS as OSs
-- já calculadas em JSON e esta função insere tudo + apaga as antigas numa ÚNICA
-- transação no servidor, com statement_timeout folgado. Atômica: qualquer falha
-- → ROLLBACK total ("gerar antes de apagar" fica de graça).
--
-- A função é "burra": não conhece PMOC/máquina. Recebe orders prontas e insere.
-- Toda a lógica de cálculo continua em TS (buildContractVisits/buildVisitOrderPayload).
--
-- p_orders: array de { os:{...}, equipment:[...], assignees:[...],
--                      activities:[...], preserve_code:text|null }
-- p_delete_ids: OSs antigas a apagar APÓS inserir as novas.
-- p_dry_run: true → executa tudo e dá ROLLBACK (via EXCEPTION), retornando as
--            contagens no DETAIL da exceção (mensagem 'DRY_RUN_ROLLBACK').
-- =============================================================================
  ...
  IF p_orders IS NOT NULL AND jsonb_typeof(p_orders) = 'array' THEN
    ...
    LOOP
      ...
      VALUES (
        ...
        COALESCE(v_os->>'scheduled_time','08:00'),
        v_os->>'description',
...
COMMENT ON FUNCTION public.regenerate_contract_visits(uuid, jsonb, uuid[], boolean) IS
  'Persistência atômica da regeneração de OSs de contrato (fix do 57014). SECURITY DEFINER com guarda de posse; company_id/contract_id forçados ao do contrato. Insere orders (OS+filhos) e apaga p_delete_ids em cascata numa transação. p_dry_run=true faz tudo e dá rollback via EXCEPTION, com as contagens no DETAIL.';

GRANT EXECUTE ON FUNCTION public.regenerate_contract_visits(uuid, jsonb, uuid[], boolean) TO authenticated;
