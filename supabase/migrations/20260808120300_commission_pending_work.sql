-- ============================================================================
-- Comissão de lead SELF-SERVICE nasce SEGURADA (status='pending_work') — parte
-- SQL. Espelha a EcoSistema. Corpo IDÊNTICO à definição viva mais recente da
-- RPC register_manual_company_payment (migration 20260716120000), EXCETO o
-- INSERT em salesperson_sales, que ganha a coluna status calculada por CASE:
--   CASE WHEN (NOT v_company.is_self_service OR v_company.lead_worked_at IS NOT NULL)
--        THEN 'confirmed' ELSE 'pending_work' END
--
-- Nenhuma outra linha muda — a matemática da comissão (base/rate/split
-- closer/sdr/total) é byte-a-byte a mesma. v_company é %ROWTYPE (SELECT *), então
-- os campos is_self_service/lead_worked_at (adicionados na migration de fundação)
-- já vêm carregados sem alterar o SELECT.
--
-- ⚠️ A OUTRA fonte de INSERT em salesperson_sales é a edge asaas-webhook (TS),
-- ajustada em paralelo (COMPANY_COLS ganha is_self_service/lead_worked_at + status
-- no insert). Ver report.
--
-- Idempotente: CREATE OR REPLACE + GRANT/REVOKE re-executáveis. Sem mudança de
-- schema aqui → sem regen de types.ts por conta desta migration.
-- ============================================================================
