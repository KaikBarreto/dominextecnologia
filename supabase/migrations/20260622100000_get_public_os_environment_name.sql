-- =============================================================================
-- get_public_os: ADICIONA environment_name (NOME do ambiente) em equipment_items
--                e em activities
-- =============================================================================
-- POR QUÊ ("relatório PMOC mostra o ambiente de cada equipamento"):
--   O relatório de OS PMOC precisa exibir o AMBIENTE de cada equipamento
--   (ex.: "Cassete 1 | 1º Andar"). O nome do ambiente vive em
--   contract_environments.identificacao, ligado ao equipamento por
--   contract_items.environment_id (que carrega contract_items.equipment_id +
--   contract_items.contract_id). No modo cliente (anon) o RLS bloqueia leitura
--   direta dessas tabelas, então o relatório depende deste payload. Até agora o
--   get_public_os NÃO expunha o ambiente.
--
-- O QUE MUDA:
--   1) equipment_items[]  → acrescenta 'environment_name'
--   2) activities[]       → acrescenta 'environment_name'
--   Em AMBOS resolvido por SUBSELECT ESCALAR (não LEFT JOIN) para evitar
--   multiplicar linhas no jsonb_agg quando o equipamento tem mais de uma
--   contract_items row:
--     (SELECT ce.identificacao
--        FROM contract_items ci
--        JOIN contract_environments ce ON ce.id = ci.environment_id
--       WHERE ci.equipment_id = <eq_id>
--         AND ci.contract_id  = v_so.contract_id
--         AND ci.environment_id IS NOT NULL
--       ORDER BY ci.sort_order ASC NULLS LAST
--       LIMIT 1)
--   contract_id vem da própria OS (v_so.contract_id). environment_name fica NULL
--   quando o equipamento não está mapeado a um ambiente no contrato.
--
-- NADA MAIS MUDA: todos os campos existentes (form_responses com
--   template_id/template_name, activities com freq_code/form_template_id,
--   rating, nps_*, contract, etc.) ficam idênticos à definição CORRENTE
--   (20260621130000). Função copiada integralmente.
--
-- SEGURANÇA: environment_name é metadado do contrato da própria OS, sem dado
--   sensível e sem cruzar tenant (filtra por v_so.contract_id da OS).
--
-- Idempotente: CREATE OR REPLACE. SECURITY DEFINER, search_path fixo, mesmos GRANTs.
-- =============================================================================
