-- Onda D — Orçamento → Cobrança.
--
-- A proposta pública (/proposta/:token, ProposalPublic.tsx) precisa saber se JÁ
-- existe uma cobrança gerada a partir daquele orçamento, para exibir um botão
-- "Pagar" que leva a /pagar/:code. Este link usa o CÓDIGO PÚBLICO da cobrança
-- (tenant_charges.public_short_code).
--
-- Cobranças de orçamento são gravadas em tenant_charges com
-- source_type = 'quote' e source_id = <quotes.id> (por um edge, em paralelo —
-- aqui a RPC só LÊ).
--
-- MUDANÇA (aditiva): acrescenta ao payload UM campo novo:
--   charge_public_short_code  text | null
-- Regra: a cobrança mais recente NÃO cancelada/estornada daquele quote.
--
-- ALLOWLIST ESTRITA: expõe SOMENTE o public_short_code. NADA de value/net_value/
-- custo/asaas_payment_id/customer_id nem qualquer coluna interna de tenant_charges.
-- O componente público só precisa do short_code pra montar o link /pagar/:code.
--
-- Base: partida da DEFINIÇÃO VIVA (pg_get_functiondef) da função em produção —
-- NÃO de memória. A allowlist de 20260722140100 é a versão viva; todos os campos
-- de quote/items/customer/company preservados byte-a-byte, só ACRESCENTAMOS o
-- novo campo no jsonb_build_object final. SECURITY DEFINER, search_path, STABLE e
-- os GRANTs anon-safe permanecem idênticos.
