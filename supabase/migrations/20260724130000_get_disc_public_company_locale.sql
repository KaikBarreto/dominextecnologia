-- =============================================================================
-- Adiciona company_locale ao payload de get_disc_public
-- POR QUE: a tela publica do DISC precisa usar o idioma do tenant sem seletor;
--          a coluna company_settings.language (adicionada em 20260717120000_i18n_regional_columns)
--          e a fonte da verdade. Padrao das demais RPCs publicas (get_public_os,
--          get_quote_public_payload, get_portal_data — migration 20260719120000).
-- ESCOPO: so adiciona a chave company_locale no jsonb de retorno.
--         Nada mais muda: allowlist, SECURITY DEFINER, search_path, grants.
-- =============================================================================
...
COMMENT ON FUNCTION public.get_disc_public(text) IS
  'Payload publico do teste DISC (link /avaliacao/:token). SECURITY DEFINER: recebe so o short_code. Allowlist rigida — expoe branding + company_locale + 1o nome + status; scores/profile so quando completed E visivel ao funcionario. NUNCA vaza salario/CPF/email/telefone nem outro tenant.';
