-- =============================================================================
-- get_public_os: ADICIONA template_id + template_name em cada form_response
-- =============================================================================
-- POR QUÊ ("checklist personalizado por equipamento com NOME real no relatório"):
--   As respostas de checklist personalizado vivem em form_responses (com
--   equipment_id + question_id). O NOME do checklist está em form_templates.name,
--   resolvido via form_questions.template_id. No modo cliente (anon) o relatório
--   lê tudo do get_public_os porque o RLS bloqueia leitura direta. Até agora o
--   payload de form_responses NÃO trazia o nome do template, então o relatório
--   caía num fallback genérico "Checklist". Agora cada resposta carrega o
--   template_id e o template_name reais.
--
-- O QUE MUDA:
--   No jsonb_build_object de cada item de `form_responses`, acrescenta:
--     'template_id'   = template_id da pergunta (form_questions.template_id)
--     'template_name' = form_templates.name (resolvido por aquele template_id)
--   Todos os campos existentes (id, question_id, response_value,
--   response_photo_url, equipment_id, question) são mantidos intactos. Resolvi
--   ambos via subselect a partir de fr.question_id pra não alterar a estrutura
--   do FROM/jsonb_agg corrente.
--
-- NADA MAIS MUDA: activities, conformidade, rating, nps_* etc. ficam idênticos
--   à definição CORRENTE (20260621120000). Copiei a função integralmente.
--
-- SEGURANÇA: template_id/template_name são metadados do checklist da própria OS,
--   sem dado sensível e sem cruzar tenant.
--
