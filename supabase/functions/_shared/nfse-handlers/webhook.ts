// =============================================================================
// Handler de WEBHOOK de NFS-e (rotas: nfse-webhook / fisqal-webhook).
// =============================================================================
...
// Até confirmar, o MVP roda em POLLING (nfse-status); este webhook é
...
export async function handleNfseWebhook(req: Request): Promise<Response> {
  ...
  if (!secret || !secret.trim()) {
    console.error("[nfse-webhook] FISQAL_WEBHOOK_SECRET ausente — rejeitando.");
  ...
  if (!provided) {
    console.error("[nfse-webhook] header de assinatura ausente:", headerName);
    return jsonResponse({ error: "unauthorized" }, 401);
  }
  ...
  if (providedBytes.length === 0 || !timingSafeEqual(expected, providedBytes)) {
    console.error("[nfse-webhook] assinatura HMAC inválida.");
    return jsonResponse({ error: "unauthorized" }, 401);
  }
  ...
  if (!emission) {
    ...
    console.warn("[nfse-webhook] emissão não encontrada", { event, documentId, fiscalRequestId });
  ...
  if (updErr) {
    console.error("[nfse-webhook] update error", { message: updErr.message });
