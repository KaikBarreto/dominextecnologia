...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ja semeou as categorias de sistema no INSERT da empresa, e o auto-cadastro
    // pode ser reexecutado (retry do cliente, timeout, reenvio do formulario).
    // Com .insert() puro, a segunda passada estouraria 23505 e derrubaria o
    // cadastro — caminho critico. ignoreDuplicates = ON CONFLICT DO NOTHING:
    // nao sobrescreve o que ja existe, nao duplica, nao quebra.
    await supabaseAdmin
