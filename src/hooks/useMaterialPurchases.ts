...
/** Payload de um preço (fornecedor × material). */
...
export function useMaterialPurchases() {
  ...
  const updatePurchase = useMutation({
    mutationFn: async ({ id, ...input }: SavePurchaseInput & { id: string }) => {
      ...
      await Promise.all([
        ...
        supabase.from('material_purchase_quotes').delete().eq('purchase_id', id),
      ]);
  ...
  // ---- Registrar entrada no estoque (manual, em cotação aprovada) ----
  // Pra cada item, chama a RPC atômica que dá entrada e registra no Kardex.
    ...
    mutationFn: async (purchaseId: string) => {
      ...
      for (const item of items) {
        ...
        const quote = quotes.find(
          (q) => q.supplier_id === supplierId && q.inventory_id === item.inventory_id,
        );
        const { error } = await supabase.rpc('register_inventory_movement', {
          ...
          p_unit_cost: quote?.unit_price ?? undefined,
          p_notes: `Entrada da cotação #${shortId(purchase.id)}`,
        });
        if (error) throw error;
      }
      return { id: purchaseId, count: items.filter((i) => i.inventory_id).length };
    },
    onSuccess: (res) => {
...
async function writeChildren(companyId: string, purchaseId: string, input: SavePurchaseInput) {
  if (input.items.length > 0) {
    const { error } = await supabase.from('material_purchase_items').insert(
      input.items.map((i) => ({
        company_id: companyId,
  ...
  // Só grava preços > 0 (célula vazia = sem cotação daquele fornecedor pro item).
  const validQuotes = input.quotes.filter((q) => q.unit_price > 0);
  if (validQuotes.length > 0) {
    const { error } = await supabase.from('material_purchase_quotes').insert(
      validQuotes.map((q) => ({
        company_id: companyId,
        purchase_id: purchaseId,
...
/** Σ quantidade × preço unitário de um fornecedor numa cotação. */
export function computeSupplierTotal(
  supplierId: string,
  items: Pick<MaterialPurchaseItem, 'inventory_id' | 'quantity'>[],
  quotes: Pick<MaterialPurchaseQuote, 'supplier_id' | 'inventory_id' | 'unit_price'>[],
): number {
