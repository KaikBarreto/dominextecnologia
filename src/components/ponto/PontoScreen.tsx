// PontoScreen — tela de bater ponto, mobile-first (movida de PontoPublico.tsx
// no refactor preparatório do Ponto em Grupo). Recebe a IDENTIDADE de quem está
// batendo (link pessoal ou, futuramente, quiosque) por prop — não conhece
// useParams nem rota.
//
...
//
// TRÊS ESTADOS DE PIN (só pra quem TEM PIN cadastrado; sem PIN nada disso
// aparece e a tela é exatamente a de sempre):
//   1. pedindo  — `state.pin_required`: teclado numérico, sem histórico/CTA;
//   2. errado   — 401 do servidor: mensagem com as tentativas restantes;
...
export const ACTION_CLASSNAME: Record<PunchType, string> = {
...
export const ACTION_ICON: Record<PunchType, typeof LogIn> = {
  clock_in: LogIn,
  break_start: Coffee,
  break_end: RotateCcw,
  clock_out: LogOut,
...
export const TYPE_DOT: Record<PunchType, string> = {
...
export const HEADER_DEPTH_OVERLAY =
...
export const HEADER_DOME_RADIUS = "0 0 50% 50% / 0 0 140px 140px";
...
// -----------------------------------------------------------------------------
// Botão de voltar — SÓ existe quando `onBack` é passado (hoje: o quiosque em
// grupo, voltando pra grade de crachás). O link pessoal nunca recebe `onBack`,
// então continua sem nenhum botão, como sempre (comportamento preservado).
//
// Fixo (não sticky) por cima do header, chip escuro translúcido: lê bem sobre
// QUALQUER cor de marca (clara ou escura) sem precisar calcular contraste por
// white-label. Presente em TODOS os estados da tela (skeleton, erro, sucesso)
// pra sempre existir um jeito de voltar pro quiosque.
// -----------------------------------------------------------------------------
function BackButton({ onBack, label }: { onBack: () => void; label: string }) {
  return (
    <button
      ...
      onClick={onBack}
      aria-label={label}
      className="fixed left-4 z-30 flex h-10 w-10 items-center justify-center rounded-full bg-black/30 text-white backdrop-blur-sm active:scale-95"
...
// -----------------------------------------------------------------------------
// PIN do funcionário (opcional) — teclado numérico e tela de bloqueio
//
// Só entram em cena pra quem TEM PIN cadastrado: a edge responde `pin_required`
// (só cartão + marca, sem histórico e sem próxima ação) ou 423 quando a pessoa
// errou demais. Quem não tem PIN nunca vê nenhuma destas telas.
//
// Teclado PRÓPRIO (e não <input type="number">) porque o alvo é tablet de
// parede: botões grandes, sem depender do teclado do sistema e sem zoom no foco.
// Marca vem do PAYLOAD e é aplicada inline (regra-lei nº2): nada de localStorage
// nem CSS var global.
// -----------------------------------------------------------------------------

/** Tipo das mensagens do domínio (pt-br é a fonte do shape). */
type TimeclockMessages = (typeof MESSAGES)["pt-br"]["app"]["timeclock"];

// 4 OU 6 dígitos, nunca 5 — é o que `set_ponto_pin` aceita no banco, e é o
// tamanho que todo mundo já conhece de cartão e de banco.
const PIN_LENGTHS = [4, 6] as const;
const PIN_MAX_LENGTH = 6;

function isCompletePin(pin: string): boolean {
  return (PIN_LENGTHS as readonly number[]).includes(pin.length);
}

/** Avatar + nome da pessoa, com o mesmo fallback de iniciais do header. */
function PinPersonCard({
  employee,
  accentColor,
}: {
  ...
  const showPhoto = !!employee.photo_url && !photoFailed;
...
export function PinKeypadScreen({
  ...
  company: PontoCompany;
  t: TimeclockMessages;
  /** Resolve em sucesso; rejeita com PontoError (pin_invalid traz attemptsLeft). */
  ...
  const { accentColor, textColor, resolvedLogo } = resolveBranding(company);

  // Enquanto não passa de 4, mostra 4 bolinhas; o 5º dígito abre pra 6. Evita
  // duas bolinhas eternamente vazias pra quem tem PIN de 4.
  const dotCount = pin.length > 4 ? PIN_MAX_LENGTH : 4;

  const push = (digit: string) => {
    if (submitting || pin.length >= PIN_MAX_LENGTH) return;
    setMessage(null);
    setPin((p) => p + digit);
  };
  ...
  const confirm = async () => {
    if (submitting || !isCompletePin(pin)) return;
    setSubmitting(true);
    setMessage(null);
    try {
      await onSubmit(pin);
      ...
      setPin("");
      // Falha NUNCA silenciosa: PIN errado diz quantas tentativas sobraram.
      if (err?.code === "pin_invalid" || err?.code === "pin_required") {
        const left = err.attemptsLeft;
        const suffix =
          typeof left === "number" && left > 0
            ? ` ${left === 1 ? t.pin.attemptsLeftOne : t.pin.attemptsLeft.replace("{n}", String(left))}`
            : "";
        setMessage(`${t.pin.invalid}${suffix}`);
      } else if (err?.status === 423) {
        // A tela inteira troca pro aviso de bloqueio (estado do hook).
        setMessage(null);
      } else if (err?.status === 0) {
        setMessage(t.connectionError);
      } else {
        setMessage(t.pin.genericError);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="dark flex min-h-[100svh] flex-col bg-zinc-950 text-foreground">
      {onBack && <BackButton onBack={onBack} label={t.backButton} />}
      <div
        ...
        >
          {Array.from({ length: dotCount }).map((_, i) => (
            <span
              ...
              style={{
                background: i < pin.length ? accentColor : "rgba(255,255,255,0.18)",
              }}
            />
          ))}
        </div>
        ...
        <div className="grid w-full grid-cols-3 gap-3">
          {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((d) => (
            <button
              ...
              type="button"
              onClick={() => push(d)}
              className="h-14 rounded-2xl bg-white/10 text-2xl font-semibold text-foreground active:scale-95 active:bg-white/20"
            >
              {d}
            </button>
          ))}
          <button
            ...
            className="flex h-14 items-center justify-center rounded-2xl bg-white/5 text-foreground active:scale-95"
          >
          ...
          <button
            ...
            disabled={submitting || !isCompletePin(pin)}
            aria-label={t.pin.confirm}
            className="flex h-14 items-center justify-center rounded-2xl text-base font-bold active:scale-95 disabled:opacity-40"
...
/**
 * Bloqueio temporário (5 erros = 15 min no banco). Mostra DE QUEM é o PIN e A
 * QUE HORAS destrava, no fuso e idioma da empresa. Sem marca: o 423 não carrega
 * branding de propósito (payload mínimo), e esta é uma tela de aviso.
 */
export function PinLockedScreen({
  lock,
  t,
  lockedUntilLabel,
  onRetry,
  ...
  lock: PontoPinLock;
  t: TimeclockMessages;
  /** Horário de liberação já formatado no fuso/idioma da empresa, ou null. */
  lockedUntilLabel: string | null;
  onRetry: () => void;
  onBack?: () => void;
}) {
  return (
    <div className="dark flex min-h-[100svh] flex-col items-center justify-center gap-4 bg-zinc-950 p-6 text-center text-foreground">
      {onBack && <BackButton onBack={onBack} label={t.backButton} />}
      {/* Selo saturado: fundo vermelho + ícone BRANCO direto no fundo. */}
      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-destructive">
        <Lock className="h-8 w-8 text-white" />
      </div>
      {lock.employee && (
        <p className="max-w-[16rem] truncate text-base font-semibold">
          {lock.employee.name}
        </p>
      )}
      <h1 className="text-xl font-semibold">{t.pin.lockedTitle}</h1>
      <p className="max-w-xs text-sm text-muted-foreground">
        {lockedUntilLabel
          ? t.pin.lockedDescription.replace("{time}", lockedUntilLabel)
          : t.pin.lockedDescriptionNoTime}
      </p>
...
// -----------------------------------------------------------------------------
// Tela — consome locale/timezone do contexto (PublicAppLocaleProvider, provido
// pelo shell da página que renderiza este componente)
// -----------------------------------------------------------------------------
...
export function PontoScreen({ identity, onBack, onPunchSuccess }: PontoScreenProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const err = e as PontoError;
      // PIN recusado na hora da batida (o PIN foi cadastrado/trocado no meio da
      // sessão da tela): a mensagem é a do i18n, não o código de máquina.
      const isPinError =
        err?.code === "pin_invalid" || err?.code === "pin_required";
      toast({
        variant: "destructive",
        title: t.toasts.punchError,
        description: isPinError ? t.pin.required : err?.message,
      });
      if (err?.status === 409 || isPinError || err?.status === 423) {
        // Estado mudou no servidor — recarrega pra refletir a próxima ação
        // correta (ou a tela de PIN / de bloqueio, quando for o caso).
        setFlowOpen(false);
        await refetch();
      }
    } finally {
  ...
  if (notFound)
    return (
      <>
        {onBack && <BackButton onBack={onBack} label={t.backButton} />}
        <LinkBroken title={t.linkInvalid.title} description={t.linkInvalid.description} />
      </>
    );

  // PIN bloqueado por tentativas (423). Vem ANTES do skeleton e do erro genérico:
  // é o motivo REAL de não haver estado, e a pessoa precisa saber quando destrava.
  if (pinLock) {
    return (
      <PinLockedScreen
        lock={pinLock}
        ...
        lockedUntilLabel={
          pinLock.lockedUntil
            ? fmtTime(pinLock.lockedUntil, locale as LocaleCode, timezone)
            : null
        }
        onRetry={() => {
          clearPinLock();
  ...
  // Tem PIN cadastrado e ainda não digitou: só cartão + teclado. Nem histórico
  // nem CTA de batida são montados aqui (o servidor também não mandou nenhum).
  ...
  return (
    ...
    >
      ...
      <BrandedHeader
        ...
        // `position` é opcional no cartão (a resposta de PIN não manda cargo).
      ...
      >
        ...
        >
          ...
          {/* `first-letter:uppercase`, não `capitalize`: o pt-BR devolve
              "quinta-feira, 17 de setembro" e `capitalize` põe maiúscula em
              TODA palavra ("Quinta-Feira, 17 De Setembro"). */}
          <p className="text-sm text-muted-foreground first-letter:uppercase -mt-1">
            {todayLabel}
          </p>
        </section>
