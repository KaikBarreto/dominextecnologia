...
import { useWhatsappTiers, formatWhatsappTierLimit } from '@/hooks/useWhatsappTiers';
import { useWhatsappTierChange } from '@/hooks/useWhatsappTierChange';

interface AdminWhatsappTierControlProps {
  companyId: string;
  /** Nível atual da empresa (companies.whatsapp_tier). */
  currentTier: number;
}

/**
 * Controle do nível de Avisos de WhatsApp da empresa no painel master Auctus.
 * Clone de AdminNfseTierControl. Mostra o nível atual e abre um seletor de upgrade
 * que chama `change-whatsapp-tier` (o edge aceita super_admin). Downgrade fora de
 * escopo — só níveis acima são selecionáveis.
 */
export function AdminWhatsappTierControl({ companyId, currentTier }: AdminWhatsappTierControlProps) {
  const { tiers } = useWhatsappTiers();
  const { changeTier, isChanging } = useWhatsappTierChange();
    ...
    <>
      <div className="flex items-center gap-2 mt-0.5">
        ...
        <span className="text-xs text-muted-foreground">
          {current ? formatWhatsappTierLimit(current.monthlyLimit) : '—'}
        </span>
      ...
      <ResponsiveModal
        ...
        title="Alterar nível de Avisos de WhatsApp"
        footer={
      ...
      >
        <div className="space-y-2.5 py-1">
          <p className="text-sm text-muted-foreground">
            Sobe o nível de Avisos de WhatsApp da empresa (atual: Nível {currentTier}). O valor da
            assinatura é ajustado automaticamente e sincronizado com o Asaas.
          </p>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">{formatWhatsappTierLimit(t.monthlyLimit)}</p>
                  </div>
