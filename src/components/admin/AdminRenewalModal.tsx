...
export function AdminRenewalModal({ company, open, onOpenChange, onSuccess }: AdminRenewalModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      const { error: companyError } = await supabase.from('companies').update({ subscription_expires_at: data.subscription_expires_at.toISOString(), subscription_status: 'active' }).eq('id', company.id);
      if (companyError) throw companyError;
      const { error: paymentError } = await supabase.from('company_payments').insert({ company_id: company.id, amount: paymentValue, payment_date: new Date().toISOString(), payment_method: 'Pix', notes: data.observations || null, type: 'renewal' });
      if (paymentError) throw paymentError;
      const { data: { user } } = await supabase.auth.getUser();
      await supabase.from('admin_financial_transactions').insert({ type: 'income', category: 'renewal', amount: paymentValue, description: `Renovação - ${company.name}`, transaction_date: new Date().toISOString(), created_by: user?.id, reference_type: 'company_renewal', reference_id: company.id });
    },
    onSuccess: () => {
    ...
    <ResponsiveModal
      ...
      footer={
        <div className="flex gap-2">
          ...
          <Button onClick={form.handleSubmit((data) => mutation.mutate(data))} disabled={isSubmitting} className="flex-1">{isSubmitting ? 'Renovando...' : 'Confirmar'}</Button>
        </div>
      }
    >
      <div className="space-y-4">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
            <FormField control={form.control} name="subscription_expires_at" render={({ field }) => (
            ...
            <FormField control={form.control} name="payment_value" render={({ field }) => (
              <FormItem><FormLabel>Valor do Pagamento *</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
            )} />
