...
import {
  DomiflixSectionWithTitleIds,
  useCreateSection,
  useDeleteSection,
  useDomiflixSectionsWithTitles,
  useReorderSections,
  useUpdateSection,
...
function TitlePicker({
  ...
  return (
    <div className="space-y-3">
              ...
              >
                ...
                <div className="w-10 h-6 flex-shrink-0 rounded overflow-hidden bg-muted">
                  {(t.banner_url || t.thumbnail_url) ? (
                    <img
                      src={t.banner_url || t.thumbnail_url || ""}
                      alt=""
      ...
      <p className="text-xs text-muted-foreground">
        {selectedIds.length} título(s) selecionado(s)
      </p>
    </div>
  );
}

// ─── Sortable list of titles inside a section ────────────────────────────────

function SortableTitleChip({
  id,
  title,
  onRemove,
}: {
  id: string;
  title: DomiflixTitle | undefined;
  onRemove: () => void;
}) {
...
function SortableSectionCard({
  ...
  section: DomiflixSectionWithTitleIds;
  titlesById: Map<string, DomiflixTitle>;
}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } =
  ...
  function removeTitle(id: string) {
    const next = section.titleIds.filter((x) => x !== id);
    updateSectionTitles.mutate({ sectionId: section.id, titleIds: next });
  }

  function openPicker() {
    setDraftIds(section.titleIds);
    setPickerOpen(true);
  }

  function savePicker() {
    updateSectionTitles.mutate(
      { sectionId: section.id, titleIds: draftIds },
  ...
  function saveLabel() {
    ...
    if (!v || v === section.label) {
      setEditingLabel(false);
      setLabelDraft(section.label);
      return;
    }
    updateSection.mutate(
      { id: section.id, label: v },
  ...
  return (
    <Card ref={setNodeRef} style={style} className="border">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="text-xs">
            {section.titleIds.length} título(s)
          </Badge>

          <div className="flex items-center gap-2">
            <Label htmlFor={`active-${section.id}`} className="text-xs text-muted-foreground">
              Ativa
            </Label>
            <Switch
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Os títulos vinculados não serão excluídos, apenas o vínculo com esta seção. Ação irreversível.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <CardContent className="space-y-3">
        {section.titleIds.length === 0 ? (
          <p className="text-xs text-muted-foreground italic">
            Nenhum título nesta seção. Clique em "Adicionar títulos" para começar.
          </p>
        ) : (
          <DndContext
            sensors={innerSensors}
            collisionDetection={closestCenter}
            onDragEnd={handleTitlesDragEnd}
          >
              ...
              <div className="flex flex-wrap gap-1.5">
                {section.titleIds.map((id) => (
                  <SortableTitleChip
                    key={id}
                    id={id}
                    title={titlesById.get(id)}
                    onRemove={() => removeTitle(id)}
                  />
                ))}
              </div>
            </SortableContext>
      ...
      <ResponsiveModal
        ...
        title={`Títulos de "${section.label}"`}
        description="Selecione os títulos que aparecem nessa seção."
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setPickerOpen(false)}>
              Cancelar
            </Button>
            <Button onClick={savePicker} disabled={updateSectionTitles.isPending}>
              Salvar
            </Button>
          </div>
        }
...
export function AdminDomiflixSections() {
  ...
  function handleCreate() {
    ...
    if (!label) return;
    create.mutate(
      { label, description: newDescription.trim() || null },
      {
        onSuccess: () => {
          setNewLabel("");
          setNewDescription("");
  ...
  return (
    <div className="space-y-4">
      ...
      <ResponsiveModal
        ...
        title="Nova seção"
        description="Crie um novo carrossel para a home do Domiflix."
        footer={
          <div className="flex justify-end gap-2">
      ...
      >
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Input
              autoFocus
              placeholder='Ex: "Mais vistos", "Reciclagem", "Novidades"'
              value={newLabel}
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              placeholder="Descrição interna desta seção"
