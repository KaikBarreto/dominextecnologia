...
export function CostResourceFormSheet({
  ...
  const footer = (
    <Button className="w-full" onClick={handleSubmit} disabled={!name.trim() || isPending || isUploading}>
      ...
      {isPending || isUploading ? t.saving : t.save}
    </Button>
  );

  const categoryLabel = t.categoryLabels[category];

  return (
    <ResponsiveModal
      open={open}
      onOpenChange={onOpenChange}
      title={resource ? t.titleEdit.replace('{category}', categoryLabel) : t.titleNew.replace('{category}', categoryLabel)}
      className="sm:max-w-[540px]"
      footer={footer}
    >
      <div className="space-y-6">
        <p className="text-sm text-muted-foreground">
          {isGift ? t.descGift : t.descResource}
        </p>

        {/* Basic fields */}
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>{isGift ? t.nameLabelGift : t.nameLabel.replace('{entity}', categoryLabel.toLowerCase())}</Label>
            <Input
              value={name}
          ...
          <div className="space-y-2">
            ...
            {photoPreview ? (
              ...
              <label className="flex items-center gap-2 cursor-pointer border border-dashed border-border rounded-lg p-3 hover:bg-muted/30 transition-colors">
                ...
                <span className="text-sm text-muted-foreground">{t.photoAdd}</span>
          ...
          {!isGift && (
            <div className="space-y-2">
              <Label>{t.monthlyHoursLabel}</Label>
              <NumericInput value={String(monthlyHours ?? '')} onValueChange={v => setMonthlyHours(Number(v) || 176)} />
              <p className="text-xs text-muted-foreground">{t.monthlyHoursHint}</p>
            </div>
          )}

          <div className="flex items-center justify-between">
            <Label>{isGift ? t.activeLabelGift : t.activeLabel}</Label>
            ...
            <CollapsibleContent className="mt-2">
              <Card>
                <CardContent className="p-4 space-y-3">
                  <p className="text-sm text-muted-foreground">{t.depCalcFormula}</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs">{t.depCalcLabel0km}</Label>
                      <Input
                        type="text"
                        inputMode="numeric"
                        placeholder={t.depCalcPlaceholder0km}
                        value={value0kmText}
                        onChange={e => setValue0kmText(e.target.value)}
                      />
                      {value0kmText && (
                        <p className="text-xs text-muted-foreground">
                          {t.depCalcParsed.replace('{amount}', fmt(value0km))}
                        </p>
                      )}
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">{t.depCalcLabelAfter2y}</Label>
                      <Input
                        type="text"
                        inputMode="numeric"
                        placeholder={t.depCalcPlaceholderAfter2y}
                        value={value2yearsText}
                        onChange={e => setValue2yearsText(e.target.value)}
                      />
                      {value2yearsText && (
                        <p className="text-xs text-muted-foreground">
                          {t.depCalcParsed.replace('{amount}', fmt(value2years))}
                        </p>
                      )}
                    </div>
                  </div>
                  {calculatedDepreciation > 0 && (
                    <div className="flex items-center justify-between p-2 bg-muted/50 rounded-md">
                      <span className="text-sm">{t.depCalcResult}</span>
                      <span className="font-semibold text-primary">{fmt(calculatedDepreciation)}</span>
                    </div>
                  )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <Label>{isGift ? t.itemsSectionLabelGift : t.itemsSectionLabel}</Label>
            <Button size="sm" variant="outline" onClick={handleAddItem}>
              <Plus className="h-3.5 w-3.5 mr-1" /> {t.itemsAdd}
            </Button>
          </div>
          ...
          <div className="space-y-3">
              ...
              <Card key={index}>
                <CardContent className="p-3 space-y-2">
                  <div className="flex gap-2">
                    <Input
                      placeholder={isGift ? t.itemPlaceholderNameGift : t.itemPlaceholderName}
                      value={item.name}
                  ...
                  {isGift ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          <Label className="text-xs">{t.itemTotalCostLabel}</Label>
                          <Input
                            type="number"
                            min={0}
                            step="0.01"
                        ...
                        <div className="space-y-1">
                          <Label className="text-xs">{t.itemBatchUnitsLabel}</Label>
                          <NumericInput
                            placeholder="500"
                      ...
                      {(item.total_cost ?? 0) > 0 && (item.total_units ?? 0) > 0 && (
                        <div className="flex items-center justify-between p-2 bg-muted/50 rounded-md text-sm">
                          <span className="text-muted-foreground">{t.itemUnitCostResult}</span>
                          <span className="font-semibold text-foreground">
                            {fmt((item.total_cost ?? 0) / (item.total_units ?? 1))}
                          </span>
                        </div>
                      )}

                      <div className="space-y-1">
                        <Label className="text-xs">{t.itemQtyPerGiftLabel}</Label>
                        <NumericInput
                          placeholder="1"
                      ...
                      {item.value > 0 && (
                        <div className="flex items-center justify-between p-2 bg-primary/5 rounded-md text-sm">
                          <span className="text-muted-foreground">{t.itemCostPerGiftResult}</span>
                          <span className="font-semibold text-primary">{fmt(item.value)}</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor={`annual-${index}`} className="text-xs cursor-pointer">{t.itemAnnualToggle}</Label>
                      </div>
                      {!item.is_monthly ? (
            ...
            {items.length === 0 && (
              <div className="text-center py-4 text-sm text-muted-foreground border border-dashed rounded-md">
                {isGift ? t.itemEmptyGift : t.itemEmptyResource}
              </div>
            )}
          </div>
        </div>
            ...
            {isGift ? (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t.summaryItemsCount}</span>
                  <span>{items.filter(i => i.name.trim()).length}</span>
                </div>
                <div className="flex justify-between text-sm pt-2 border-t border-border">
                  <span className="font-medium text-primary">{t.summaryGiftTotal}</span>
                  <span className="text-lg font-bold text-primary">{fmt(totalMonthly)}</span>
                </div>
              </>
            ) : (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t.summaryMonthlyTotal}</span>
                  <span className="font-semibold">{fmt(totalMonthly)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t.summaryMonthlyHours}</span>
                  <span>{monthlyHours}h</span>
                </div>
                <div className="flex justify-between text-sm pt-2 border-t border-border">
                  <span className="font-medium text-primary">{t.summaryHourlyRate}</span>
                  <span className="text-lg font-bold text-primary">{fmt(hourlyRate)}/h</span>
                </div>
              </>
            )}
          </CardContent>
