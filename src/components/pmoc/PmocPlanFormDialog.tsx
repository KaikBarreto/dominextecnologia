...
export function PmocPlanFormDialog({ open, onOpenChange, plan }: PmocPlanFormDialogProps) {
  ...
  useEffect(() => {
    if (plan) {
      setCustomerId(plan.customer_id); setName(plan.name);
      ...
      setStatus(plan.status); setTechnicianId(plan.technician_id || '');
      setServiceTypeId(plan.service_type_id || ''); setFormTemplateId(plan.form_template_id || '');
      setContractId(plan.contract_id || ''); setNotes(plan.notes || '');
      setSelectedEquipmentIds(plan.pmoc_items?.map(i => i.equipment_id) || []);
      setStep(0);
    } else {
      setCustomerId(''); setName(''); setFrequencyType('months'); setFrequencyValue(1);
      setStartDate(format(new Date(), 'yyyy-MM-dd')); setHorizonMonths(12); setStatus('ativo');
      setTechnicianId(''); setServiceTypeId(''); setFormTemplateId(''); setContractId('');
      setNotes(''); setSelectedEquipmentIds([]); setStep(0);
    }
  }, [plan, open]);

  const customerOptions = useMemo(() => customers.map(c => ({ value: c.id, label: c.name, sublabel: c.document || c.email || undefined })), [customers]);
  const customerContracts = useMemo(() => contracts.filter(c => c.customer_id === customerId), [contracts, customerId]);

  const toggleEquipment = (eqId: string) => {
    setSelectedEquipmentIds(prev => prev.includes(eqId) ? prev.filter(id => id !== eqId) : [...prev, eqId]);
  };
  ...
  const previewDates = useMemo(() => {
    ...
    while (current <= end && dates.length < 60) {
      ...
      if (frequencyType === 'days') { current = addDays(current, frequencyValue); }
      else { current = addMonthsFn(current, frequencyValue); }
    }
    return dates;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const input: PmocPlanInput = {
        customer_id: customerId, name, frequency_months: frequencyMonths, next_generation_date: startDate,
        status, technician_id: technicianId || null, service_type_id: serviceTypeId || null,
        form_template_id: formTemplateId || null, contract_id: contractId || null,
        notes: notes || null, equipment_ids: selectedEquipmentIds,
      };
      let planId: string;
      if (isEditing && plan) { await updatePlan.mutateAsync({ id: plan.id, ...input }); planId = plan.id; }
      ...
      if (!isEditing && planId && status === 'ativo') {
        for (const date of previewDates) {
          ...
          if (selectedEquipmentIds.length > 0) {
            ...
            await supabase.from('service_order_equipment').insert(eqLinks);
          }
          await supabase.from('pmoc_generated_os').insert({ plan_id: planId, service_order_id: os.id, scheduled_for: format(date, 'yyyy-MM-dd') });
        }
  ...
  const quickFreqLabel = frequencyType === 'months' ? QUICK_FREQUENCIES.find(f => f.months === frequencyValue)?.label : `A cada ${frequencyValue} dias`;

  return (
    <ResponsiveModal
      open={open}
      onOpenChange={onOpenChange}
      title={isEditing ? 'Editar Plano PMOC' : 'Novo Plano PMOC'}
      ...
      <div className="flex items-center gap-1 mb-4">
        {STEPS.map((s, i) => (
          <div key={s.key} className="flex items-center gap-1 flex-1">
            ...
            <span className={cn('text-xs hidden sm:inline', i === step ? 'font-medium text-foreground' : 'text-muted-foreground')}>{s.label}</span>
      ...
      {step === 0 && (
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label>Cliente *</Label>
              <SearchableSelect options={customerOptions} value={customerId} onValueChange={(v) => { setCustomerId(v); setSelectedEquipmentIds([]); setContractId(''); }} placeholder="Selecione o cliente" searchPlaceholder="Buscar cliente..." />
            </div>
            <div className="space-y-2">
              ...
              <Select value={technicianId || 'none'} onValueChange={v => setTechnicianId(v === 'none' ? '' : v)}>
                ...
                <SelectContent>
                  ...
                  {technicians?.map(t => (<SelectItem key={t.user_id} value={t.user_id}>{t.full_name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              ...
              <Select value={serviceTypeId || 'none'} onValueChange={v => setServiceTypeId(v === 'none' ? '' : v)}>
                ...
                <SelectContent>
                  <SelectItem value="none">Nenhum</SelectItem>
                  {serviceTypes.filter(t => t.is_active).map(st => (<SelectItem key={st.id} value={st.id}>{st.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
            ...
            {customerContracts.length > 0 && (
              <div className="space-y-2">
                ...
                <Select value={contractId || 'none'} onValueChange={v => setContractId(v === 'none' ? '' : v)}>
                  ...
                  <SelectContent>
                    <SelectItem value="none">Nenhum</SelectItem>
                    {customerContracts.map(c => (<SelectItem key={c.id} value={c.id}>{c.contract_number || `Contrato ${c.id.slice(0, 8)}`}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            )}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {FREQUENCY_TYPES.map(ft => (
                <button key={ft.value} onClick={() => { setFrequencyType(ft.value as any); setFrequencyValue(ft.value === 'months' ? 1 : 7); }}
                  className={cn('flex-1 rounded-lg border-2 p-3 text-sm font-medium transition-colors', frequencyType === ft.value ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50')}>
                  {ft.label}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {QUICK_FREQUENCIES.map(f => (
                  <button key={f.months} onClick={() => setFrequencyValue(f.months)}
                    className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-colors', frequencyValue === f.months ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:border-primary/50')}>
                    {f.label}
                  </button>
                ))}
              </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="number" min={1} value={frequencyValue} onChange={e => setFrequencyValue(Number(e.target.value))} />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <Input type="number" min={1} max={60} value={horizonMonths} onChange={e => setHorizonMonths(Number(e.target.value))} className="w-24" />
              <span className="text-sm text-muted-foreground">= {previewDates.length} ocorrências</span>
            </div>
          </div>
          {previewDates.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="max-h-[120px] overflow-y-auto rounded-lg border p-2 space-y-1">
                ...
                {previewDates.length > 20 && <p className="text-xs text-muted-foreground italic">... e mais {previewDates.length - 20} datas</p>}
              </div>
            </div>
          )}
        </div>
            ...
            <>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {activeEquipment.map(eq => (
                  <label key={eq.id} className={cn("flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors hover:bg-muted/50", selectedEquipmentIds.includes(eq.id) && "border-primary bg-primary/5")}>
                    <Checkbox checked={selectedEquipmentIds.includes(eq.id)} onCheckedChange={() => toggleEquipment(eq.id)} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">{eq.name}</p>
                      <p className="text-xs text-muted-foreground">{[eq.brand, eq.model].filter(Boolean).join(' - ') || 'Sem detalhes'}</p>
                    </div>
                  </label>
                ))}
              </div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Plano</p><p className="font-medium">{name}</p></div>
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Cliente</p><p className="font-medium">{customers.find(c => c.id === customerId)?.name || '-'}</p></div>
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Frequência</p><p className="font-medium">{quickFreqLabel || `${frequencyValue} ${frequencyType === 'days' ? 'dias' : 'meses'}`}</p></div>
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Início</p><p className="font-medium">{format(new Date(startDate), 'dd/MM/yyyy')}</p></div>
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Equipamentos</p><p className="font-medium">{selectedEquipmentIds.length || 'Nenhum'}</p></div>
              <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Status</p><Badge variant={status === 'ativo' ? 'success' : 'outline'}>{status === 'ativo' ? 'Ativo' : 'Pausado'}</Badge></div>
            </div>
          </div>
          {!isEditing && status === 'ativo' && (
            <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
              <p className="text-sm font-medium text-primary">🗓️ Serão geradas {totalOsToGenerate} OSs automaticamente</p>
