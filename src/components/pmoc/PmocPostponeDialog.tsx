...
export function PmocPostponeDialog({ open, onOpenChange, plan, generatedOs }: PmocPostponeDialogProps) {
  ...
  // Get all future generated OSs for this plan, sorted by date
  ...
  // Find the index of the current OS in the sorted list
  const currentIndex = allFutureOs.findIndex(g => g.id === generatedOs.id);

  // Calculate the cascading effect: from postponed OS onward, recalculate dates
  ...
  const handlePostpone = async () => {
    ...
    try {
      for (const entry of cascadePreview) {
        // Update the service order scheduled_date
        await supabase
          .from('service_orders')
          .update({ scheduled_date: entry.newDate })
          .eq('id', entry.serviceOrderId);

        // Update the pmoc_generated_os scheduled_for
        await supabase
          .from('pmoc_generated_os')
          .update({ scheduled_for: entry.newDate } as any)
          .eq('id', entry.id);
      }
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="max-h-[200px] overflow-y-auto rounded-lg border divide-y">
                {cascadePreview.map((entry, i) => {
                  ...
                  return (
                    <div
                      key={entry.id}
                      className={`flex items-center gap-3 px-3 py-2 text-xs ${changed ? 'bg-warning/5' : 'bg-muted/30'}`}
                    >
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={submitting || !newDate || changedCount === 0}
          >
