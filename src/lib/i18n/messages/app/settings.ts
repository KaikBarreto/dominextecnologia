// ─────────────────────────────────────────────────────────────────────────────
// i18n do APP LOGADO — domínio CONFIGURAÇÕES (Settings) + Central de Ajuda.
//
// Sub-namespaces por seção:
//   page        — cabeçalho da página + abas do sidebar
//   saveStatus  — indicador de auto-save
//   company     — aba Empresa (identidade visual, dados cadastrais, contato, endereço, white-label, documentos legais)
//   usability   — aba Usabilidade + card de Origens
//   regional    — aba Regional (idioma pessoal + padrões da empresa)
//   appearance  — aba Aparência
//   shortcuts   — aba Atalhos de Teclado + labels dos atalhos
//   dangerZone  — Zona de Perigo (DangerZoneCard)
//   resetSystem — modal Zerar Sistema (ResetSystemDialog)
//   help        — Central de Ajuda (HelpCenterDrawer)
//
// FORMATO: as 4 traduções JUNTAS, chaveadas por locale (ver app/index.ts).
// pt-br é a FONTE — texto EXATO que hoje está no código, corrigindo apenas
// acentos/encoding que estavam faltando nas strings hardcoded.
// Tradução SEMÂNTICA: termos de mercado por idioma, não palavra a palavra.
// ─────────────────────────────────────────────────────────────────────────────

export const settings = {
  'pt-br': {
    // ── Cabeçalho da página e abas do sidebar ────────────────────────────────
    page: {
      title: 'Configurações',
      subtitle: 'Gerencie as configurações do sistema',
      tabs: {
        empresa: 'Empresa',
        regional: 'Regional',
        usuarios: 'Usuários e Permissões',
        usabilidade: 'Usabilidade',
        atalhos: 'Atalhos',
        aparencia: 'Aparência',
        integracoes: 'Integrações',
      },
    },

    // ── Indicador de auto-save ────────────────────────────────────────────────
    saveStatus: {
      saving: 'Salvando…',
      unsaved: 'Alterações não salvas',
      saved: 'Salvo',
    },

    // ── Aba Empresa ───────────────────────────────────────────────────────────
    company: {
      cardTitle: 'Dados da Empresa',
      cardDescription: 'Informações da empresa que aparecem em etiquetas e documentos',

      // Identidade Visual
      visualIdentity: {
        sectionTitle: 'Identidade Visual',
        sectionDescription: 'Logo da empresa para documentos e sistema',
        replace: 'Substituir',
        remove: 'Remover',
        removeLogoTitle: 'Remover logo?',
        removeLogoDesc: 'O logo atual será removido permanentemente.',
        uploadPrompt: 'Clique para enviar o logo',
        uploadHint: 'PNG, JPG até 5MB',
        errorFileSize: 'Arquivo muito grande (máx 5MB)',
        errorFileType: 'Apenas imagens são permitidas',
        errorUpload: 'Erro ao enviar logo',
        errorIconUpload: 'Erro ao enviar ícone',
      },

      // Dados Cadastrais
      registrationData: {
        sectionTitle: 'Dados Cadastrais',
        sectionDescription: 'Razão social, documento e informações oficiais',
        companyName: 'Nome da Empresa',
        companyNamePlaceholder: 'Nome da sua empresa',
        cnpj: 'CNPJ/CPF',
        cnpjPlaceholder: '00.000.000/0000-00',
        showInDocs: 'Exibir em documentos',
        segment: 'Segmento de Atuação da Empresa',
        segmentNotDefined: 'Não definido',
        segmentHint: 'Define quais ferramentas e recursos do seu segmento aparecem no sistema. Para alterar, fale com a Dominex.',
      },

      // Contato
      contact: {
        sectionTitle: 'Contato',
        sectionDescription: 'Telefone e e-mail da empresa',
        phone: 'Telefone',
        phonePlaceholder: '(00) 0000-0000',
        email: 'Email',
        emailPlaceholder: 'contato@empresa.com',
        showInDocs: 'Exibir em documentos',
      },

      // Endereço
      address: {
        sectionTitle: 'Endereço',
        sectionDescription: 'Localização física da empresa',
        showInDocs: 'Exibir em documentos',
        zip: 'CEP',
        street: 'Endereço',
        streetPlaceholder: 'Rua, Avenida…',
        number: 'Número',
        numberPlaceholder: 'Nº',
        complement: 'Complemento',
        complementPlaceholder: 'Sala, Andar…',
        neighborhood: 'Bairro',
        neighborhoodPlaceholder: 'Bairro',
        stateCity: 'UF / Cidade',
      },

      // White Label
      whiteLabelSection: {
        sectionTitle: 'White Label',
        sectionDescription: 'Personalize o sistema com a identidade visual da sua marca',
        moduleUnavailable: 'Módulo não disponível no seu plano atual',
        hire: 'Contratar',
        enableLabel: 'Ativar White Label',
        enableDescription: 'Substitui o logo e a cor padrão do sistema',
        fullLogo: 'Logo completo',
        fullLogoConfigured: 'Logo personalizado configurado',
        fullLogoUsingCompany: 'Usando o logo da empresa por padrão',
        fullLogoFallback: 'Será utilizado o logo da empresa',
        uploadLogoPrompt: 'Enviar logo (opcional)',
        icon: 'Ícone (1:1)',
        iconDescription: 'Ícone quadrado para o menu lateral recolhido. 128×128px.',
        uploadIconPrompt: 'Enviar ícone',
        primaryColor: 'Cor primária',
        primaryColorDescription: 'Substitui a cor verde padrão do sistema',
        qrSection: {
          title: 'QR Code',
          description: 'Personalize o visual dos QR Codes gerados pelo sistema, com o logo da sua marca no centro',
          dotStyleLabel: 'Estilo dos pontos',
          dotStyle: {
            square: 'Quadrado',
            rounded: 'Arredondado',
            dots: 'Pontos',
            classy: 'Elegante',
          },
          cornerStyleLabel: 'Cantos',
          cornerStyle: {
            square: 'Quadrado',
            rounded: 'Arredondado',
            dot: 'Ponto',
          },
          colorLabel: 'Cor do QR Code',
          previewCaption: 'Pré-visualização',
        },
        reportHeader: 'Cabeçalho do Relatório de Serviço',
        reportHeaderDescription: 'Personalize o visual do cabeçalho que aparece no relatório da OS concluída',
        reportBgColor: 'Cor de fundo do cabeçalho',
        reportTextColor: 'Cor do texto',
        reportStatusBarColor: 'Cor da barra de status',
        reportLogoSize: 'Tamanho do logo ({size}px)',
        reportLogoBg: 'Fundo atrás do logo',
        reportLogoBgDescription: 'Remove o fundo quando desativado',
        reportLogoBgColor: 'Cor do fundo',
        reportLogoType: 'Tipo de logo no relatório',
        reportLogoTypeFull: 'Logo Completo',
        reportLogoTypeIcon: 'Ícone',
        reportLogoTypeHintFull: 'Usa o logo completo da empresa',
        reportLogoTypeHintIcon: 'Usa o ícone configurado no White Label',
      },

      // Documentos Legais
      legalDocs: {
        sectionTitle: 'Documentos Legais',
        sectionDescription: 'Consulte e baixe os termos que regem o uso do Dominex',
        viewTerms: 'Ver termos de uso',
        acceptedAt: 'Aceito em {date}',
      },
    },

    // ── Aba Usabilidade ───────────────────────────────────────────────────────
    usability: {
      cardTitle: 'Usabilidade',
      cardDescription: 'Preferências de comportamento do sistema',
      preferenceSaved: 'Preferência salva!',

      sections: {
        os: {
          title: 'Ordens de Serviço',
          description: 'Comportamentos relacionados às ordens de serviço',
          autoSaveOS: {
            title: 'Salvamento Automático',
            description: 'Salvar automaticamente rascunhos de ordens de serviço ao editar',
          },
          showOSValues: {
            title: 'Exibir Valores',
            description: 'Mostrar valores financeiros (mão de obra, peças) nas ordens de serviço',
          },
          requireSignature: {
            title: 'Exigir Assinatura',
            description: 'Tornar obrigatória a assinatura do cliente ao finalizar OS',
          },
          saveOSPhotosToDevice: {
            title: 'Salvar fotos no dispositivo',
            description: 'Mostra um botão para salvar a foto no seu aparelho. No iPhone abre a opção "Salvar Imagem"; no Android baixa direto.',
          },
          osStockConsumptionEnabled: {
            title: 'Consumo de estoque na OS',
            description: 'Permite que o técnico registre os materiais usados durante a ordem de serviço e dê baixa no estoque ao finalizar',
          },
        },
        interface: {
          title: 'Interface',
          description: 'Preferências visuais de listagens e tabelas',
          compactTables: {
            title: 'Tabelas Compactas',
            description: 'Reduzir espaçamento nas tabelas para exibir mais dados por página',
          },
          showEquipmentPhotos: {
            title: 'Fotos de Equipamentos',
            description: 'Exibir miniaturas de fotos dos equipamentos nas listagens',
          },
        },
        security: {
          title: 'Segurança',
          description: 'Confirmações e validações de segurança',
          confirmDelete: {
            title: 'Confirmar Exclusões',
            description: 'Exibir diálogo de confirmação antes de excluir registros',
          },
        },
        schedule: {
          title: 'Agenda',
          description: 'Configurações da agenda e calendário',
          showHolidays: {
            title: 'Exibir Feriados',
            description: 'Mostrar feriados nacionais e municipais na agenda',
          },
        },
      },

      // Card de Origens
      origins: {
        cardTitle: 'Origens',
        cardDescription: 'Lista de origens usada no cadastro de clientes e nas oportunidades do CRM',
        manage: 'Gerenciar origens',
        empty: 'Nenhuma origem cadastrada. Crie um conjunto inicial e edite à vontade depois.',
        seedButton: 'Criar origens padrão',
        seedLoading: 'Criando…',
        inactive: 'Inativa',
      },
    },

    // ── Aba Regional ──────────────────────────────────────────────────────────
    regional: {
      personalCard: {
        title: 'Meu idioma',
        description: 'O idioma que VOCÊ vê no sistema. Aplica na hora, só para você.',
        languageLabel: 'Idioma',
        languageHint: 'Preferência pessoal. Sobrepõe o idioma padrão da empresa só para você.',
      },
      companyCard: {
        title: 'Padrões da empresa',
        description: 'Idioma padrão, moeda e fuso horário para toda a empresa',
        adminOnly: '(somente admin)',
        languageLabel: 'Idioma padrão da empresa',
        languageHint: 'Idioma aplicado a todos que não escolheram um pessoalmente.',
        localeDefaultsPrompt: 'Usar os padrões de {locale} para moeda e fuso?',
        keepCurrent: 'Manter atuais',
        useDefaults: 'Usar padrões',
        localeDefaultsApplied: 'Padrões de {locale} aplicados',
        currencyLabel: 'Moeda',
        currencyHint: 'Moeda de operação usada nos valores do sistema.',
        currencyChangedWarning: 'A moeda é a de operação da empresa. Trocar não converte os valores já registrados, eles continuam com o número original.',
        timezoneLabel: 'Fuso horário',
        timezoneHint: 'Usado para datas e horários exibidos no sistema.',
        timezonePlaceholder: 'Selecione o fuso',
        timezoneSearchPlaceholder: 'Buscar fuso (ex: Sao Paulo)',
        timezoneEmpty: 'Nenhum fuso encontrado.',
        savingStatus: 'Salvando...',
        unsavedStatus: 'Alterações não salvas',
        savedStatus: 'Salvo',
      },
      discCard: {
        title: 'Perfil Comportamental',
        description: 'Como o teste comportamental (DISC) se comporta para os funcionários.',
        visibilityLabel: 'Funcionário vê o próprio resultado do teste comportamental',
        visibilityHint:
          'Ligado: ao terminar, o funcionário vê o próprio perfil. Desligado: vê só uma tela de agradecimento, e o resultado fica só para o RH.',
        on: 'Sim',
        off: 'Não',
      },
    },

    // ── Aba Aparência ─────────────────────────────────────────────────────────
    appearance: {
      cardTitle: 'Aparência',
      cardDescription: 'Personalize a interface visual do sistema',
      navigationStyle: {
        sectionTitle: 'Estilo de Navegação (Desktop)',
        sectionDescription: 'Escolha entre menu lateral ou menu superior. Esta opção só afeta a visualização em desktop.',
        sidebar: 'Menu Lateral',
        sidebarDescription: 'Sidebar tradicional à esquerda',
        topbar: 'Menu Superior',
        topbarDescription: 'Barra horizontal no topo',
      },
      theme: {
        sectionTitle: 'Tema do Sistema',
        sectionDescription: 'Escolha entre tema claro ou escuro para a interface.',
        light: 'Tema Claro',
        dark: 'Tema Escuro',
      },
      navTheme: {
        sectionTitle: 'Menu (lateral e superior)',
        sectionDescription: 'Escolha a cor do menu de navegação, independente do tema do sistema.',
        dark: 'Escuro (recomendado)',
        darkDescription: 'Menu sempre escuro, com destaque na cor da marca',
        system: 'Seguir o tema',
        systemDescription: 'Menu acompanha o tema claro ou escuro',
      },
    },

    // ── Aba Atalhos de Teclado ────────────────────────────────────────────────
    shortcuts: {
      pageTitle: 'Atalhos de Teclado',
      pageDescription: 'Use atalhos para acessar funcionalidades rapidamente',
      toggleLabel: 'Atalhos de Teclado',
      toggleEnabled: 'Atalhos estão ativados',
      toggleDisabled: 'Atalhos estão desativados',
      groupNavigation: 'Navegação',
      groupGeneral: 'Geral',
      tips: {
        title: 'Dicas',
        shiftNav: 'Use {modifier} + letra para navegação rápida',
        noTyping: 'Atalhos não funcionam quando você está digitando em campos de texto',
        canDisable: 'Você pode desativar os atalhos a qualquer momento pelo toggle acima',
      },
      // Labels dos atalhos individuais (id → label + description)
      goto_dashboard: { label: 'Dashboard', description: 'Navegar para o painel principal' },
      goto_os: { label: 'Ordens de Serviço', description: 'Navegar para ordens de serviço' },
      goto_agenda: { label: 'Agenda', description: 'Navegar para a agenda' },
      goto_clients: { label: 'Clientes', description: 'Navegar para clientes' },
      goto_equipment: { label: 'Equipamentos', description: 'Navegar para equipamentos' },
      goto_crm: { label: 'CRM', description: 'Navegar para o CRM' },
      goto_finance: { label: 'Financeiro', description: 'Navegar para o financeiro' },
      goto_inventory: { label: 'Estoque', description: 'Navegar para o estoque' },
      goto_quotes: { label: 'Orçamentos', description: 'Navegar para orçamentos' },
      goto_contracts: { label: 'Contratos', description: 'Navegar para contratos' },
      goto_settings: { label: 'Configurações', description: 'Abrir configurações' },
      goto_profile: { label: 'Perfil', description: 'Abrir perfil do usuário' },
    },

    // ── Zona de Perigo ────────────────────────────────────────────────────────
    dangerZone: {
      title: 'Zona de Perigo',
      description: 'Ações irreversíveis. Revise cuidadosamente antes de confirmar.',
      resetSystem: {
        label: 'Zerar Sistema',
        description: 'Exclui dados operacionais selecionados. Dados da empresa e usuários são preservados. Toda operação fica registrada em log de auditoria.',
        button: 'Zerar Sistema',
      },
    },

    // ── Exportar meus dados ──────────────────────────────────────────────────
    dataExport: {
      cardTitle: 'Exportar meus dados',
      cardDescription: 'Baixe uma planilha do Excel com os dados da sua empresa: clientes, equipamentos, ordens de serviço, orçamentos, contratos, financeiro e mais. Serve de backup e para levar seus dados para onde quiser.',
      button: 'Exportar dados',
      dialogTitle: 'Exportar meus dados',
      dialogDescription: 'Escolha o que deseja incluir na planilha. O arquivo é gerado na hora e baixado automaticamente.',
      selectAll: 'Marcar tudo',
      clearAll: 'Limpar seleção',
      confirm: 'Exportar',
      cancel: 'Cancelar',
      running: 'Exportando...',
      progressLabel: 'Exportando {sheet}... {rows} registros',
      successToast: 'Exportação concluída! O arquivo foi baixado.',
      errorToast: 'Não foi possível exportar os dados. Tente novamente.',
      emptySelection: 'Selecione pelo menos um item para exportar.',
      heavyHint: 'Movimentações de Estoque e Ponto vêm desmarcados por padrão porque costumam ter muitos registros. Marque-os se também precisar deles.',
      fileNamePrefix: 'backup',
      groups: {
        customers: 'Clientes',
        equipment: 'Equipamentos',
        serviceOrders: 'Ordens de Serviço',
        quotes: 'Orçamentos',
        contracts: 'Contratos',
        financial: 'Financeiro',
        inventory: 'Estoque',
        inventoryMovements: 'Movimentações de Estoque',
        employees: 'Funcionários',
        leads: 'CRM / Leads',
        timeRecords: 'Ponto',
      },
      sheets: {
        summary: 'Resumo',
        customers: 'Clientes',
        customerContacts: 'Contatos dos Clientes',
        equipment: 'Equipamentos',
        serviceOrders: 'Ordens de Serviço',
        serviceMaterials: 'Materiais das OS',
        quotes: 'Orçamentos',
        quoteItems: 'Itens dos Orçamentos',
        contracts: 'Contratos',
        contractItems: 'Itens dos Contratos',
        financialTransactions: 'Financeiro',
        financialAccounts: 'Contas Bancárias',
        inventory: 'Estoque',
        inventoryMovements: 'Movimentações de Estoque',
        employees: 'Funcionários',
        leads: 'CRM Leads',
        timeRecords: 'Ponto',
      },
      summary: {
        companyLabel: 'Empresa',
        generatedAtLabel: 'Gerado em',
        sheetColumn: 'Aba',
        rowCountColumn: 'Registros',
        totalLabel: 'Total',
      },
      columns: {
        summary: {
          company: 'Empresa',
          generatedAt: 'Gerado em',
          sheet: 'Aba',
          rows: 'Registros',
        },
        customers: {
          name: 'Nome',
          nome_fantasia: 'Nome Fantasia',
          company_name: 'Razão Social',
          customer_type: 'Tipo de Cliente',
          document: 'CPF/CNPJ',
          inscricao_estadual: 'Inscrição Estadual',
          inscricao_municipal: 'Inscrição Municipal',
          email: 'Email',
          phone: 'Telefone',
          celular: 'Celular',
          address: 'Endereço',
          address_number: 'Número',
          complement: 'Complemento',
          neighborhood: 'Bairro',
          city: 'Cidade',
          state: 'Estado',
          zip_code: 'CEP',
          origin: 'Origem',
          birth_date: 'Data de Nascimento',
          notes: 'Observações',
          created_at: 'Cadastrado em',
        },
        customerContacts: {
          customer: 'Cliente',
          name: 'Nome',
          position: 'Cargo',
          phone: 'Telefone',
          email: 'Email',
          notes: 'Observações',
          created_at: 'Criado em',
        },
        equipment: {
          customer: 'Cliente',
          name: 'Nome',
          category: 'Categoria',
          brand: 'Marca',
          model: 'Modelo',
          serial_number: 'Número de Série',
          identifier: 'Identificador',
          capacity: 'Capacidade',
          location: 'Local',
          status: 'Status',
          install_date: 'Data de Instalação',
          warranty_until: 'Garantia até',
          notes: 'Observações',
          created_at: 'Criado em',
        },
        serviceOrders: {
          order_number: 'Número da OS',
          status: 'Status',
          os_type: 'Tipo de OS',
          service_type: 'Tipo de Serviço',
          entry_type: 'Tipo de Entrada',
          origin: 'Origem',
          customer: 'Cliente',
          equipment: 'Equipamento',
          technician: 'Técnico',
          team: 'Equipe',
          contract: 'Contrato',
          task_title: 'Título da Tarefa',
          description: 'Descrição',
          diagnosis: 'Diagnóstico',
          solution: 'Solução',
          notes: 'Observações',
          scheduled_date: 'Data Agendada',
          scheduled_time: 'Horário Agendado',
          started_at: 'Iniciado em',
          completed_at: 'Concluído em',
          duration_minutes: 'Duração (minutos)',
          labor_hours: 'Horas Trabalhadas',
          labor_value: 'Valor da Mão de Obra',
          parts_value: 'Valor das Peças',
          total_value: 'Valor Total',
          pmoc_conformity_status: 'Conformidade PMOC',
          service_address: 'Endereço do Serviço',
          service_address_number: 'Número do Endereço',
          service_neighborhood: 'Bairro do Serviço',
          service_city: 'Cidade do Serviço',
          service_state: 'Estado do Serviço',
          service_zip_code: 'CEP do Serviço',
          created_at: 'Criado em',
        },
        serviceMaterials: {
          order_number: 'Número da OS',
          item_name: 'Material',
          quantity: 'Quantidade',
          unit: 'Unidade',
          purchase_price: 'Preço de Compra',
          sale_price: 'Preço de Venda',
          subtotal: 'Subtotal',
          created_at: 'Criado em',
        },
        quotes: {
          quote_number: 'Número do Orçamento',
          status: 'Status',
          customer: 'Cliente',
          prospect_name: 'Nome do Prospect',
          prospect_email: 'Email do Prospect',
          prospect_phone: 'Telefone do Prospect',
          subtotal: 'Subtotal',
          discount_type: 'Tipo de Desconto',
          discount_value: 'Valor do Desconto',
          discount_amount: 'Desconto Aplicado',
          displacement_cost: 'Custo de Deslocamento',
          distance_km: 'Distância (km)',
          km_cost: 'Custo por Km',
          total_cost: 'Custo Total',
          total_price: 'Preço Total',
          total_value: 'Valor Total',
          bdi: 'BDI (%)',
          profit_rate: 'Margem de Lucro (%)',
          tax_rate: 'Alíquota de Impostos (%)',
          admin_indirect_rate: 'Taxa Administrativa (%)',
          card_installments: 'Parcelas no Cartão',
          card_discount_rate: 'Desconto no Cartão (%)',
          valid_until: 'Válido até',
          view_count: 'Nº de Visualizações',
          last_viewed_at: 'Última Visualização',
          terms: 'Condições',
          notes: 'Observações',
          created_at: 'Criado em',
        },
        quoteItems: {
          quote_number: 'Número do Orçamento',
          position: 'Posição',
          item_type: 'Tipo de Item',
          description: 'Descrição',
          details: 'Detalhes',
          service_type: 'Tipo de Serviço',
          quantity: 'Quantidade',
          unit_price: 'Preço Unitário',
          price_override: 'Preço Ajustado',
          total_price: 'Preço Total',
          unit_total_cost: 'Custo Unitário Total',
          unit_labor_cost: 'Custo Unitário de Mão de Obra',
          unit_materials_cost: 'Custo Unitário de Materiais',
          unit_extras_cost: 'Custo Unitário de Extras',
          unit_hours: 'Horas por Unidade',
          unit_hourly_rate: 'Valor da Hora',
          bdi: 'BDI (%)',
          profit_rate: 'Margem de Lucro (%)',
        },
        contracts: {
          name: 'Nome',
          customer: 'Cliente',
          status: 'Status',
          is_pmoc: 'É PMOC',
          frequency_type: 'Tipo de Frequência',
          frequency_value: 'Valor da Frequência',
          start_date: 'Data de Início',
          horizon_months: 'Horizonte (meses)',
          next_pmoc_generation_date: 'Próxima Geração PMOC',
          service_type: 'Tipo de Serviço',
          technician: 'Técnico',
          team: 'Equipe',
          responsible_technician: 'Técnico Responsável',
          unidade_nome: 'Nome da Unidade',
          unidade_endereco: 'Endereço da Unidade',
          unidade_numero: 'Número da Unidade',
          unidade_complemento: 'Complemento da Unidade',
          unidade_bairro: 'Bairro da Unidade',
          unidade_cidade: 'Cidade da Unidade',
          unidade_uf: 'UF da Unidade',
          unidade_cep: 'CEP da Unidade',
          pmoc_identificacao_ambiente: 'Identificação do Ambiente (PMOC)',
          pmoc_tipo_atividade: 'Tipo de Atividade (PMOC)',
          pmoc_area_climatizada_m2: 'Área Climatizada (m²)',
          pmoc_carga_termica_tr: 'Carga Térmica (TR)',
          pmoc_ocupantes_fixos: 'Ocupantes Fixos',
          pmoc_ocupantes_flutuantes: 'Ocupantes Flutuantes',
          notes: 'Observações',
          created_at: 'Criado em',
        },
        contractItems: {
          contract: 'Contrato',
          item_name: 'Nome do Item',
          item_description: 'Descrição do Item',
          equipment: 'Equipamento',
          pmoc_scope: 'Escopo PMOC',
          pmoc_start_visit: 'Visita Inicial PMOC',
          sort_order: 'Ordem',
          created_at: 'Criado em',
        },
        financialTransactions: {
          transaction_date: 'Data da Transação',
          due_date: 'Vencimento',
          paid_date: 'Data de Pagamento',
          description: 'Descrição',
          transaction_type: 'Tipo',
          category: 'Categoria',
          amount: 'Valor',
          amount_received: 'Valor Recebido',
          is_paid: 'Pago',
          payment_method: 'Forma de Pagamento',
          account: 'Conta',
          customer: 'Cliente',
          employee: 'Funcionário',
          contract: 'Contrato',
          order_number: 'Número da OS',
          installment_number: 'Parcela',
          installment_total: 'Total de Parcelas',
          payroll_kind: 'Tipo de Folha',
          payroll_period: 'Período da Folha',
          credit_card_bill_date: 'Data da Fatura do Cartão',
          cancelled_at: 'Cancelado em',
          cancelled_reason: 'Motivo do Cancelamento',
          notes: 'Observações',
          created_at: 'Criado em',
        },
        financialAccounts: {
          name: 'Nome',
          type: 'Tipo',
          bank_name: 'Banco',
          institution_name: 'Instituição',
          initial_balance: 'Saldo Inicial',
          credit_limit: 'Limite de Crédito',
          closing_day: 'Dia de Fechamento',
          due_day: 'Dia de Vencimento',
          payment_due_days: 'Prazo de Pagamento (dias)',
          is_active: 'Ativo',
          created_at: 'Criado em',
        },
        inventory: {
          name: 'Nome',
          sku: 'Código (SKU)',
          category: 'Categoria',
          unit: 'Unidade',
          stock: 'Local de Estoque',
          quantity: 'Quantidade',
          min_quantity: 'Estoque Mínimo',
          cost_price: 'Preço de Custo',
          sale_price: 'Preço de Venda',
          total_value: 'Valor Total em Estoque',
          supplier: 'Fornecedor',
          description: 'Descrição',
        },
        inventoryMovements: {
          created_at: 'Data',
          movement_type: 'Tipo de Movimentação',
          item: 'Material',
          sku: 'Código (SKU)',
          stock: 'Local de Estoque',
          quantity: 'Quantidade',
          unit_cost: 'Custo Unitário',
          stock_before: 'Saldo Antes',
          stock_after: 'Saldo Depois',
          supplier: 'Fornecedor',
          order_number: 'Número da OS',
          notes: 'Observações',
        },
        employees: {
          name: 'Nome',
          matricula: 'Matrícula',
          cpf: 'CPF',
          position: 'Cargo',
          cbo: 'CBO',
          employment_regime: 'Regime de Contratação',
          hire_date: 'Data de Admissão',
          is_active: 'Ativo',
          email: 'Email',
          phone: 'Telefone',
          address: 'Endereço',
          salary: 'Salário',
          monthly_cost: 'Custo Mensal',
          dependents_count: 'Nº de Dependentes',
          payment_frequency: 'Frequência de Pagamento',
          payment_day: 'Dia de Pagamento',
          payment_day_2: '2º Dia de Pagamento',
          payment_day_type: 'Tipo de Dia de Pagamento',
          payment_weekday: 'Dia da Semana de Pagamento',
          vt_enabled: 'Vale-Transporte Ativo',
          vt_monthly_value: 'Valor Mensal do Vale-Transporte',
          pix_key: 'Chave Pix',
          created_at: 'Criado em',
        },
        leads: {
          title: 'Título',
          status: 'Status',
          stage: 'Etapa',
          customer: 'Cliente',
          assigned_to: 'Responsável',
          source: 'Origem',
          value: 'Valor',
          probability: 'Probabilidade (%)',
          expected_close_date: 'Previsão de Fechamento',
          notes: 'Observações',
          created_at: 'Criado em',
          updated_at: 'Atualizado em',
        },
        timeRecords: {
          date: 'Data',
          recorded_at: 'Registrado em',
          type: 'Tipo',
          employee: 'Funcionário',
          source: 'Origem',
          is_valid: 'Válido',
          address: 'Endereço',
          latitude: 'Latitude',
          longitude: 'Longitude',
          notes: 'Observações',
        },
      },
      // ── Rótulos de VALOR de enum (não confundir com `columns`, que são
      // rótulos de CABEÇALHO). Usados pelo useCompanyDataExport pra traduzir
      // célula crua do banco (ex.: "em_andamento") sem duplicar dicionário já
      // existente em outra tela — só entra aqui o que não tinha rótulo em
      // lugar nenhum do app.
      values: {
        discountTypeValor: 'Valor',
        discountTypePercentual: 'Percentual',
        itemTypeServico: 'Serviço',
        itemTypeMaterial: 'Material',
        itemTypeMaoDeObra: 'Mão de Obra',
        originManual: 'Manual',
        originContract: 'Gerado por Contrato',
        originPortal: 'Portal do Cliente',
        payrollKindRescission: 'Rescisão',
        timeRecordSourceApp: 'Aplicativo',
        timeRecordSourceAdmin: 'Administrador',
        timeRecordSourceLinkPublico: 'Link Público',
      },
    },

    // ── Modal Zerar Sistema ───────────────────────────────────────────────────
    resetSystem: {
      modalTitle: 'Zerar Sistema',
      irreversibleWarning: 'Esta ação irá',
      deletePermanently: 'DELETAR PERMANENTEMENTE',
      warningCompany: 'os dados operacionais da empresa',
      chooseWhat: 'Escolha o que deseja remover:',
      selectAll: 'Marcar tudo',
      selectedCount: '{count}/{total} selecionados',
      groupSelectAriaLabel: 'Selecionar tudo em {group}',
      forcedByMaterials: 'Necessário porque o cadastro de materiais foi marcado.',
      preserved: {
        title: 'Serão mantidos:',
        companyData: 'Dados básicos da empresa',
        users: 'Usuários cadastrados',
        paymentHistory: 'Histórico de pagamentos',
      },
      finalWarning: 'Esta ação NÃO PODE ser desfeita!',
      confirmLabel: 'Para confirmar, digite o nome da empresa:',
      confirmPlaceholder: 'Nome da empresa',
      nameMismatch: 'O nome digitado não confere com o nome da empresa.',
      offline: 'Esta ação precisa de conexão com a internet. Você está offline.',
      progressTitle: 'Apagando dados...',
      progressStep: 'Etapa {current} de {total}:',
      cancel: 'Cancelar',
      confirmButton: 'Sim, Zerar Sistema',
      successTitle: 'Sistema zerado com sucesso',
      successDesc: 'Os dados selecionados foram removidos. O log de auditoria foi gerado.',
      errorTitle: 'Não foi possível zerar o sistema',
      errorPermission: 'Você não tem permissão para zerar o sistema. Apenas o administrador da empresa pode fazer isso.',
      errorTimeout: 'Uma etapa demorou demais. Tente novamente, o que já foi apagado continua apagado.',
      errorDependency: 'Não foi possível concluir a etapa por uma dependência inesperada. Avise o suporte com o nome da etapa.',
      errorDuplicate: 'Não foi possível recriar uma configuração padrão. Tente novamente em alguns segundos.',
      errorGeneric: 'Não foi possível zerar o sistema. Tente novamente ou avise o suporte.',
      errorStepPrefix: 'Erro na etapa "{step}".',

      // Grupos de seleção
      groups: {
        operacional: {
          label: 'Operacional',
          delete_service_orders: {
            title: 'Ordens de Serviço',
            description: 'Remove todas as OS, fotos, materiais consumidos, avaliações e formulários respondidos',
          },
          delete_equipment: {
            title: 'Equipamentos',
            description: 'Remove todos os equipamentos cadastrados nos clientes',
          },
          delete_contracts: {
            title: 'Contratos e PMOC',
            description: 'Remove todos os contratos, cronogramas PMOC e documentos PMOC personalizados',
          },
          delete_custom_configs: {
            title: 'Configurações personalizadas',
            description: 'Remove CRM stages personalizados, formulários de OS e recursos de custo',
          },
        },
        comercial: {
          label: 'Comercial (CRM)',
          delete_quotes: {
            title: 'Orçamentos e Propostas',
            description: 'Remove todos os orçamentos e propostas',
          },
          delete_customers: {
            title: 'Clientes e Leads',
            description: 'Remove todos os cadastros de clientes, contatos, portais e leads',
          },
        },
        financeiro: {
          label: 'Financeiro',
          delete_financial_movements: {
            title: 'Movimentações',
            description: 'Remove todas as transações de caixa, banco, cartão, receitas e despesas',
          },
          delete_financial_categories: {
            title: 'Categorias financeiras',
            description: 'Remove as categorias financeiras personalizadas (as padrões são mantidas)',
          },
        },
        rh_inventario: {
          label: 'RH e Inventário',
          delete_employees: {
            title: 'Funcionários e equipe',
            description: 'Remove todos os funcionários, vales, pagamentos, ponto eletrônico e equipes',
          },
          delete_materials: {
            title: 'Materiais',
            description: 'Remove todos os materiais cadastrados (e o estoque deles)',
          },
          delete_stock: {
            title: 'Estoque',
            description: 'Zera o estoque (movimentações), mantém os materiais cadastrados',
          },
        },
      },

      // Mapa de rótulos de steps de progresso
      stepLabels: {
        service_orders: 'Ordens de Serviço',
        contracts: 'Contratos',
        quotes: 'Orçamentos',
        equipment: 'Equipamentos',
        custom_configs: 'Configurações Personalizadas',
        financial_movements: 'Movimentações Financeiras',
        financial_categories: 'Categorias Financeiras',
        employees: 'Funcionários e RH',
        stock: 'Estoque de Materiais',
        materials: 'Cadastro de Materiais',
        customers: 'Clientes',
      },
    },

    // ── Tela Usuários e Permissões ────────────────────────────────────────────
    users: {
      pageTitle: 'Usuários e Permissões',
      pageSubtitle: '{count}/{max} usuários • {active} ativos',
      tabs: {
        users: 'Usuários',
        presets: 'Cargos',
      },
      searchPlaceholderUsers: 'Buscar usuários...',
      searchPlaceholderPresets: 'Buscar cargos...',
      searchPlaceholderDesktop: 'Buscar por nome ou telefone...',
      viewList: 'Lista',
      viewCards: 'Cards',
      btnSettings: 'Configurações',
      btnCreateUser: 'Criar Usuário',
      btnHireMore: 'Contratar mais usuários',
      limitTitle: 'Limite de usuários atingido ({count}/{max})',
      limitDesc: 'Para cadastrar novos usuários, adicione usuários extras ao seu plano.',
      badgeYou: 'Você',
      badgeActive: 'Ativo',
      badgeInactive: 'Inativo',
      badgeAllPerms: 'Acesso Total',
      badgePermCount: '{count} permissões',
      fabUser: 'Usuário',
      fabPreset: 'Cargo',
      btnEdit: 'Editar',
      btnDeactivate: 'Desativar',
      btnReactivate: 'Reativar',
      btnReactivateDisabledTitle: 'Sem slot livre — faça upgrade ou desative outro',
      emptyFiltered: 'Nenhum usuário encontrado',
      emptyAll: 'Nenhum usuário cadastrado',
      deleteUserTitle: 'Excluir usuário permanentemente?',
      deleteUserDesc: 'Esta ação é irreversível. O usuário {name} será removido permanentemente do sistema, incluindo login, permissões e vínculos.',
      deletePresetTitle: 'Excluir cargo?',
      deletePresetDesc: 'O cargo {name} será removido. Usuários que estavam vinculados a ele mantêm as permissões individuais. Esta ação não pode ser desfeita.',
      deleteLoading: 'Excluindo...',
      deletePermanently: 'Excluir Permanentemente',
      cancel: 'Cancelar',
      delete: 'Excluir',
      toastCreated: 'Usuário criado com sucesso!',
      toastCreateError: 'Erro ao criar usuário',
      toastCreateErrorDuplicate: 'Este e-mail já está cadastrado no sistema.',
      toastUpdated: 'Usuário atualizado!',
      toastUpdateError: 'Erro',
      toastDeactivated: 'Usuário desativado',
      toastDeactivatedDesc: 'O slot foi liberado. Você pode reativá-lo depois.',
      toastDeactivateError: 'Erro ao desativar',
      toastReactivated: 'Usuário reativado!',
      toastReactivateError: 'Erro ao reativar',
      toastDeleted: 'Usuário excluído permanentemente!',
      toastDeleteError: 'Erro ao excluir',
      toastPresetDuplicated: 'Cargo duplicado!',
      toastPresetDuplicateError: 'Erro ao duplicar',
      toastPresetDeleted: 'Cargo excluído!',
      toastPresetDeleteError: 'Erro ao excluir',
      blockLastAdminTitle: 'Ação bloqueada',
      blockLastAdminDesc: 'A empresa precisa de pelo menos um administrador ativo.',
      btnDeleteTitle: 'Excluir usuário permanentemente',

      // ── Dialog de formulário de usuário ──────────────────────────────────────
      form: {
        titleCreate: 'Criar Usuário',
        titleEdit: 'Editar Usuário',
        descCreate: 'Preencha os dados do novo usuário',
        descEdit: 'Atualize os dados e permissões do usuário',
        cancel: 'Cancelar',
        save: 'Atualizar',
        create: 'Criar Usuário',
        photoLabel: 'Foto do Usuário',
        photoReplace: 'Substituir',
        photoSelect: 'Selecionar foto',
        photoRemove: 'Remover',
        photoHint: 'Foto opcional do usuário',
        labelFullName: 'Nome Completo *',
        placeholderFullName: 'Nome do usuário',
        labelPassword: 'Senha *',
        placeholderPassword: 'Crie uma senha segura',
        labelEmail: 'Email',
        labelEmailRequired: 'Email *',
        labelEmailHint: 'Alterar o email mudará o login de acesso do usuário',
        labelPhone: 'Telefone',
        labelEmployee: 'Vincular a Funcionário',
        placeholderEmployee: 'Nenhum funcionário vinculado',
        employeeNone: 'Nenhum',
        employeeHint: 'Vincula este usuário a um funcionário cadastrado',
        emailConflictTitle: 'Os emails são diferentes. Qual deve prevalecer?',
        emailConflictUser: 'Usuário:',
        emailConflictEmployee: 'Funcionário:',
        emailConflictHint: 'O email escolhido será aplicado em ambos os cadastros',
        labelUserType: 'Tipo de Usuário',
        typeInternal: 'Interno',
        typeTech: 'Técnico',
        descTech: 'Aparece na listagem de técnicos da OS e pode ser adicionado a equipes',
        descInternal: 'Usuário interno do sistema, não aparece como técnico nas OS',
        labelProfile: 'Perfil de Acesso',
        placeholderProfile: 'Selecione um perfil',
        profileCustom: 'Personalizado',
        profileAll: 'Acesso Total',
        sectionScreens: 'Telas',
        hintScreens: 'Selecione quais telas este usuário pode acessar',
        sectionFunctions: 'Funções',
        hintFunctions: 'Selecione quais ações este usuário pode realizar',
        sectionPermissions: 'Permissões',
        hintPermissions: 'Escolha um perfil rápido ou ligue tela por tela. As ações de cada tela ficam dentro dela.',
      },

      // ── Editor de permissões (compartilhado usuário/cargo) ────────────────────
      permissionsEditor: {
        quickProfiles: 'Perfis rápidos',
        fullAccess: 'Acesso Total',
        fullAccessHint: 'Libera todas as telas e ações, inclusive as que forem criadas no futuro',
        custom: 'Personalizado',
        searchPlaceholder: 'Buscar tela ou ação...',
        clearSearch: 'Limpar busca',
        actionsTitle: 'Ações desta tela',
        screenOffHint: 'A tela está desligada, mas estas ações continuam valendo por conta própria.',
        emptySearch: 'Nenhuma tela ou ação encontrada para "{query}".',
        generalGroup: 'Geral',
        ariaScreenSwitch: 'Acesso à tela {screen}',
        ariaScreenActions: 'Ver ações de {screen}',
      },

      // ── Dialog de cargos/presets ──────────────────────────────────────────────
      presets: {
        dialogTitle: 'Configurações de Cargos',
        dialogDesc: 'Crie e edite perfis de acesso com permissões pré-definidas',
        btnCreate: 'Novo Cargo',
        btnSave: 'Salvar',
        btnSaveNew: 'Criar Cargo',
        btnCancel: 'Cancelar',
        btnBack: '← Voltar',
        empty: 'Nenhum cargo cadastrado',
        permCount: '{count} permissões',
        labelName: 'Nome *',
        placeholderName: 'Ex: Técnico de Campo',
        labelDesc: 'Descrição',
        placeholderDesc: 'Descrição opcional',
        sectionScreens: 'Telas',
        sectionFunctions: 'Funções',
        emptyMobileTitle: 'Nenhum cargo cadastrado',
        emptyMobileDesc: 'Toque em "Novo Cargo" para criar um perfil de acesso',
        noDescription: 'Sem descrição',
        actionEdit: 'Editar',
        actionDuplicate: 'Duplicar',
        actionDelete: 'Excluir',
      },

      // ── Lista mobile de usuários ──────────────────────────────────────────────
      list: {
        ariaViewPhoto: 'Ver foto',
        emptyFilteredDesc: 'Tente uma busca diferente',
        emptyAllDesc: 'Toque em "Novo Usuário" para começar',
        reactivateNoSlot: 'Reativar (sem slot)',
      },
    },

    // ── Tela Perfil ───────────────────────────────────────────────────────────
    profile: {
      pageTitle: 'Meu Perfil',
      pageSubtitle: 'Gerencie suas informações pessoais',
      labelFullName: 'Nome Completo',
      placeholderFullName: 'Seu nome',
      labelEmail: 'Email',
      labelPhone: 'Telefone',
      placeholderPhone: '(00) 00000-0000',
      labelCreatedAt: 'Conta criada em',
      btnSave: 'Salvar Alterações',
      btnSaving: 'Salvando...',
      cardPasswordTitle: 'Senha',
      cardPasswordDesc: 'Altere a senha da sua conta',
      btnChangePassword: 'Alterar Senha',
      labelNewPassword: 'Nova Senha',
      placeholderNewPassword: 'Crie uma senha segura',
      labelConfirmPassword: 'Confirmar Nova Senha',
      placeholderConfirmPassword: 'Repita a nova senha',
      btnSavePassword: 'Salvar Nova Senha',
      btnCancel: 'Cancelar',
      toastProfileUpdated: 'Perfil atualizado com sucesso!',
      toastSaveError: 'Erro ao salvar',
      toastPhotoUpdated: 'Foto atualizada!',
      toastPhotoError: 'Erro ao enviar foto',
      toastPasswordWeak: 'Senha fraca',
      toastPasswordWeakDesc: 'Use ao menos 8 caracteres com letras maiúsculas, minúsculas, números e/ou caracteres especiais.',
      toastPasswordMismatch: 'As senhas não coincidem',
      toastPasswordChanged: 'Senha alterada com sucesso!',
      toastPasswordError: 'Erro ao alterar senha',
      btnBack: 'Voltar',
    },

    // ── Tela Assinatura/Cobrança (Billing) ───────────────────────────────────
    billing: {
      // Cabeçalho da tela
      pageTitle: 'Assinatura',
      pageSubtitle: 'Gerencie seu plano, módulos e pagamentos',
      // Hero de trial
      heroTrialTitle: 'Ative sua Assinatura',
      heroTrialDesc: 'Você está no período de teste. Escolha o plano ideal e garanta acesso completo.',
      heroTrialDaysLeft: '{days} dia{s} restante{s} de teste',
      btnChoosePlan: 'Escolher Plano e Ativar',
      // Hero plano ativo
      planLabel: 'Plano {name}',
      // Status badges
      statusExpired: 'Vencida',
      statusExpiringSoon: 'Vence em breve',
      statusActive: 'Ativa',
      statusMsgExpired: 'Vencida há {days} dia{s}',
      statusMsgExpiringSoon: 'Vence em {days} dia{s}',
      statusMsgActive: '{days} dias restantes',
      statusMsgActiveNoDate: 'Ativa',
      // Cards de detalhe no hero
      labelMonthlyValue: 'Valor mensal',
      labelExpiration: 'Vencimento',
      // Seção Módulos Ativos
      sectionActiveModules: 'Módulos Ativos',
      // Seção Módulos Disponíveis
      sectionAvailableModules: 'Módulos Disponíveis',
      btnHireModule: 'Contratar',
      // Card Pagamento
      cardPaymentTitle: 'Pagamento',
      cardPaymentDescExpired: 'Sua assinatura está vencida. Renove agora.',
      cardPaymentDescActive: 'Mantenha sua assinatura em dia.',
      labelAmountDue: 'Valor a pagar',
      labelDueDate: 'Vencimento',
      btnPayNow: 'Pagar Agora',
      paymentYearlyDiscount: '20% de desconto no plano anual · válido para Pix ou Boleto',
      // Banners informativos (entre o hero e o restante)
      bannerPromoTitle: 'Preço promocional ativo',
      bannerPromoDesc: 'Após o período promocional, o valor voltará para {value}.',
      bannerPendingTitle: 'Mudança de valor agendada',
      bannerPendingDesc: 'Valor atual: {current}. A partir da próxima cobrança: {new}.',
      bannerRecurringTitle: 'Renovação automática ativa',
      bannerRecurringDesc: 'Sua assinatura é renovada automaticamente no cartão cadastrado.',
      bannerNfseTitle: 'Dados pendentes para emissão de notas fiscais',
      bannerNfseDesc: 'Preencha nas configurações da empresa: {fields}',
      // Card Resumo do Plano
      cardSummaryTitle: 'Resumo do Plano',
      summaryPlan: 'Plano',
      summaryActiveModules: 'Módulos ativos',
      summaryMaxUsers: 'Máx. Usuários',
      summaryCycle: 'Ciclo',
      summaryStatus: 'Status',
      cycleYearly: 'Anual',
      cycleMonthly: 'Mensal',
      // Histórico de pagamentos
      cardPaymentHistoryTitle: 'Histórico de pagamentos',
      cardPaymentHistoryDesc: 'Cobranças e pagamentos da sua assinatura.',
      // Cancelar assinatura
      btnCancelSubscription: 'Cancelar assinatura',
      cancelSubscriptionNote: 'Ao cancelar, a renovação automática para e você mantém acesso até o vencimento.',
      // Botão gerenciar plano (ModulesManagementCard)
      btnManagePlan: 'Gerenciar Meu Plano',
      // Nível de NFS-e no gerenciar plano
      nfseTierSectionTitle: 'Nível de Notas Fiscais (NFS-e)',
      nfseTierCurrent: 'atual',
      nfseTierUpgradeNote: 'O novo nível é liberado imediatamente e o valor entra na próxima cobrança.',
      // Painel "Seu Plano" ao vivo (ModulesManagementCard)
      planSummaryTitle: 'Seu Plano',
      planSummaryIncluded: 'Incluso',
      planSummaryEmptyCustom: 'Escolha módulos à esquerda para montar seu plano.',
      planSummaryEmptyPlans: 'Selecione um plano pronto à esquerda.',
      planSummaryUsersOne: 'usuário',
      planSummaryUsersPlural: 'usuários',
      planSummaryUsersIncluded: 'inclusos',
      planSummaryCycleLabel: 'Ciclo',
      planSummaryCycleYearlyDesc: '20% de desconto no Pix/Boleto',
      planSummaryCycleYearlyInfo: 'O desconto de 20% vale só para pagamento à vista (Pix ou Boleto). No cartão, a cobrança é mensal.',
      planSummaryYearlySavings: 'Você economiza {value}/ano',
      planSummaryTotalMonthly: 'Total mensal',
      planSummaryTotalYearly: 'Total anual',
      planSummaryDiffLabel: 'vs plano atual',
      planSummaryCtaMonthly: 'Aplicar plano',
      planSummaryCtaYearly: 'Pagar e ativar (anual)',
      planSummaryCtaApplying: 'Aplicando...',
      planSummaryUpgradeNote: 'Upgrade: os recursos são liberados na hora e o novo valor já entra na próxima cobrança.',
      planSummaryDowngradeNote: 'Downgrade agendado: você mantém o plano atual até o fim do período já pago. O novo valor de {value}/mês passa a valer na próxima cobrança.',
      // Estado vazio
      noCompanyTitle: 'Nenhuma empresa vinculada',
      noCompanyDesc: 'Sua conta ainda não está vinculada a uma empresa.',

      // ── Modal de cancelamento de assinatura (CancelSubscriptionModal) ────────
      cancelSubscription: {
        titleReason: 'Cancelar assinatura',
        titleConfirm: 'Confirmar cancelamento',
        titleDone: 'Cancelamento concluído',
        headerSubtitle: 'Nos ajude a entender o motivo',
        labelReason: 'Motivo do cancelamento',
        placeholderReason: 'Selecione o motivo',
        labelDetailsRequired: 'Explique seu motivo *',
        labelDetailsOptional: 'Quer nos contar mais? (opcional)',
        placeholderDetailsRequired: 'Descreva o motivo do cancelamento...',
        placeholderDetailsOther: 'Alguma sugestão ou comentário...',
        // Retenção
        retentionTalkSupport: 'Falar com o suporte',
        // Step "confirmar"
        confirmOnConfirm: 'Ao confirmar:',
        confirmBullet1: 'A renovação automática será cancelada',
        confirmBullet2Until: 'Você continua com acesso até',
        confirmBullet2NoDate: 'o fim do período já pago',
        confirmBullet3: 'Cobranças futuras em aberto serão canceladas automaticamente',
        confirmBullet4: 'Você pode reativar quando quiser',
        confirmReasonPrefix: 'Motivo:',
        confirmDetailsPrefix: 'Detalhes:',
        // Step "concluído"
        doneTitle: 'Assinatura cancelada',
        doneDesc: 'A renovação automática foi cancelada. Você continua com acesso',
        doneDescUntil: 'até {date}',
        doneDescNoDate: 'até o fim do período já pago',
        doneDescSuffix: '. Se mudar de ideia, é só reativar a qualquer momento.',
        // Botões
        btnBack: 'Voltar',
        btnContinue: 'Continuar',
        btnCanceling: 'Cancelando...',
        btnConfirm: 'Confirmar cancelamento',
        btnUnderstood: 'Entendido',
        // Toast de erro
        toastError: 'Não foi possível cancelar agora. Tente novamente ou fale com o suporte.',
        toastSelectReason: 'Selecione um motivo',
        // Motivos
        reasons: {
          preco_alto: 'Preço muito alto',
          nao_uso: 'Não estou usando o suficiente',
          funcionalidades: 'Faltam funcionalidades que preciso',
          dificuldade_uso: 'Dificuldade em usar o sistema',
          concorrente: 'Encontrei outra solução',
          fechando_empresa: 'Estou fechando a empresa',
          temporario: 'Pausa temporária',
          outro: 'Outro motivo',
        },
        // Mensagens de retenção
        retention: {
          preco_alto: {
            title: 'Que tal um desconto?',
            message: 'Fale com a gente pelo WhatsApp — conseguimos negociar um valor especial pra você continuar no Dominex.',
            waText: 'Olá! Penso em cancelar minha assinatura do Dominex por causa do preço. Tem alguma condição especial?',
          },
          nao_uso: {
            title: 'Podemos te ajudar!',
            message: 'Nossa equipe faz um treinamento personalizado gratuito pra você aproveitar o sistema ao máximo.',
            waText: 'Olá! Quero agendar um treinamento do Dominex antes de decidir sobre minha assinatura.',
          },
          funcionalidades: {
            title: 'Sua opinião é valiosa!',
            message: 'Conte quais funcionalidades você precisa. Estamos sempre evoluindo e sua sugestão pode entrar em breve!',
            waText: 'Olá! Sinto falta de algumas funcionalidades no Dominex e gostaria de sugerir antes de cancelar.',
          },
          dificuldade_uso: {
            title: 'Suporte dedicado',
            message: 'Podemos agendar uma sessão de suporte dedicada pra resolver suas dúvidas e facilitar o uso.',
            waText: 'Olá! Tenho dificuldade em usar o Dominex e gostaria de uma sessão de suporte antes de cancelar.',
          },
        },
        // Oferta de downgrade
        downgradeOfferTitle: 'Que tal reduzir seu plano?',
        downgradeOfferDesc: 'Em vez de cancelar, mude para o plano {plan} e economize {savings} por mês.',
        downgradeOfferCta: 'Mudar para {plan}',
        downgradeOfferApplying: 'Aplicando...',
        downgradeOfferToastSuccess: 'Plano alterado para {plan} com sucesso.',
        downgradeOfferToastError: 'Não foi possível alterar o plano agora. Tente novamente.',
      },

      // ── Lista de histórico de pagamentos (PaymentHistoryList) ─────────────────
      paymentHistory: {
        empty: 'Nenhum pagamento registrado ainda.',
        perPage: 'Por página',
        pageSummary: 'Página {current} de {total}',
        prevPage: 'Página anterior',
        nextPage: 'Próxima página',
        viewCharge: 'Ver cobrança',
        // Status das cobranças
        statusPaid: 'Pago',
        statusPending: 'Pendente',
        statusOverdue: 'Vencido',
        statusCancelled: 'Cancelado',
        // Tipo de cobrança
        typeSale: 'Venda',
        typeRenewal: 'Renovação',
        // Métodos de pagamento
        methodPix: 'PIX',
        methodBoleto: 'Boleto',
        methodCard: 'Cartão',
        methodDefault: 'Cobrança',
      },

      // ── Card "Uso da conta" (AccountUsageCard) ────────────────────────────────
      accountUsage: {
        title: 'Uso da conta',
        description: 'Acompanhe o consumo do seu plano',
        users: 'Usuários',
        nfseThisMonth: 'Notas fiscais este mês',
        usersIncluded: '{base} incluídos + {extra} adicional',
        usersIncludedPlural: '{base} incluídos + {extra} adicionais',
        usersPlanLimit: '{max} usuário no plano atual',
        usersPlanLimitPlural: '{max} usuários no plano atual',
        nfseUnlimitedLabel: '{used} emitidas',
        nfseLevelLimit: 'Limite do Nível {level} — {limit} notas/mês',
        nfsePlanLimit: 'Limite do plano — {limit} notas/mês',
        paymentMethods: 'Formas de pagamento',
      },
    },

    // ── Chrome do Changelog ───────────────────────────────────────────────────
    changelog: {
      pageTitle: 'Novidades',
      searchPlaceholder: 'Buscar atualizações...',
      currentVersionBadge: 'Versão atual',
      filterAll: 'Todas',
      filterRecurso: 'Recursos',
      filterMelhoria: 'Melhorias',
      filterCorrecao: 'Correções',
      filterSeguranca: 'Segurança',
      categoryRecurso: 'Nova Funcionalidade',
      categoryMelhoria: 'Melhoria',
      categoryCorrecao: 'Correção',
      categorySeguranca: 'Segurança',
      emptyTitle: 'Nenhuma atualização encontrada',
      emptyDesc: 'Tente uma busca diferente ou mude os filtros.',
      changesCount_one: '{count} alteração',
      changesCount_other: '{count} alterações',
      showMore: 'Ver mais',
      showLess: 'Ver menos',
    },

    // ── Chrome do Modal/PDF de Termos de Uso ─────────────────────────────────
    terms: {
      modalTitle: 'Termos de Uso — Dominex',
      pdfTitle: 'TERMOS DE USO — DOMINEX',
      pdfFilename: 'TERMOS DE USO - DOMINEX',
      checkboxLabel: 'Li e concordo com os Termos de Uso do Dominex',
      btnAccept: 'Aceito os Termos',
      btnAccepting: 'Confirmando...',
      btnDownloadPdf: 'Baixar PDF',
      btnGeneratingPdf: 'Gerando PDF...',
      btnClose: 'Fechar',
      acceptedAt: 'Aceito em {date}',
      pdfFooterPage: 'Página {current} de {total}',
      pdfGeneratedOn: 'Documento gerado em {date}',
      errorPdf: 'Erro ao gerar o PDF dos termos. Tente novamente.',
    },

    // ── Tela Meus Dados (LGPD) ────────────────────────────────────────────────
    myData: {
      backToDashboard: '← Voltar ao painel',
      pageTitle: 'Central de Privacidade',
      pageSubtitle: 'Gerencie seus dados pessoais (Art. 18 LGPD)',
      // Card dados pessoais
      cardPersonalTitle: 'Seus dados pessoais',
      labelName: 'Nome',
      labelEmail: 'E-mail',
      labelPhone: 'Telefone',
      labelCreatedAt: 'Conta criada em',
      // Card exportar dados
      cardExportTitle: 'Exportar meus dados',
      cardExportDesc: 'Baixe uma cópia dos seus dados em formato JSON (portabilidade — Art. 18 V LGPD).',
      btnExport: 'Baixar meus dados',
      btnExporting: 'Exportando...',
      // Card solicitar exclusão
      cardDeleteTitle: 'Solicitar exclusão da conta',
      cardDeleteDesc: 'Solicite a exclusão permanente da sua conta e dados pessoais (Art. 18 VI LGPD). Dados fiscais e contábeis podem ser mantidos pelo prazo legal (5 anos). O processamento ocorre em até 15 dias úteis.',
      deletionRequested: 'Solicitação registrada. Nossa equipe entrará em contato.',
      btnRequestDeletion: 'Solicitar exclusão',
      btnRequesting: 'Registrando...',
      // Card DPO
      cardDpoTitle: 'Contato com o DPO',
      cardDpoDesc: 'Para exercer outros direitos (correção, portabilidade, revogação de consentimento) ou tirar dúvidas sobre o tratamento dos seus dados:',
      // Links legais
      linkPrivacy: 'Política de Privacidade',
      linkTerms: 'Termos de Uso',
      // Toasts
      toastExported: 'Dados exportados com sucesso!',
      toastExportError: 'Erro ao exportar dados',
      toastDeletionRequested: 'Solicitação de exclusão enviada',
      toastDeletionRequestedDesc: 'Nossa equipe processará sua solicitação em até 15 dias úteis conforme o Art. 18 da LGPD.',
      toastDeletionError: 'Erro ao registrar solicitação',
    },

    // ── Aba Integrações ───────────────────────────────────────────────────────
    integrations: {
      subTabs: {
        whatsapp: 'WhatsApp',
        asaas: 'Recebimentos',
        simulador: 'Simulador',
      },
      simulator: {
        title: 'Simulador de venda',
        description: 'Veja quanto sobra de uma venda depois da taxa da Asaas e quando o dinheiro cai na sua conta.',
        notConnectedTitle: 'Sua conta Asaas ainda não está conectada',
        notConnectedDesc: 'Os números abaixo usam a tabela de referência da Asaas. Conecte a sua conta na aba Recebimentos para simular com as taxas reais do seu plano.',
        notContractedTitle: 'Recebimentos não contratado',
        notContractedDesc: 'O simulador mostra quanto sobra de cada venda por Pix, boleto ou cartão. Fale com nosso time para liberar as cobranças online.',
        amountLabel: 'Valor da venda',
        methodLabel: 'Forma de pagamento',
        methodPix: 'Pix',
        methodBoleto: 'Boleto',
        methodCard: 'Cartão',
        installmentsLabel: 'Parcelas',
        installmentsOption: '{n}x',
        feePayerLabel: 'Quem paga a taxa',
        feePayerCompany: 'A empresa absorve',
        feePayerCustomer: 'Repassar ao cliente',
        anticipateLabel: 'Antecipar o recebimento',
        anticipateDesc: 'Receba tudo em 1 dia útil pagando a taxa de antecipação da Asaas.',
        anticipateUnavailable: 'A Asaas não informou a taxa de antecipação da sua conta. Usamos a taxa de referência.',
        resultTitle: 'Resultado da simulação',
        resultGross: 'Valor bruto',
        resultCustomerPays: 'O cliente paga',
        resultFee: 'Taxa da Asaas',
        resultNet: 'Valor líquido na sua conta',
        resultInstallment: 'Cada parcela para o cliente',
        resultAnticipationCost: 'Custo da antecipação',
        resultNetAnticipated: 'Líquido antecipado',
        resultEffectiveCost: 'Custo total da venda',
        compareTitle: 'Com e sem antecipação',
        compareWithout: 'Sem antecipar',
        compareWith: 'Antecipando',
        compareWhen: 'quando cai',
        scheduleTitle: 'Quando o dinheiro cai na sua conta',
        scheduleInstallment: 'Parcela {n}',
        scheduleSingle: 'Pagamento',
        scheduleTotal: 'Total',
        settlementSameDay: 'na hora',
        settlementInDay: 'em 1 dia útil ({date})',
        settlementInDays: 'em {count} dias ({date})',
        settlementBusinessDayNote: 'Fim de semana e feriado não contam: se a data cair em sábado, domingo ou feriado, o crédito vai para o próximo dia útil.',
        settlementFallbackNote: 'Prazo de referência, a Asaas não informa esse prazo pela API.',
        referenceWarning: 'Alguns valores vêm da tabela de referência da Asaas, não da sua conta.',
        pixFreeNote: 'As primeiras {count} recebidas por Pix no mês não pagam tarifa, então nesses casos o líquido é o valor cheio.',
        disclaimer: 'Estimativa. O valor final é calculado pela Asaas no momento da cobrança.',
        emptyAmount: 'Digite o valor da venda para ver a simulação.',
      },
      asaas: {
        loading: 'Carregando…',
        notContracted: {
          title: 'Recebimentos não contratado',
          description: 'Este recurso não está incluído no seu plano atual. Fale com nosso time para cobrar seus clientes online por Pix, boleto e cartão.',
        },
        card: {
          title: 'Recebimentos (Asaas)',
          description: 'Cobre seus clientes por Pix, boleto e cartão usando sua própria conta Asaas.',
        },
        status: {
          active: 'Ativo',
          pending: 'Em verificação',
          disabled: 'Desativado',
          rejected: 'Recusado',
        },
        inactive: {
          intro: 'Cole a chave de API da sua conta Asaas para ativar os recebimentos. Você gera essa chave no painel da Asaas, em Configurações, Integrações.',
          apiKeyLabel: 'Chave de API da Asaas',
          apiKeyPlaceholder: 'Cole aqui sua chave de API',
          activate: 'Ativar recebimentos',
          activating: 'Ativando…',
          helpLink: 'Onde encontro minha chave?',
          apiKeyGuide: {
            guideTitle: 'Como obter sua chave de API da Asaas',
            trigger: 'Como consigo minha chave de API da Asaas?',
            step1: 'Acesse asaas.com e faça login (ou crie sua conta gratuita).',
            step2: 'No menu, vá em "Configurações da conta" (ícone de engrenagem), depois em "Integrações".',
            step3: 'Na seção "Chave de API", clique em "Gerar chave de API" ou copie a chave existente.',
            step4: 'Copie a chave (começa com "$aact_") e cole no campo acima. A chave é secreta, guarde com cuidado.',
            sandboxHint: 'Para testar sem dinheiro real, use a conta sandbox em sandbox.asaas.com pelo mesmo caminho.',
          },
        },
        activeState: {
          connected: 'Recebimentos ativos. Já é possível cobrar seus clientes.',
          pendingDesc: 'Sua conta está em verificação na Asaas. Assim que aprovada, os recebimentos ficam ativos.',
          feesTitle: 'Taxas da Asaas',
          feesNote: 'As taxas são cobradas pela Asaas diretamente na sua conta. A Dominex não retém nada por transação.',
          feePix: 'Pix',
          feePixValue: 'R$ 0,99 a R$ 1,99 por recebimento',
          feeBoleto: 'Boleto',
          feeBoletoValue: 'R$ 0,99 a R$ 1,99',
          feeCard: 'Cartão de crédito',
          feeCardValue: '1,99% a 4,29% + R$ 0,49 por cobrança',
          feeAnticipation: 'Antecipação',
          feeSettlement: 'Prazo para o dinheiro cair',
          feeNotInformed: 'Não informado pela Asaas',
          feeReferenceTag: 'referência',
          feeAnticipationValue: '1,25% a 1,70% ao mês (cartão), 5,79% ao mês (boleto e Pix)',
          feeSettlementValue: 'Pix na hora, boleto em 1 dia, cartão em 32 dias',
          feesFromAccount: 'Taxas da sua conta Asaas, atualizadas em',
          feesFromAccountNoDate: 'Taxas lidas da sua conta Asaas.',
          feesFromOverride: 'Taxas ajustadas manualmente para a sua empresa.',
          feesFromReference: 'Taxas de referência, não conseguimos ler as da sua conta agora.',
          feesRefresh: 'Atualizar taxas',
          feesRefreshing: 'Atualizando…',
          feesRefreshOk: 'Taxas atualizadas com a Asaas.',
          feesRefreshError: 'Não foi possível ler as taxas da sua conta Asaas agora.',
          feePixFreeNote: 'As primeiras {count} recebidas no mês não pagam tarifa.',
          feePerCharge: 'por cobrança',
          feeOneShot: 'à vista',
          feeInstallments: 'parcelado',
          feeMonthly: 'ao mês',
          settlementSameDay: 'na hora',
          settlementInDay: 'em 1 dia',
          settlementInDays: 'em {count} dias',
          financeAccountRequiredError: 'Escolha a conta que vai receber o dinheiro, sem isso a receita não entra no seu Financeiro.',
          financeAccountPlaceholder: 'Escolha a conta…',
          financeAccountPendingTitle: 'Falta escolher a conta que recebe o dinheiro',
          financeAccountPendingDesc: 'Enquanto você não escolher, as cobranças pagas não entram sozinhas no seu Financeiro.',
          financeAccountPendingCta: 'Escolher agora',
          financeAccountEmptyTitle: 'Você ainda não tem conta bancária cadastrada',
          financeAccountEmptyDesc: 'Cadastre a conta em Financeiro, aba Movimentações, para escolher onde a receita vai cair.',
          financeAccountEmptyCta: 'Abrir o Financeiro',
          dueDaysError: 'Informe em quantos dias a cobrança vence (0 ou mais).',
          maxInstallmentsError: 'Informe o número máximo de parcelas (1 ou mais).',
          discountPercentError: 'O desconto precisa ser um número entre 0 e 100.',
          discountDaysError: 'Informe quantos dias antes do vencimento o desconto vale (0 ou mais).',
          deactivate: 'Desativar recebimentos',
          deactivating: 'Desativando…',
          autoPostLabel: 'Lançar cobranças no Financeiro automaticamente',
          autoPostDesc: 'Cada cobrança vira um "a receber" que baixa sozinho quando o cliente paga.',
          autoPostToastOn: 'Lançamento automático ativado.',
          autoPostToastOff: 'Lançamento automático desativado.',
          autoPostToastError: 'Não foi possível salvar a configuração. Tente novamente.',
          autoFeeLabel: 'Lançar a taxa da Asaas como despesa',
          autoFeeDesc: 'Ao receber, a tarifa cobrada pela Asaas entra como despesa no seu Financeiro, para o caixa bater certo.',
          autoFeeToastOn: 'Lançamento de taxa ativado.',
          autoFeeToastOff: 'Lançamento de taxa desativado.',
          autoFeeError: 'Não foi possível salvar a configuração. Tente novamente.',
          lateFeesTitle: 'Multa e juros por atraso',
          lateFeesDesc: 'Aplicado nas próximas cobranças quando o cliente paga em atraso.',
          fineLabel: 'Multa por atraso (%)',
          interestLabel: 'Juros ao mês (%)',
          interestCapWarning: 'O limite máximo da Asaas é 10% ao mês.',
          lateFeesSave: 'Salvar',
          lateFeesSaving: 'Salvando…',
          lateFeesToastOk: 'Multa e juros atualizados.',
          lateFeesToastError: 'Não foi possível salvar. Tente novamente.',
          // ── Preferências de cobrança ──────────────────────────────────────
          save: 'Salvar',
          saving: 'Salvando…',
          chargePrefsTitle: 'Preferências de cobrança',
          chargePrefsDesc: 'Configurações padrão aplicadas em novas cobranças.',
          dueDaysLabel: 'Vencimento padrão',
          dueDaysSuffix: 'dias',
          defaultDescriptionLabel: 'Descrição/instrução padrão',
          defaultDescriptionPlaceholder: 'Ex.: Referente ao serviço de manutenção…',
          paymentMethodsLabel: 'Meios de pagamento habilitados',
          methodPix: 'Pix',
          methodBoleto: 'Boleto',
          methodCard: 'Cartão de crédito',
          maxInstallmentsLabel: 'Máximo de parcelas (cartão)',
          maxInstallmentsSuffix: 'x',
          chargePrefsToastOk: 'Preferências de cobrança salvas.',
          // ── Desconto por antecipação ──────────────────────────────────────
          discountTitle: 'Desconto por antecipação',
          discountDesc: 'Ofereça desconto para clientes que pagarem antes do vencimento.',
          discountPercentLabel: 'Desconto (%)',
          discountPercentPlaceholder: 'Ex.: 5',
          discountDaysLabel: 'Até X dias antes',
          discountDaysPlaceholder: 'Ex.: 10',
          discountDaysSuffix: 'dias',
          discountHint: 'Deixe em branco para não aplicar desconto.',
          discountToastOk: 'Desconto por antecipação salvo.',
          // ── Lançamento no Financeiro ──────────────────────────────────────
          financeConfigTitle: 'Lançamento no Financeiro',
          financeConfigDesc: 'Defina conta e categorias usadas ao baixar automaticamente cobranças pagas.',
          financeAccountLabel: 'Conta bancária de destino',
          financeAccountNone: 'Nenhuma',
          incomeCategoryLabel: 'Categoria da receita',
          incomeCategoryNone: 'Nenhuma',
          feeCategoryLabel: 'Categoria da taxa (Asaas)',
          feeCategoryHint: 'Categoria usada para lançar a tarifa cobrada pela Asaas como despesa.',
          financeConfigToastOk: 'Configuração de lançamento salva.',
        },
        deactivateConfirm: {
          title: 'Desativar recebimentos?',
          description: 'Cobranças já geradas continuam válidas, mas você não poderá criar novas até reativar.',
          confirm: 'Sim, desativar',
          cancel: 'Cancelar',
        },
        errors: {
          activateGeneric: 'Não foi possível ativar os recebimentos. Confira a chave e tente novamente.',
          deactivateGeneric: 'Não foi possível desativar os recebimentos.',
        },
        toasts: {
          activated: 'Recebimentos ativados!',
          deactivated: 'Recebimentos desativados.',
        },
      },
      whatsapp: {
        loading: 'Carregando…',
        comingSoon: {
          badge: 'Em breve',
          title: 'Avisos por WhatsApp',
          description: 'Em breve você poderá avisar seus clientes automaticamente pelo WhatsApp. Estamos finalizando os últimos ajustes.',
        },
        notContracted: {
          title: 'Avisos de WhatsApp não contratado',
          description: 'Este recurso não está incluído no seu plano atual. Fale com nosso time para liberar os avisos automáticos por WhatsApp aos seus clientes.',
        },
        masterToggle: {
          title: 'Avisos por WhatsApp',
          description: 'Ativar ou desativar todos os avisos enviados ao cliente via WhatsApp',
          ariaLabel: 'Ativar avisos por WhatsApp',
        },
        connection: {
          sectionTitle: 'Conexão',
          sectionDescription: 'Conecte o WhatsApp da sua empresa para enviar avisos aos clientes',
          statusConnected: 'Conectado',
          statusQr: 'Aguardando QR',
          statusDisconnected: 'Desconectado',
          statusBanned: 'Banido',
          btnConnect: 'Conectar WhatsApp',
          connecting: 'Conectando…',
          qrInstructions: 'Abra o WhatsApp no seu celular, acesse Menu > Aparelhos conectados > Conectar um aparelho e escaneie o QR code abaixo.',
          connectedDesc: 'WhatsApp conectado e pronto para enviar avisos.',
          bannedDesc: 'Este número foi banido pelo WhatsApp. Entre em contato com o suporte.',
          errorTitle: 'Não foi possível conectar',
          errorHint: 'Verifique sua conexão ou tente novamente em instantes. O servidor de integração pode estar sendo configurado.',
        },
        triggers: {
          sectionTitle: 'Gatilhos de Aviso',
          sectionDescription: 'Escolha em quais momentos da OS o cliente recebe um aviso automático',
          aCaminho: {
            label: 'Técnico a caminho',
            description: 'Avisa o cliente quando o técnico sai para atender a OS',
          },
          emAndamento: {
            label: 'Serviço iniciado',
            description: 'Avisa o cliente quando o técnico inicia o atendimento',
          },
          concluida: {
            label: 'OS finalizada',
            description: 'Avisa o cliente quando a ordem de serviço é concluída',
          },
        },
        quota: {
          sectionTitle: 'Plano e Cota',
          sectionDescription: 'Seu nível atual e o consumo de avisos no mês corrente',
          currentPlan: 'Plano atual',
          usageLabel: 'Avisos enviados este mês',
          unlimited: 'Ilimitado',
          nearLimitWarning: 'Você está próximo do limite mensal.',
          resetHint: 'A cota é renovada automaticamente no início de cada mês.',
        },
      },
    },

    // ── Central de Ajuda ──────────────────────────────────────────────────────
    help: {
      title: 'Central de Ajuda',
      intro: 'Dúvidas frequentes sobre o sistema. Caso não encontre sua resposta, entre em contato pelo Suporte via WhatsApp.',
      faqs: [
        {
          q: 'Como criar uma Ordem de Serviço?',
          a: 'Acesse "Ordens de Serviço" no menu lateral, clique em "+ Nova OS", preencha os dados do cliente, tipo de serviço e técnico responsável, depois salve.',
        },
        {
          q: 'Como cadastrar um novo cliente?',
          a: 'Vá em "Clientes" no menu, clique em "Novo Cliente" e preencha os dados. Você pode cadastrar pessoa física (CPF) ou jurídica (CNPJ).',
        },
        {
          q: 'Como funciona o controle financeiro?',
          a: 'No menu "Financeiro" você tem visão geral, movimentações, contas a pagar/receber e DRE. As OS podem gerar lançamentos financeiros automaticamente.',
        },
        {
          q: 'Como funcionam os pagamentos de funcionários?',
          a: 'Em "Funcionários", clique no ícone de pagamento no card do funcionário. O sistema calcula automaticamente salário + bônus - vales - faltas e permite selecionar a conta de débito.',
        },
        {
          q: 'Como configurar o controle de ponto?',
          a: 'Acesse Funcionários > Controle de Ponto. Defina horários padrão, tolerância de atraso e requisitos de selfie/geolocalização nas configurações.',
        },
        {
          q: 'Como criar contratos recorrentes?',
          a: 'Em "Contratos", crie um novo contrato definindo cliente, frequência (mensal, bimestral, etc.) e período. O sistema gera automaticamente as ocorrências programadas.',
        },
        {
          q: 'O que é o CRM e como usar?',
          a: 'O CRM é seu funil de vendas. Crie leads, mova entre etapas (Novo, Contato, Proposta, Ganho/Perdido) e acompanhe o valor do pipeline em tempo real.',
        },
        {
          q: 'Como funciona o módulo de Estoque?',
          a: 'Em "Estoque", cadastre itens com preço de custo/venda, quantidade mínima e unidade. O sistema alerta quando o estoque fica abaixo do mínimo.',
        },
        {
          q: 'Como exportar relatórios?',
          a: 'Diversos módulos possuem botões de exportação (PDF, HTML). No extrato de funcionários, orçamentos e DRE há opções de exportação e impressão.',
        },
        {
          q: 'Como alterar minha senha?',
          a: 'Acesse "Perfil" pelo menu do usuário (canto inferior da sidebar) e clique em "Alterar Senha". Você receberá um email para redefinição.',
        },
      ],
    },
  },

  // ═══════════════════════════════════════════════════════════════════════════
  // ENGLISH
  // ═══════════════════════════════════════════════════════════════════════════
  en: {
    page: {
      title: 'Settings',
      subtitle: 'Manage system settings',
      tabs: {
        empresa: 'Company',
        regional: 'Regional',
        usuarios: 'Users & Permissions',
        usabilidade: 'Usability',
        atalhos: 'Shortcuts',
        aparencia: 'Appearance',
        integracoes: 'Integrations',
      },
    },

    saveStatus: {
      saving: 'Saving…',
      unsaved: 'Unsaved changes',
      saved: 'Saved',
    },

    company: {
      cardTitle: 'Company Details',
      cardDescription: 'Company information shown on labels and documents',

      visualIdentity: {
        sectionTitle: 'Visual Identity',
        sectionDescription: 'Company logo for documents and the system',
        replace: 'Replace',
        remove: 'Remove',
        removeLogoTitle: 'Remove logo?',
        removeLogoDesc: 'The current logo will be permanently removed.',
        uploadPrompt: 'Click to upload logo',
        uploadHint: 'PNG, JPG up to 5MB',
        errorFileSize: 'File too large (max 5MB)',
        errorFileType: 'Only images are allowed',
        errorUpload: 'Error uploading logo',
        errorIconUpload: 'Error uploading icon',
      },

      registrationData: {
        sectionTitle: 'Registration Data',
        sectionDescription: 'Legal name, document number and official information',
        companyName: 'Company Name',
        companyNamePlaceholder: 'Your company name',
        cnpj: 'Tax ID',
        cnpjPlaceholder: '00.000.000/0000-00',
        showInDocs: 'Show in documents',
        segment: 'Business Segment',
        segmentNotDefined: 'Not defined',
        segmentHint: 'Defines which tools and features appear for your segment. To change it, contact Dominex.',
      },

      contact: {
        sectionTitle: 'Contact',
        sectionDescription: 'Company phone and email',
        phone: 'Phone',
        phonePlaceholder: '(00) 0000-0000',
        email: 'Email',
        emailPlaceholder: 'contact@company.com',
        showInDocs: 'Show in documents',
      },

      address: {
        sectionTitle: 'Address',
        sectionDescription: 'Physical location of the company',
        showInDocs: 'Show in documents',
        zip: 'ZIP Code',
        street: 'Street',
        streetPlaceholder: 'Street, Avenue…',
        number: 'Number',
        numberPlaceholder: 'No.',
        complement: 'Complement',
        complementPlaceholder: 'Suite, Floor…',
        neighborhood: 'Neighborhood',
        neighborhoodPlaceholder: 'Neighborhood',
        stateCity: 'State / City',
      },

      whiteLabelSection: {
        sectionTitle: 'White Label',
        sectionDescription: 'Customize the system with your brand identity',
        moduleUnavailable: 'Module not available in your current plan',
        hire: 'Upgrade',
        enableLabel: 'Enable White Label',
        enableDescription: 'Replaces the default system logo and color',
        fullLogo: 'Full logo',
        fullLogoConfigured: 'Custom logo configured',
        fullLogoUsingCompany: 'Using company logo by default',
        fullLogoFallback: 'Company logo will be used',
        uploadLogoPrompt: 'Upload logo (optional)',
        icon: 'Icon (1:1)',
        iconDescription: 'Square icon for the collapsed sidebar. 128×128px.',
        uploadIconPrompt: 'Upload icon',
        primaryColor: 'Primary color',
        primaryColorDescription: 'Replaces the default green color of the system',
        qrSection: {
          title: 'QR Code',
          description: 'Customize the look of the QR Codes generated by the system, with your brand logo in the center',
          dotStyleLabel: 'Dot style',
          dotStyle: {
            square: 'Square',
            rounded: 'Rounded',
            dots: 'Dots',
            classy: 'Classy',
          },
          cornerStyleLabel: 'Corners',
          cornerStyle: {
            square: 'Square',
            rounded: 'Rounded',
            dot: 'Dot',
          },
          colorLabel: 'QR Code color',
          previewCaption: 'Preview',
        },
        reportHeader: 'Service Report Header',
        reportHeaderDescription: 'Customize the header shown on the completed work order report',
        reportBgColor: 'Header background color',
        reportTextColor: 'Text color',
        reportStatusBarColor: 'Status bar color',
        reportLogoSize: 'Logo size ({size}px)',
        reportLogoBg: 'Background behind logo',
        reportLogoBgDescription: 'Removes the background when disabled',
        reportLogoBgColor: 'Background color',
        reportLogoType: 'Logo type in report',
        reportLogoTypeFull: 'Full Logo',
        reportLogoTypeIcon: 'Icon',
        reportLogoTypeHintFull: 'Uses the full company logo',
        reportLogoTypeHintIcon: 'Uses the icon configured in White Label',
      },

      legalDocs: {
        sectionTitle: 'Legal Documents',
        sectionDescription: 'View and download the terms governing the use of Dominex',
        viewTerms: 'View terms of use',
        acceptedAt: 'Accepted on {date}',
      },
    },

    usability: {
      cardTitle: 'Usability',
      cardDescription: 'System behavior preferences',
      preferenceSaved: 'Preference saved!',

      sections: {
        os: {
          title: 'Work Orders',
          description: 'Behaviors related to work orders',
          autoSaveOS: {
            title: 'Auto-save',
            description: 'Automatically save work order drafts while editing',
          },
          showOSValues: {
            title: 'Show Values',
            description: 'Display financial values (labor, parts) on work orders',
          },
          requireSignature: {
            title: 'Require Signature',
            description: 'Make customer signature mandatory when closing a work order',
          },
          saveOSPhotosToDevice: {
            title: 'Save photos to device',
            description: 'Shows a button to save the photo to your device. On iPhone it opens "Save Image"; on Android it downloads directly.',
          },
          osStockConsumptionEnabled: {
            title: 'Stock consumption on work orders',
            description: 'Lets the technician log the materials used during the work order and deduct them from stock when finishing it',
          },
        },
        interface: {
          title: 'Interface',
          description: 'Visual preferences for lists and tables',
          compactTables: {
            title: 'Compact Tables',
            description: 'Reduce table row spacing to show more data per page',
          },
          showEquipmentPhotos: {
            title: 'Equipment Photos',
            description: 'Show equipment photo thumbnails in lists',
          },
        },
        security: {
          title: 'Security',
          description: 'Confirmations and security validations',
          confirmDelete: {
            title: 'Confirm Deletions',
            description: 'Show a confirmation dialog before deleting records',
          },
        },
        schedule: {
          title: 'Schedule',
          description: 'Calendar and scheduling settings',
          showHolidays: {
            title: 'Show Holidays',
            description: 'Display national and local holidays on the schedule',
          },
        },
      },

      origins: {
        cardTitle: 'Sources',
        cardDescription: 'Source list used in customer registration and CRM opportunities',
        manage: 'Manage sources',
        empty: 'No sources registered. Create an initial set and edit it freely afterwards.',
        seedButton: 'Create default sources',
        seedLoading: 'Creating…',
        inactive: 'Inactive',
      },
    },

    regional: {
      personalCard: {
        title: 'My language',
        description: 'The language YOU see in the system. Applies immediately, for you only.',
        languageLabel: 'Language',
        languageHint: 'Personal preference. Overrides the company default language for you only.',
      },
      companyCard: {
        title: 'Company defaults',
        description: 'Default language, currency and timezone for the entire company',
        adminOnly: '(admin only)',
        languageLabel: 'Default company language',
        languageHint: 'Language applied to all users who have not chosen one personally.',
        localeDefaultsPrompt: 'Apply {locale} defaults for currency and timezone?',
        keepCurrent: 'Keep current',
        useDefaults: 'Use defaults',
        localeDefaultsApplied: '{locale} defaults applied',
        currencyLabel: 'Currency',
        currencyHint: 'Operating currency used for values in the system.',
        currencyChangedWarning: 'The currency is the company operating currency. Changing it does not convert already-recorded values, they keep their original numbers.',
        timezoneLabel: 'Timezone',
        timezoneHint: 'Used for dates and times displayed in the system.',
        timezonePlaceholder: 'Select timezone',
        timezoneSearchPlaceholder: 'Search timezone (e.g. New_York)',
        timezoneEmpty: 'No timezone found.',
        savingStatus: 'Saving...',
        unsavedStatus: 'Unsaved changes',
        savedStatus: 'Saved',
      },
      discCard: {
        title: 'Behavioral Profile',
        description: 'How the behavioral (DISC) assessment behaves for employees.',
        visibilityLabel: 'Employee sees their own behavioral assessment result',
        visibilityHint:
          'On: when they finish, the employee sees their own profile. Off: they see only a thank-you screen, and the result stays with HR.',
        on: 'Yes',
        off: 'No',
      },
    },

    appearance: {
      cardTitle: 'Appearance',
      cardDescription: 'Customize the visual interface of the system',
      navigationStyle: {
        sectionTitle: 'Navigation Style (Desktop)',
        sectionDescription: 'Choose between a sidebar or a top navigation bar. This option only affects the desktop view.',
        sidebar: 'Sidebar',
        sidebarDescription: 'Traditional left-side sidebar',
        topbar: 'Top Bar',
        topbarDescription: 'Horizontal bar at the top',
      },
      theme: {
        sectionTitle: 'System Theme',
        sectionDescription: 'Choose between light or dark theme for the interface.',
        light: 'Light Theme',
        dark: 'Dark Theme',
      },
      navTheme: {
        sectionTitle: 'Menu (sidebar and top bar)',
        sectionDescription: 'Choose the navigation menu color, independent of the system theme.',
        dark: 'Dark (recommended)',
        darkDescription: 'Menu always dark, highlighted with the brand color',
        system: 'Follow the theme',
        systemDescription: 'Menu follows the light or dark theme',
      },
    },

    shortcuts: {
      pageTitle: 'Keyboard Shortcuts',
      pageDescription: 'Use shortcuts to quickly access features',
      toggleLabel: 'Keyboard Shortcuts',
      toggleEnabled: 'Shortcuts are enabled',
      toggleDisabled: 'Shortcuts are disabled',
      groupNavigation: 'Navigation',
      groupGeneral: 'General',
      tips: {
        title: 'Tips',
        shiftNav: 'Use {modifier} + key for quick navigation',
        noTyping: 'Shortcuts do not work when you are typing in text fields',
        canDisable: 'You can disable shortcuts at any time using the toggle above',
      },
      goto_dashboard: { label: 'Dashboard', description: 'Navigate to main dashboard' },
      goto_os: { label: 'Work Orders', description: 'Navigate to work orders' },
      goto_agenda: { label: 'Schedule', description: 'Navigate to schedule' },
      goto_clients: { label: 'Customers', description: 'Navigate to customers' },
      goto_equipment: { label: 'Equipment', description: 'Navigate to equipment' },
      goto_crm: { label: 'CRM', description: 'Navigate to CRM' },
      goto_finance: { label: 'Finance', description: 'Navigate to finance' },
      goto_inventory: { label: 'Inventory', description: 'Navigate to inventory' },
      goto_quotes: { label: 'Quotes', description: 'Navigate to quotes' },
      goto_contracts: { label: 'Contracts', description: 'Navigate to contracts' },
      goto_settings: { label: 'Settings', description: 'Open settings' },
      goto_profile: { label: 'Profile', description: 'Open user profile' },
    },

    dangerZone: {
      title: 'Danger Zone',
      description: 'Irreversible actions. Review carefully before confirming.',
      resetSystem: {
        label: 'Reset System',
        description: 'Deletes selected operational data. Company data and users are preserved. Every operation is recorded in an audit log.',
        button: 'Reset System',
      },
    },

    dataExport: {
      cardTitle: 'Export my data',
      cardDescription: 'Download an Excel spreadsheet with your company data: customers, equipment, service orders, quotes, contracts, finances and more. Useful for backup or to take your data elsewhere.',
      button: 'Export data',
      dialogTitle: 'Export my data',
      dialogDescription: 'Choose what to include in the spreadsheet. The file is generated instantly and downloaded automatically.',
      selectAll: 'Select all',
      clearAll: 'Clear selection',
      confirm: 'Export',
      cancel: 'Cancel',
      running: 'Exporting...',
      progressLabel: 'Exporting {sheet}... {rows} records',
      successToast: 'Export complete! The file has been downloaded.',
      errorToast: 'Could not export your data. Please try again.',
      emptySelection: 'Select at least one item to export.',
      heavyHint: 'Stock Movements and Time Clock come unchecked by default because they tend to have a lot of records. Check them if you need those too.',
      fileNamePrefix: 'backup',
      groups: {
        customers: 'Customers',
        equipment: 'Equipment',
        serviceOrders: 'Service Orders',
        quotes: 'Quotes',
        contracts: 'Contracts',
        financial: 'Finances',
        inventory: 'Inventory',
        inventoryMovements: 'Stock Movements',
        employees: 'Employees',
        leads: 'CRM / Leads',
        timeRecords: 'Time Clock',
      },
      sheets: {
        summary: 'Summary',
        customers: 'Customers',
        customerContacts: 'Customer Contacts',
        equipment: 'Equipment',
        serviceOrders: 'Service Orders',
        serviceMaterials: 'Service Order Materials',
        quotes: 'Quotes',
        quoteItems: 'Quote Items',
        contracts: 'Contracts',
        contractItems: 'Contract Items',
        financialTransactions: 'Finances',
        financialAccounts: 'Bank Accounts',
        inventory: 'Inventory',
        inventoryMovements: 'Stock Movements',
        employees: 'Employees',
        leads: 'CRM Leads',
        timeRecords: 'Time Clock',
      },
      summary: {
        companyLabel: 'Company',
        generatedAtLabel: 'Generated at',
        sheetColumn: 'Sheet',
        rowCountColumn: 'Records',
        totalLabel: 'Total',
      },
      columns: {
        summary: {
          company: 'Company',
          generatedAt: 'Generated at',
          sheet: 'Sheet',
          rows: 'Records',
        },
        customers: {
          name: 'Name',
          nome_fantasia: 'Trade Name',
          company_name: 'Legal Name',
          customer_type: 'Customer Type',
          document: 'Tax ID',
          inscricao_estadual: 'State Registration',
          inscricao_municipal: 'Municipal Registration',
          email: 'Email',
          phone: 'Phone',
          celular: 'Mobile',
          address: 'Address',
          address_number: 'Number',
          complement: 'Complement',
          neighborhood: 'Neighborhood',
          city: 'City',
          state: 'State',
          zip_code: 'ZIP Code',
          origin: 'Source',
          birth_date: 'Birth Date',
          notes: 'Notes',
          created_at: 'Created at',
        },
        customerContacts: {
          customer: 'Customer',
          name: 'Name',
          position: 'Position',
          phone: 'Phone',
          email: 'Email',
          notes: 'Notes',
          created_at: 'Created at',
        },
        equipment: {
          customer: 'Customer',
          name: 'Name',
          category: 'Category',
          brand: 'Brand',
          model: 'Model',
          serial_number: 'Serial Number',
          identifier: 'Identifier',
          capacity: 'Capacity',
          location: 'Location',
          status: 'Status',
          install_date: 'Install Date',
          warranty_until: 'Warranty Until',
          notes: 'Notes',
          created_at: 'Created at',
        },
        serviceOrders: {
          order_number: 'Order Number',
          status: 'Status',
          os_type: 'Order Type',
          service_type: 'Service Type',
          entry_type: 'Entry Type',
          origin: 'Source',
          customer: 'Customer',
          equipment: 'Equipment',
          technician: 'Technician',
          team: 'Team',
          contract: 'Contract',
          task_title: 'Task Title',
          description: 'Description',
          diagnosis: 'Diagnosis',
          solution: 'Solution',
          notes: 'Notes',
          scheduled_date: 'Scheduled Date',
          scheduled_time: 'Scheduled Time',
          started_at: 'Started at',
          completed_at: 'Completed at',
          duration_minutes: 'Duration (minutes)',
          labor_hours: 'Labor Hours',
          labor_value: 'Labor Value',
          parts_value: 'Parts Value',
          total_value: 'Total Value',
          pmoc_conformity_status: 'PMOC Compliance',
          service_address: 'Service Address',
          service_address_number: 'Address Number',
          service_neighborhood: 'Service Neighborhood',
          service_city: 'Service City',
          service_state: 'Service State',
          service_zip_code: 'Service ZIP Code',
          created_at: 'Created at',
        },
        serviceMaterials: {
          order_number: 'Order Number',
          item_name: 'Material',
          quantity: 'Quantity',
          unit: 'Unit',
          purchase_price: 'Purchase Price',
          sale_price: 'Sale Price',
          subtotal: 'Subtotal',
          created_at: 'Created at',
        },
        quotes: {
          quote_number: 'Quote Number',
          status: 'Status',
          customer: 'Customer',
          prospect_name: 'Prospect Name',
          prospect_email: 'Prospect Email',
          prospect_phone: 'Prospect Phone',
          subtotal: 'Subtotal',
          discount_type: 'Discount Type',
          discount_value: 'Discount Value',
          discount_amount: 'Discount Applied',
          displacement_cost: 'Travel Cost',
          distance_km: 'Distance (km)',
          km_cost: 'Cost per Km',
          total_cost: 'Total Cost',
          total_price: 'Total Price',
          total_value: 'Total Value',
          bdi: 'Overhead Rate (%)',
          profit_rate: 'Profit Margin (%)',
          tax_rate: 'Tax Rate (%)',
          admin_indirect_rate: 'Admin Fee (%)',
          card_installments: 'Card Installments',
          card_discount_rate: 'Card Discount (%)',
          valid_until: 'Valid Until',
          view_count: 'View Count',
          last_viewed_at: 'Last Viewed',
          terms: 'Terms',
          notes: 'Notes',
          created_at: 'Created at',
        },
        quoteItems: {
          quote_number: 'Quote Number',
          position: 'Position',
          item_type: 'Item Type',
          description: 'Description',
          details: 'Details',
          service_type: 'Service Type',
          quantity: 'Quantity',
          unit_price: 'Unit Price',
          price_override: 'Adjusted Price',
          total_price: 'Total Price',
          unit_total_cost: 'Total Unit Cost',
          unit_labor_cost: 'Unit Labor Cost',
          unit_materials_cost: 'Unit Materials Cost',
          unit_extras_cost: 'Unit Extras Cost',
          unit_hours: 'Hours per Unit',
          unit_hourly_rate: 'Hourly Rate',
          bdi: 'Overhead Rate (%)',
          profit_rate: 'Profit Margin (%)',
        },
        contracts: {
          name: 'Name',
          customer: 'Customer',
          status: 'Status',
          is_pmoc: 'Is PMOC',
          frequency_type: 'Frequency Type',
          frequency_value: 'Frequency Value',
          start_date: 'Start Date',
          horizon_months: 'Horizon (months)',
          next_pmoc_generation_date: 'Next PMOC Generation',
          service_type: 'Service Type',
          technician: 'Technician',
          team: 'Team',
          responsible_technician: 'Responsible Technician',
          unidade_nome: 'Site Name',
          unidade_endereco: 'Site Address',
          unidade_numero: 'Site Number',
          unidade_complemento: 'Site Complement',
          unidade_bairro: 'Site Neighborhood',
          unidade_cidade: 'Site City',
          unidade_uf: 'Site State',
          unidade_cep: 'Site ZIP Code',
          pmoc_identificacao_ambiente: 'Environment ID (PMOC)',
          pmoc_tipo_atividade: 'Activity Type (PMOC)',
          pmoc_area_climatizada_m2: 'Air-Conditioned Area (m²)',
          pmoc_carga_termica_tr: 'Thermal Load (TR)',
          pmoc_ocupantes_fixos: 'Permanent Occupants',
          pmoc_ocupantes_flutuantes: 'Floating Occupants',
          notes: 'Notes',
          created_at: 'Created at',
        },
        contractItems: {
          contract: 'Contract',
          item_name: 'Item Name',
          item_description: 'Item Description',
          equipment: 'Equipment',
          pmoc_scope: 'PMOC Scope',
          pmoc_start_visit: 'PMOC Start Visit',
          sort_order: 'Order',
          created_at: 'Created at',
        },
        financialTransactions: {
          transaction_date: 'Transaction Date',
          due_date: 'Due Date',
          paid_date: 'Payment Date',
          description: 'Description',
          transaction_type: 'Type',
          category: 'Category',
          amount: 'Amount',
          amount_received: 'Amount Received',
          is_paid: 'Paid',
          payment_method: 'Payment Method',
          account: 'Account',
          customer: 'Customer',
          employee: 'Employee',
          contract: 'Contract',
          order_number: 'Order Number',
          installment_number: 'Installment',
          installment_total: 'Total Installments',
          payroll_kind: 'Payroll Type',
          payroll_period: 'Payroll Period',
          credit_card_bill_date: 'Credit Card Bill Date',
          cancelled_at: 'Cancelled at',
          cancelled_reason: 'Cancellation Reason',
          notes: 'Notes',
          created_at: 'Created at',
        },
        financialAccounts: {
          name: 'Name',
          type: 'Type',
          bank_name: 'Bank',
          institution_name: 'Institution',
          initial_balance: 'Initial Balance',
          credit_limit: 'Credit Limit',
          closing_day: 'Closing Day',
          due_day: 'Due Day',
          payment_due_days: 'Payment Term (days)',
          is_active: 'Active',
          created_at: 'Created at',
        },
        inventory: {
          name: 'Name',
          sku: 'SKU',
          category: 'Category',
          unit: 'Unit',
          stock: 'Stock Location',
          quantity: 'Quantity',
          min_quantity: 'Minimum Stock',
          cost_price: 'Cost Price',
          sale_price: 'Sale Price',
          total_value: 'Total Stock Value',
          supplier: 'Supplier',
          description: 'Description',
        },
        inventoryMovements: {
          created_at: 'Date',
          movement_type: 'Movement Type',
          item: 'Material',
          sku: 'SKU',
          stock: 'Stock Location',
          quantity: 'Quantity',
          unit_cost: 'Unit Cost',
          stock_before: 'Balance Before',
          stock_after: 'Balance After',
          supplier: 'Supplier',
          order_number: 'Order Number',
          notes: 'Notes',
        },
        employees: {
          name: 'Name',
          matricula: 'Employee ID',
          cpf: 'Tax ID',
          position: 'Position',
          cbo: 'Occupation Code',
          employment_regime: 'Employment Type',
          hire_date: 'Hire Date',
          is_active: 'Active',
          email: 'Email',
          phone: 'Phone',
          address: 'Address',
          salary: 'Salary',
          monthly_cost: 'Monthly Cost',
          dependents_count: 'Number of Dependents',
          payment_frequency: 'Payment Frequency',
          payment_day: 'Payment Day',
          payment_day_2: '2nd Payment Day',
          payment_day_type: 'Payment Day Type',
          payment_weekday: 'Payment Weekday',
          vt_enabled: 'Transit Benefit Active',
          vt_monthly_value: 'Monthly Transit Benefit',
          pix_key: 'Pix Key',
          created_at: 'Created at',
        },
        leads: {
          title: 'Title',
          status: 'Status',
          stage: 'Stage',
          customer: 'Customer',
          assigned_to: 'Assigned To',
          source: 'Source',
          value: 'Value',
          probability: 'Probability (%)',
          expected_close_date: 'Expected Close Date',
          notes: 'Notes',
          created_at: 'Created at',
          updated_at: 'Updated at',
        },
        timeRecords: {
          date: 'Date',
          recorded_at: 'Recorded at',
          type: 'Type',
          employee: 'Employee',
          source: 'Source',
          is_valid: 'Valid',
          address: 'Address',
          latitude: 'Latitude',
          longitude: 'Longitude',
          notes: 'Notes',
        },
      },
      values: {
        discountTypeValor: 'Amount',
        discountTypePercentual: 'Percentage',
        itemTypeServico: 'Service',
        itemTypeMaterial: 'Material',
        itemTypeMaoDeObra: 'Labor',
        originManual: 'Manual',
        originContract: 'Generated from Contract',
        originPortal: 'Customer Portal',
        payrollKindRescission: 'Termination',
        timeRecordSourceApp: 'App',
        timeRecordSourceAdmin: 'Admin',
        timeRecordSourceLinkPublico: 'Public Link',
      },
    },

    resetSystem: {
      modalTitle: 'Reset System',
      irreversibleWarning: 'This action will',
      deletePermanently: 'PERMANENTLY DELETE',
      warningCompany: 'the operational data of company',
      chooseWhat: 'Choose what you want to remove:',
      selectAll: 'Select all',
      selectedCount: '{count}/{total} selected',
      groupSelectAriaLabel: 'Select all in {group}',
      forcedByMaterials: 'Required because the materials catalog was selected.',
      preserved: {
        title: 'Will be kept:',
        companyData: 'Basic company data',
        users: 'Registered users',
        paymentHistory: 'Payment history',
      },
      finalWarning: 'This action CANNOT be undone!',
      confirmLabel: 'To confirm, type the company name:',
      confirmPlaceholder: 'Company name',
      nameMismatch: 'The name entered does not match the company name.',
      offline: 'This action requires an internet connection. You are offline.',
      progressTitle: 'Deleting data...',
      progressStep: 'Step {current} of {total}:',
      cancel: 'Cancel',
      confirmButton: 'Yes, Reset System',
      successTitle: 'System reset successfully',
      successDesc: 'The selected data has been removed. The audit log has been generated.',
      errorTitle: 'Could not reset the system',
      errorPermission: 'You do not have permission to reset the system. Only the company administrator can do this.',
      errorTimeout: 'A step took too long. Please try again, data already deleted remains deleted.',
      errorDependency: 'Could not complete a step due to an unexpected dependency. Please notify support with the step name.',
      errorDuplicate: 'Could not recreate a default configuration. Please try again in a few seconds.',
      errorGeneric: 'Could not reset the system. Please try again or contact support.',
      errorStepPrefix: 'Error in step "{step}".',

      groups: {
        operacional: {
          label: 'Operations',
          delete_service_orders: {
            title: 'Work Orders',
            description: 'Removes all work orders, photos, consumed materials, ratings and answered forms',
          },
          delete_equipment: {
            title: 'Equipment',
            description: 'Removes all equipment registered for customers',
          },
          delete_contracts: {
            title: 'Contracts & Maintenance Plans',
            description: 'Removes all contracts, maintenance schedules and custom maintenance documents',
          },
          delete_custom_configs: {
            title: 'Custom configurations',
            description: 'Removes custom CRM stages, work order forms and cost resources',
          },
        },
        comercial: {
          label: 'Sales (CRM)',
          delete_quotes: {
            title: 'Quotes & Proposals',
            description: 'Removes all quotes and proposals',
          },
          delete_customers: {
            title: 'Customers & Leads',
            description: 'Removes all customer records, contacts, portals and leads',
          },
        },
        financeiro: {
          label: 'Finance',
          delete_financial_movements: {
            title: 'Transactions',
            description: 'Removes all cash, bank, card, revenue and expense transactions',
          },
          delete_financial_categories: {
            title: 'Financial categories',
            description: 'Removes custom financial categories (default ones are kept)',
          },
        },
        rh_inventario: {
          label: 'HR & Inventory',
          delete_employees: {
            title: 'Employees & Teams',
            description: 'Removes all employees, advances, payments, time records and teams',
          },
          delete_materials: {
            title: 'Materials',
            description: 'Removes all registered materials (and their stock)',
          },
          delete_stock: {
            title: 'Inventory',
            description: 'Clears inventory (movements), keeps registered materials',
          },
        },
      },

      stepLabels: {
        service_orders: 'Work Orders',
        contracts: 'Contracts',
        quotes: 'Quotes',
        equipment: 'Equipment',
        custom_configs: 'Custom Configurations',
        financial_movements: 'Financial Transactions',
        financial_categories: 'Financial Categories',
        employees: 'Employees & HR',
        stock: 'Materials Inventory',
        materials: 'Materials Catalog',
        customers: 'Customers',
      },
    },

    users: {
      pageTitle: 'Users & Permissions',
      pageSubtitle: '{count}/{max} users • {active} active',
      tabs: {
        users: 'Users',
        presets: 'Roles',
      },
      searchPlaceholderUsers: 'Search users...',
      searchPlaceholderPresets: 'Search roles...',
      searchPlaceholderDesktop: 'Search by name or phone...',
      viewList: 'List',
      viewCards: 'Cards',
      btnSettings: 'Settings',
      btnCreateUser: 'Create User',
      btnHireMore: 'Add more users',
      limitTitle: 'User limit reached ({count}/{max})',
      limitDesc: 'To register new users, add extra user slots to your plan.',
      badgeYou: 'You',
      badgeActive: 'Active',
      badgeInactive: 'Inactive',
      badgeAllPerms: 'Full Access',
      badgePermCount: '{count} permissions',
      fabUser: 'User',
      fabPreset: 'Role',
      btnEdit: 'Edit',
      btnDeactivate: 'Deactivate',
      btnReactivate: 'Reactivate',
      btnReactivateDisabledTitle: 'No slot available — upgrade or deactivate another user',
      emptyFiltered: 'No users found',
      emptyAll: 'No users registered',
      deleteUserTitle: 'Permanently delete user?',
      deleteUserDesc: 'This action is irreversible. User {name} will be permanently removed from the system, including login, permissions and links.',
      deletePresetTitle: 'Delete role?',
      deletePresetDesc: 'Role {name} will be removed. Users previously linked to it keep their individual permissions. This action cannot be undone.',
      deleteLoading: 'Deleting...',
      deletePermanently: 'Delete Permanently',
      cancel: 'Cancel',
      delete: 'Delete',
      toastCreated: 'User created successfully!',
      toastCreateError: 'Error creating user',
      toastCreateErrorDuplicate: 'This email is already registered in the system.',
      toastUpdated: 'User updated!',
      toastUpdateError: 'Error',
      toastDeactivated: 'User deactivated',
      toastDeactivatedDesc: 'The slot has been freed. You can reactivate it later.',
      toastDeactivateError: 'Error deactivating',
      toastReactivated: 'User reactivated!',
      toastReactivateError: 'Error reactivating',
      toastDeleted: 'User permanently deleted!',
      toastDeleteError: 'Error deleting',
      toastPresetDuplicated: 'Role duplicated!',
      toastPresetDuplicateError: 'Error duplicating',
      toastPresetDeleted: 'Role deleted!',
      toastPresetDeleteError: 'Error deleting',
      blockLastAdminTitle: 'Action blocked',
      blockLastAdminDesc: 'The company must have at least one active administrator.',
      btnDeleteTitle: 'Permanently delete user',

      form: {
        titleCreate: 'Create User',
        titleEdit: 'Edit User',
        descCreate: 'Fill in the details for the new user',
        descEdit: 'Update the user details and permissions',
        cancel: 'Cancel',
        save: 'Update',
        create: 'Create User',
        photoLabel: 'User Photo',
        photoReplace: 'Replace',
        photoSelect: 'Select photo',
        photoRemove: 'Remove',
        photoHint: 'Optional user photo',
        labelFullName: 'Full Name *',
        placeholderFullName: 'User name',
        labelPassword: 'Password *',
        placeholderPassword: 'Create a strong password',
        labelEmail: 'Email',
        labelEmailRequired: 'Email *',
        labelEmailHint: 'Changing the email will change the user login',
        labelPhone: 'Phone',
        labelEmployee: 'Link to Employee',
        placeholderEmployee: 'No employee linked',
        employeeNone: 'None',
        employeeHint: 'Links this user to a registered employee',
        emailConflictTitle: 'Emails are different. Which one should be used?',
        emailConflictUser: 'User:',
        emailConflictEmployee: 'Employee:',
        emailConflictHint: 'The chosen email will be applied to both records',
        labelUserType: 'User Type',
        typeInternal: 'Internal',
        typeTech: 'Technician',
        descTech: 'Appears in the work order technician list and can be added to teams',
        descInternal: 'Internal system user, does not appear as a technician on work orders',
        labelProfile: 'Access Profile',
        placeholderProfile: 'Select a profile',
        profileCustom: 'Custom',
        profileAll: 'Full Access',
        sectionScreens: 'Screens',
        hintScreens: 'Select which screens this user can access',
        sectionFunctions: 'Functions',
        hintFunctions: 'Select which actions this user can perform',
        sectionPermissions: 'Permissions',
        hintPermissions: 'Pick a quick profile or turn screens on one by one. Each screen holds its own actions inside.',
      },

      // ── Permissions editor (shared by user/role dialogs) ──────────────────────
      permissionsEditor: {
        quickProfiles: 'Quick profiles',
        fullAccess: 'Full Access',
        fullAccessHint: 'Unlocks every screen and action, including future ones',
        custom: 'Custom',
        searchPlaceholder: 'Search screen or action...',
        clearSearch: 'Clear search',
        actionsTitle: 'Actions for this screen',
        screenOffHint: 'The screen is off, but these actions still apply on their own.',
        emptySearch: 'No screen or action found for "{query}".',
        generalGroup: 'General',
        ariaScreenSwitch: 'Access to the {screen} screen',
        ariaScreenActions: 'View actions for {screen}',
      },

      presets: {
        dialogTitle: 'Role Settings',
        dialogDesc: 'Create and edit access profiles with predefined permissions',
        btnCreate: 'New Role',
        btnSave: 'Save',
        btnSaveNew: 'Create Role',
        btnCancel: 'Cancel',
        btnBack: '← Back',
        empty: 'No roles registered',
        permCount: '{count} permissions',
        labelName: 'Name *',
        placeholderName: 'e.g. Field Technician',
        labelDesc: 'Description',
        placeholderDesc: 'Optional description',
        sectionScreens: 'Screens',
        sectionFunctions: 'Functions',
        emptyMobileTitle: 'No roles registered',
        emptyMobileDesc: 'Tap "New Role" to create an access profile',
        noDescription: 'No description',
        actionEdit: 'Edit',
        actionDuplicate: 'Duplicate',
        actionDelete: 'Delete',
      },

      list: {
        ariaViewPhoto: 'View photo',
        emptyFilteredDesc: 'Try a different search',
        emptyAllDesc: 'Tap "New User" to get started',
        reactivateNoSlot: 'Reactivate (no slot)',
      },
    },

    profile: {
      pageTitle: 'My Profile',
      pageSubtitle: 'Manage your personal information',
      labelFullName: 'Full Name',
      placeholderFullName: 'Your name',
      labelEmail: 'Email',
      labelPhone: 'Phone',
      placeholderPhone: '(00) 00000-0000',
      labelCreatedAt: 'Account created on',
      btnSave: 'Save Changes',
      btnSaving: 'Saving...',
      cardPasswordTitle: 'Password',
      cardPasswordDesc: 'Change your account password',
      btnChangePassword: 'Change Password',
      labelNewPassword: 'New Password',
      placeholderNewPassword: 'Create a strong password',
      labelConfirmPassword: 'Confirm New Password',
      placeholderConfirmPassword: 'Repeat the new password',
      btnSavePassword: 'Save New Password',
      btnCancel: 'Cancel',
      toastProfileUpdated: 'Profile updated successfully!',
      toastSaveError: 'Error saving',
      toastPhotoUpdated: 'Photo updated!',
      toastPhotoError: 'Error uploading photo',
      toastPasswordWeak: 'Weak password',
      toastPasswordWeakDesc: 'Use at least 8 characters with uppercase, lowercase, numbers and/or special characters.',
      toastPasswordMismatch: 'Passwords do not match',
      toastPasswordChanged: 'Password changed successfully!',
      toastPasswordError: 'Error changing password',
      btnBack: 'Back',
    },

    billing: {
      pageTitle: 'Subscription',
      pageSubtitle: 'Manage your plan, modules and payments',
      heroTrialTitle: 'Activate Your Subscription',
      heroTrialDesc: 'You are in the trial period. Choose the ideal plan and get full access.',
      heroTrialDaysLeft: '{days} day{s} remaining in trial',
      btnChoosePlan: 'Choose Plan & Activate',
      planLabel: '{name} Plan',
      statusExpired: 'Expired',
      statusExpiringSoon: 'Expiring soon',
      statusActive: 'Active',
      statusMsgExpired: 'Expired {days} day{s} ago',
      statusMsgExpiringSoon: 'Expires in {days} day{s}',
      statusMsgActive: '{days} days remaining',
      statusMsgActiveNoDate: 'Active',
      labelMonthlyValue: 'Monthly value',
      labelExpiration: 'Expiration',
      sectionActiveModules: 'Active Modules',
      sectionAvailableModules: 'Available Modules',
      btnHireModule: 'Add',
      cardPaymentTitle: 'Payment',
      cardPaymentDescExpired: 'Your subscription has expired. Renew now.',
      cardPaymentDescActive: 'Keep your subscription up to date.',
      labelAmountDue: 'Amount due',
      labelDueDate: 'Due date',
      btnPayNow: 'Pay Now',
      paymentYearlyDiscount: '20% off on the annual plan · valid for Pix or Bank slip',
      bannerPromoTitle: 'Promotional price active',
      bannerPromoDesc: 'After the promotional period, the value will return to {value}.',
      bannerPendingTitle: 'Value change scheduled',
      bannerPendingDesc: 'Current value: {current}. From the next charge: {new}.',
      bannerRecurringTitle: 'Automatic renewal active',
      bannerRecurringDesc: 'Your subscription is renewed automatically on the registered card.',
      bannerNfseTitle: 'Missing data for issuing invoices',
      bannerNfseDesc: 'Fill in your company settings: {fields}',
      cardSummaryTitle: 'Plan Summary',
      summaryPlan: 'Plan',
      summaryActiveModules: 'Active modules',
      summaryMaxUsers: 'Max. Users',
      summaryCycle: 'Cycle',
      summaryStatus: 'Status',
      cycleYearly: 'Annual',
      cycleMonthly: 'Monthly',
      cardPaymentHistoryTitle: 'Payment history',
      cardPaymentHistoryDesc: 'Charges and payments for your subscription.',
      btnCancelSubscription: 'Cancel subscription',
      cancelSubscriptionNote: 'When you cancel, automatic renewal stops and you keep access until the expiration date.',
      btnManagePlan: 'Manage My Plan',
      nfseTierSectionTitle: 'Invoice Level (NFS-e)',
      nfseTierCurrent: 'current',
      nfseTierUpgradeNote: 'The new level is activated immediately and the new price takes effect on the next charge.',
      // Plan Summary live panel (ModulesManagementCard)
      planSummaryTitle: 'Your Plan',
      planSummaryIncluded: 'Included',
      planSummaryEmptyCustom: 'Choose modules on the left to build your plan.',
      planSummaryEmptyPlans: 'Select a plan on the left.',
      planSummaryUsersOne: 'user',
      planSummaryUsersPlural: 'users',
      planSummaryUsersIncluded: 'included',
      planSummaryCycleLabel: 'Cycle',
      planSummaryCycleYearlyDesc: '20% off via Pix/Bank slip',
      planSummaryCycleYearlyInfo: 'The 20% discount applies to upfront payments (Pix or Bank slip) only. Card billing remains monthly.',
      planSummaryYearlySavings: 'You save {value}/year',
      planSummaryTotalMonthly: 'Monthly total',
      planSummaryTotalYearly: 'Annual total',
      planSummaryDiffLabel: 'vs current plan',
      planSummaryCtaMonthly: 'Apply plan',
      planSummaryCtaYearly: 'Pay and activate (annual)',
      planSummaryCtaApplying: 'Applying...',
      planSummaryUpgradeNote: 'Upgrade: features are unlocked immediately and the new price takes effect on the next charge.',
      planSummaryDowngradeNote: 'Scheduled downgrade: you keep the current plan until the end of the paid period. The new value of {value}/month takes effect on the next charge.',
      noCompanyTitle: 'No company linked',
      noCompanyDesc: 'Your account is not yet linked to a company.',

      cancelSubscription: {
        titleReason: 'Cancel subscription',
        titleConfirm: 'Confirm cancellation',
        titleDone: 'Cancellation complete',
        headerSubtitle: 'Help us understand why',
        labelReason: 'Cancellation reason',
        placeholderReason: 'Select a reason',
        labelDetailsRequired: 'Explain your reason *',
        labelDetailsOptional: 'Would you like to tell us more? (optional)',
        placeholderDetailsRequired: 'Describe the cancellation reason...',
        placeholderDetailsOther: 'Any suggestions or comments...',
        retentionTalkSupport: 'Talk to support',
        confirmOnConfirm: 'By confirming:',
        confirmBullet1: 'Automatic renewal will be cancelled',
        confirmBullet2Until: 'You keep access until',
        confirmBullet2NoDate: 'the end of the already-paid period',
        confirmBullet3: 'Any pending future charges will be automatically cancelled',
        confirmBullet4: 'You can reactivate at any time',
        confirmReasonPrefix: 'Reason:',
        confirmDetailsPrefix: 'Details:',
        doneTitle: 'Subscription cancelled',
        doneDesc: 'Automatic renewal has been cancelled. You keep access',
        doneDescUntil: 'until {date}',
        doneDescNoDate: 'until the end of the already-paid period',
        doneDescSuffix: '. If you change your mind, just reactivate anytime.',
        btnBack: 'Back',
        btnContinue: 'Continue',
        btnCanceling: 'Cancelling...',
        btnConfirm: 'Confirm cancellation',
        btnUnderstood: 'Got it',
        toastError: 'Could not cancel right now. Please try again or contact support.',
        toastSelectReason: 'Select a reason',
        reasons: {
          preco_alto: 'Price is too high',
          nao_uso: "I'm not using it enough",
          funcionalidades: 'Missing features I need',
          dificuldade_uso: 'Difficulty using the system',
          concorrente: 'Found another solution',
          fechando_empresa: "I'm closing my company",
          temporario: 'Temporary pause',
          outro: 'Other reason',
        },
        retention: {
          preco_alto: {
            title: 'How about a discount?',
            message: 'Chat with us on WhatsApp, we can negotiate a special price so you can stay on Dominex.',
            waText: "Hi! I'm thinking about cancelling my Dominex subscription due to the price. Is there any special deal?",
          },
          nao_uso: {
            title: 'We can help!',
            message: 'Our team offers a free personalised training session so you can get the most out of the system.',
            waText: "Hi! I'd like to schedule a Dominex training session before deciding about my subscription.",
          },
          funcionalidades: {
            title: 'Your opinion matters!',
            message: 'Tell us which features you need. We are always evolving and your suggestion could be added soon!',
            waText: "Hi! I'm missing some features in Dominex and would like to suggest them before cancelling.",
          },
          dificuldade_uso: {
            title: 'Dedicated support',
            message: 'We can schedule a dedicated support session to answer your questions and make things easier.',
            waText: "Hi! I'm having difficulty using Dominex and would like a support session before cancelling.",
          },
        },
        // Downgrade offer
        downgradeOfferTitle: 'How about a cheaper plan?',
        downgradeOfferDesc: 'Instead of cancelling, switch to the {plan} plan and save {savings} per month.',
        downgradeOfferCta: 'Switch to {plan}',
        downgradeOfferApplying: 'Applying...',
        downgradeOfferToastSuccess: 'Plan changed to {plan} successfully.',
        downgradeOfferToastError: 'Could not change the plan right now. Please try again.',
      },

      paymentHistory: {
        empty: 'No payments recorded yet.',
        perPage: 'Per page',
        pageSummary: 'Page {current} of {total}',
        prevPage: 'Previous page',
        nextPage: 'Next page',
        viewCharge: 'View invoice',
        statusPaid: 'Paid',
        statusPending: 'Pending',
        statusOverdue: 'Overdue',
        statusCancelled: 'Cancelled',
        typeSale: 'Sale',
        typeRenewal: 'Renewal',
        methodPix: 'PIX',
        methodBoleto: 'Bank slip',
        methodCard: 'Card',
        methodDefault: 'Charge',
      },

      accountUsage: {
        title: 'Account usage',
        description: 'Track your plan consumption',
        users: 'Users',
        nfseThisMonth: 'Invoices this month',
        usersIncluded: '{base} included + {extra} add-on',
        usersIncludedPlural: '{base} included + {extra} add-ons',
        usersPlanLimit: '{max} user in current plan',
        usersPlanLimitPlural: '{max} users in current plan',
        nfseUnlimitedLabel: '{used} issued',
        nfseLevelLimit: 'Level {level} limit — {limit} invoices/mo',
        nfsePlanLimit: 'Plan limit — {limit} invoices/mo',
        paymentMethods: 'Payment methods',
      },
    },

    changelog: {
      pageTitle: 'What\'s new',
      searchPlaceholder: 'Search updates...',
      currentVersionBadge: 'Current version',
      filterAll: 'All',
      filterRecurso: 'Features',
      filterMelhoria: 'Improvements',
      filterCorrecao: 'Bug fixes',
      filterSeguranca: 'Security',
      categoryRecurso: 'New Feature',
      categoryMelhoria: 'Improvement',
      categoryCorrecao: 'Bug fix',
      categorySeguranca: 'Security',
      emptyTitle: 'No updates found',
      emptyDesc: 'Try a different search or change your filters.',
      changesCount_one: '{count} change',
      changesCount_other: '{count} changes',
      showMore: 'Show more',
      showLess: 'Show less',
    },

    terms: {
      modalTitle: 'Terms of Use — Dominex',
      pdfTitle: 'TERMS OF USE — DOMINEX',
      pdfFilename: 'TERMS OF USE - DOMINEX',
      checkboxLabel: 'I have read and agree to the Dominex Terms of Use',
      btnAccept: 'Accept Terms',
      btnAccepting: 'Confirming...',
      btnDownloadPdf: 'Download PDF',
      btnGeneratingPdf: 'Generating PDF...',
      btnClose: 'Close',
      acceptedAt: 'Accepted on {date}',
      pdfFooterPage: 'Page {current} of {total}',
      pdfGeneratedOn: 'Document generated on {date}',
      errorPdf: 'Error generating the PDF. Please try again.',
    },

    myData: {
      backToDashboard: '← Back to dashboard',
      pageTitle: 'Privacy Center',
      pageSubtitle: 'Manage your personal data (personal data protection rights)',
      cardPersonalTitle: 'Your personal data',
      labelName: 'Name',
      labelEmail: 'E-mail',
      labelPhone: 'Phone',
      labelCreatedAt: 'Account created on',
      cardExportTitle: 'Export my data',
      cardExportDesc: 'Download a copy of your data in JSON format (data portability right).',
      btnExport: 'Download my data',
      btnExporting: 'Exporting...',
      cardDeleteTitle: 'Request account deletion',
      cardDeleteDesc: 'Request permanent deletion of your account and personal data. Financial and accounting data may be retained for the legal period (5 years). Processing takes up to 15 business days.',
      deletionRequested: 'Request registered. Our team will be in touch.',
      btnRequestDeletion: 'Request deletion',
      btnRequesting: 'Registering...',
      cardDpoTitle: 'Contact the DPO',
      cardDpoDesc: 'To exercise other rights (correction, portability, consent withdrawal) or for questions about how your data is processed:',
      linkPrivacy: 'Privacy Policy',
      linkTerms: 'Terms of Use',
      toastExported: 'Data exported successfully!',
      toastExportError: 'Error exporting data',
      toastDeletionRequested: 'Deletion request submitted',
      toastDeletionRequestedDesc: 'Our team will process your request within 15 business days as required by personal data protection law.',
      toastDeletionError: 'Error registering request',
    },

    integrations: {
      subTabs: {
        whatsapp: 'WhatsApp',
        asaas: 'Payments',
        simulador: 'Simulator',
      },
      simulator: {
        title: 'Sales simulator',
        description: 'See how much is left from a sale after the Asaas fee and when the money lands in your account.',
        notConnectedTitle: 'Your Asaas account is not connected yet',
        notConnectedDesc: 'The numbers below use the Asaas reference table. Connect your account on the Payments tab to simulate with your plan\'s real fees.',
        notContractedTitle: 'Payments module not included',
        notContractedDesc: 'The simulator shows how much is left from each sale via Pix, boleto or card. Talk to our team to enable online charges.',
        amountLabel: 'Sale amount',
        methodLabel: 'Payment method',
        methodPix: 'Pix',
        methodBoleto: 'Boleto',
        methodCard: 'Card',
        installmentsLabel: 'Installments',
        installmentsOption: '{n}x',
        feePayerLabel: 'Who pays the fee',
        feePayerCompany: 'The company absorbs it',
        feePayerCustomer: 'Pass it on to the customer',
        anticipateLabel: 'Receive early',
        anticipateDesc: 'Get everything in 1 business day by paying the Asaas early settlement fee.',
        anticipateUnavailable: 'Asaas did not report your account\'s early settlement fee. We used the reference rate.',
        resultTitle: 'Simulation result',
        resultGross: 'Gross amount',
        resultCustomerPays: 'The customer pays',
        resultFee: 'Asaas fee',
        resultNet: 'Net amount in your account',
        resultInstallment: 'Each installment for the customer',
        resultAnticipationCost: 'Early settlement cost',
        resultNetAnticipated: 'Net after early settlement',
        resultEffectiveCost: 'Total cost of the sale',
        compareTitle: 'With and without early settlement',
        compareWithout: 'Without it',
        compareWith: 'With it',
        compareWhen: 'lands',
        scheduleTitle: 'When the money lands in your account',
        scheduleInstallment: 'Installment {n}',
        scheduleSingle: 'Payment',
        scheduleTotal: 'Total',
        settlementSameDay: 'instantly',
        settlementInDay: 'in 1 business day ({date})',
        settlementInDays: 'in {count} days ({date})',
        settlementBusinessDayNote: 'Weekends and holidays don\'t count: if the date falls on a Saturday, Sunday or holiday, the money lands on the next business day.',
        settlementFallbackNote: 'Reference term, Asaas does not report this one through the API.',
        referenceWarning: 'Some values come from the Asaas reference table, not from your account.',
        pixFreeNote: 'The first {count} Pix payments received each month are free, so in those cases the net is the full amount.',
        disclaimer: 'Estimate. The final amount is calculated by Asaas when the charge is created.',
        emptyAmount: 'Enter the sale amount to see the simulation.',
      },
      asaas: {
        loading: 'Loading…',
        notContracted: {
          title: 'Payments not included',
          description: 'This feature is not part of your current plan. Contact our team to charge your customers online via Pix, bank slip and card.',
        },
        card: {
          title: 'Payments (Asaas)',
          description: 'Charge your customers via Pix, bank slip and card using your own Asaas account.',
        },
        status: {
          active: 'Active',
          pending: 'Under review',
          disabled: 'Disabled',
          rejected: 'Rejected',
        },
        inactive: {
          intro: 'Paste your Asaas API key to activate payments. You generate this key in the Asaas panel, under Settings, Integrations.',
          apiKeyLabel: 'Asaas API key',
          apiKeyPlaceholder: 'Paste your API key here',
          activate: 'Activate payments',
          activating: 'Activating…',
          helpLink: 'Where do I find my key?',
          apiKeyGuide: {
            guideTitle: 'How to get your Asaas API key',
            trigger: 'How do I get my Asaas API key?',
            step1: 'Go to asaas.com and log in (or create your free account).',
            step2: 'In the menu, go to "Account settings" (gear icon), then "Integrations".',
            step3: 'In the "API key" section, click "Generate API key" or copy your existing key.',
            step4: 'Copy the key (starts with "$aact_") and paste it in the field above. The key is secret, keep it safe.',
            sandboxHint: 'To test without real money, use the sandbox account at sandbox.asaas.com following the same steps.',
          },
        },
        activeState: {
          connected: 'Payments active. You can now charge your customers.',
          pendingDesc: 'Your account is under review at Asaas. Once approved, payments become active.',
          feesTitle: 'Asaas fees',
          feesNote: 'Fees are charged by Asaas directly on your account. Dominex keeps nothing per transaction.',
          feePix: 'Pix',
          feePixValue: 'R$ 0.99 to R$ 1.99 per payment received',
          feeBoleto: 'Bank slip',
          feeBoletoValue: 'R$ 0.99 to R$ 1.99',
          feeCard: 'Credit card',
          feeCardValue: '1.99% to 4.29% + R$ 0.49 per charge',
          feeAnticipation: 'Early settlement',
          feeSettlement: 'Time until the money lands',
          feeNotInformed: 'Not reported by Asaas',
          feeReferenceTag: 'reference',
          feeAnticipationValue: '1.25% to 1.70% per month (card), 5.79% per month (boleto and Pix)',
          feeSettlementValue: 'Pix instantly, boleto in 1 day, card in 32 days',
          feesFromAccount: 'Fees from your Asaas account, updated on',
          feesFromAccountNoDate: 'Fees read from your Asaas account.',
          feesFromOverride: 'Fees manually adjusted for your company.',
          feesFromReference: 'Reference fees, we could not read the ones from your account right now.',
          feesRefresh: 'Refresh fees',
          feesRefreshing: 'Refreshing…',
          feesRefreshOk: 'Fees updated from Asaas.',
          feesRefreshError: 'We could not read the fees from your Asaas account right now.',
          feePixFreeNote: 'The first {count} payments received each month are free.',
          feePerCharge: 'per charge',
          feeOneShot: 'single payment',
          feeInstallments: 'installments',
          feeMonthly: 'per month',
          settlementSameDay: 'instantly',
          settlementInDay: 'in 1 day',
          settlementInDays: 'in {count} days',
          financeAccountRequiredError: 'Pick the account that will receive the money, without it the revenue will not reach your Finance module.',
          financeAccountPlaceholder: 'Pick the account…',
          financeAccountPendingTitle: 'You still need to pick the receiving account',
          financeAccountPendingDesc: 'Until you pick it, paid charges will not post automatically to your Finance module.',
          financeAccountPendingCta: 'Pick it now',
          financeAccountEmptyTitle: 'You have no bank account registered yet',
          financeAccountEmptyDesc: 'Create the account under Finance, Transactions tab, to choose where the revenue lands.',
          financeAccountEmptyCta: 'Open Finance',
          dueDaysError: 'Enter in how many days the charge is due (0 or more).',
          maxInstallmentsError: 'Enter the maximum number of installments (1 or more).',
          discountPercentError: 'The discount must be a number between 0 and 100.',
          discountDaysError: 'Enter how many days before the due date the discount applies (0 or more).',
          deactivate: 'Deactivate payments',
          deactivating: 'Deactivating…',
          autoPostLabel: 'Automatically post charges to Finance',
          autoPostDesc: 'Each charge creates a receivable that is automatically cleared when the customer pays.',
          autoPostToastOn: 'Automatic posting enabled.',
          autoPostToastOff: 'Automatic posting disabled.',
          autoPostToastError: 'Could not save the setting. Please try again.',
          autoFeeLabel: 'Post Asaas fees as an expense',
          autoFeeDesc: 'When a payment is received, the Asaas fee is recorded as an expense in your Finance so your cash flow stays accurate.',
          autoFeeToastOn: 'Fee posting enabled.',
          autoFeeToastOff: 'Fee posting disabled.',
          autoFeeError: 'Could not save the setting. Please try again.',
          lateFeesTitle: 'Late payment fees',
          lateFeesDesc: 'Applied to upcoming charges when the customer pays late.',
          fineLabel: 'Late fee (%)',
          interestLabel: 'Monthly interest (%)',
          interestCapWarning: 'Asaas maximum is 10% per month.',
          lateFeesSave: 'Save',
          lateFeesSaving: 'Saving…',
          lateFeesToastOk: 'Late fees updated.',
          lateFeesToastError: 'Could not save. Please try again.',
          // ── Charge preferences ────────────────────────────────────────────
          save: 'Save',
          saving: 'Saving…',
          chargePrefsTitle: 'Charge preferences',
          chargePrefsDesc: 'Default settings applied when creating new charges.',
          dueDaysLabel: 'Default due in',
          dueDaysSuffix: 'days',
          defaultDescriptionLabel: 'Default description / instructions',
          defaultDescriptionPlaceholder: 'e.g. Regarding maintenance service…',
          paymentMethodsLabel: 'Enabled payment methods',
          methodPix: 'Pix',
          methodBoleto: 'Bank slip',
          methodCard: 'Credit card',
          maxInstallmentsLabel: 'Max installments (card)',
          maxInstallmentsSuffix: 'x',
          chargePrefsToastOk: 'Charge preferences saved.',
          // ── Early payment discount ────────────────────────────────────────
          discountTitle: 'Early payment discount',
          discountDesc: 'Offer a discount to customers who pay before the due date.',
          discountPercentLabel: 'Discount (%)',
          discountPercentPlaceholder: 'e.g. 5',
          discountDaysLabel: 'Up to X days before',
          discountDaysPlaceholder: 'e.g. 10',
          discountDaysSuffix: 'days',
          discountHint: 'Leave blank to disable discount.',
          discountToastOk: 'Early payment discount saved.',
          // ── Finance posting ───────────────────────────────────────────────
          financeConfigTitle: 'Finance posting',
          financeConfigDesc: 'Set the account and categories used when automatically clearing paid charges.',
          financeAccountLabel: 'Target bank account',
          financeAccountNone: 'None',
          incomeCategoryLabel: 'Revenue category',
          incomeCategoryNone: 'None',
          feeCategoryLabel: 'Fee category (Asaas)',
          feeCategoryHint: 'Category used to record the Asaas fee as an expense.',
          financeConfigToastOk: 'Finance posting settings saved.',
        },
        deactivateConfirm: {
          title: 'Deactivate payments?',
          description: 'Existing charges remain valid, but you will not be able to create new ones until you reactivate.',
          confirm: 'Yes, deactivate',
          cancel: 'Cancel',
        },
        errors: {
          activateGeneric: 'Could not activate payments. Check the key and try again.',
          deactivateGeneric: 'Could not deactivate payments.',
        },
        toasts: {
          activated: 'Payments activated!',
          deactivated: 'Payments deactivated.',
        },
      },
      whatsapp: {
        loading: 'Loading…',
        comingSoon: {
          badge: 'Coming soon',
          title: 'WhatsApp Notifications',
          description: 'Soon you will be able to notify your customers automatically via WhatsApp. We are finishing the last adjustments.',
        },
        notContracted: {
          title: 'WhatsApp notifications not included',
          description: 'This feature is not part of your current plan. Contact our team to enable automatic WhatsApp notifications to your customers.',
        },
        masterToggle: {
          title: 'WhatsApp Notifications',
          description: 'Enable or disable all WhatsApp notifications sent to customers',
          ariaLabel: 'Enable WhatsApp notifications',
        },
        connection: {
          sectionTitle: 'Connection',
          sectionDescription: 'Connect your company WhatsApp to send notifications to customers',
          statusConnected: 'Connected',
          statusQr: 'Awaiting QR',
          statusDisconnected: 'Disconnected',
          statusBanned: 'Banned',
          btnConnect: 'Connect WhatsApp',
          connecting: 'Connecting…',
          qrInstructions: 'Open WhatsApp on your phone, go to Menu > Linked Devices > Link a Device and scan the QR code below.',
          connectedDesc: 'WhatsApp connected and ready to send notifications.',
          bannedDesc: 'This number has been banned by WhatsApp. Please contact support.',
          errorTitle: 'Could not connect',
          errorHint: 'Check your connection or try again shortly. The integration server may still be setting up.',
        },
        triggers: {
          sectionTitle: 'Notification Triggers',
          sectionDescription: 'Choose when the customer receives an automatic notification during a work order',
          aCaminho: {
            label: 'Technician on the way',
            description: 'Notifies the customer when the technician heads out to the job',
          },
          emAndamento: {
            label: 'Service started',
            description: 'Notifies the customer when the technician begins work',
          },
          concluida: {
            label: 'Work order completed',
            description: 'Notifies the customer when the work order is completed',
          },
        },
        quota: {
          sectionTitle: 'Plan & Quota',
          sectionDescription: 'Your current tier and monthly notification usage',
          currentPlan: 'Current plan',
          usageLabel: 'Notifications sent this month',
          unlimited: 'Unlimited',
          nearLimitWarning: 'You are close to your monthly limit.',
          resetHint: 'Your quota resets automatically at the start of each month.',
        },
      },
    },

    help: {
      title: 'Help Center',
      intro: 'Frequently asked questions about the system. If you cannot find your answer, contact Support via WhatsApp.',
      faqs: [
        {
          q: 'How do I create a Work Order?',
          a: 'Go to "Work Orders" in the sidebar, click "+ New Work Order", fill in the customer, service type and assigned technician, then save.',
        },
        {
          q: 'How do I register a new customer?',
          a: 'Go to "Customers" in the menu, click "New Customer" and fill in the details. You can register an individual (CPF) or a company (CNPJ).',
        },
        {
          q: 'How does financial management work?',
          a: 'In the "Finance" menu you have an overview, transactions, payables/receivables and income statement. Work orders can automatically generate financial entries.',
        },
        {
          q: 'How do employee payments work?',
          a: 'In "Employees", click the payment icon on the employee card. The system automatically calculates salary + bonuses - advances - absences and lets you select the debit account.',
        },
        {
          q: 'How do I set up time tracking?',
          a: 'Go to Employees > Time Tracking. Set default hours, late tolerance and selfie/geolocation requirements in settings.',
        },
        {
          q: 'How do I create recurring contracts?',
          a: 'In "Contracts", create a new contract with the customer, frequency (monthly, bimonthly, etc.) and period. The system automatically generates scheduled occurrences.',
        },
        {
          q: 'What is CRM and how do I use it?',
          a: 'CRM is your sales funnel. Create leads, move them between stages (New, Contact, Proposal, Won/Lost) and track your pipeline value in real time.',
        },
        {
          q: 'How does the Inventory module work?',
          a: 'In "Inventory", register items with cost/sale price, minimum quantity and unit. The system alerts you when stock falls below the minimum.',
        },
        {
          q: 'How do I export reports?',
          a: 'Several modules have export buttons (PDF, HTML). Employee statements, quotes and income statements have export and print options.',
        },
        {
          q: 'How do I change my password?',
          a: 'Go to "Profile" from the user menu (bottom of the sidebar) and click "Change Password". You will receive an email to reset it.',
        },
      ],
    },
  },

  // ═══════════════════════════════════════════════════════════════════════════
  // ESPAÑOL
  // ═══════════════════════════════════════════════════════════════════════════
  es: {
    page: {
      title: 'Configuración',
      subtitle: 'Administra la configuración del sistema',
      tabs: {
        empresa: 'Empresa',
        regional: 'Regional',
        usuarios: 'Usuarios y Permisos',
        usabilidade: 'Usabilidad',
        atalhos: 'Atajos',
        aparencia: 'Apariencia',
        integracoes: 'Integraciones',
      },
    },

    saveStatus: {
      saving: 'Guardando…',
      unsaved: 'Cambios sin guardar',
      saved: 'Guardado',
    },

    company: {
      cardTitle: 'Datos de la Empresa',
      cardDescription: 'Información de la empresa que aparece en etiquetas y documentos',

      visualIdentity: {
        sectionTitle: 'Identidad Visual',
        sectionDescription: 'Logo de la empresa para documentos y el sistema',
        replace: 'Reemplazar',
        remove: 'Eliminar',
        removeLogoTitle: '¿Eliminar logo?',
        removeLogoDesc: 'El logo actual se eliminará permanentemente.',
        uploadPrompt: 'Haz clic para subir el logo',
        uploadHint: 'PNG, JPG hasta 5MB',
        errorFileSize: 'Archivo muy grande (máx 5MB)',
        errorFileType: 'Solo se permiten imágenes',
        errorUpload: 'Error al subir el logo',
        errorIconUpload: 'Error al subir el ícono',
      },

      registrationData: {
        sectionTitle: 'Datos de Registro',
        sectionDescription: 'Razón social, documento e información oficial',
        companyName: 'Nombre de la Empresa',
        companyNamePlaceholder: 'Nombre de tu empresa',
        cnpj: 'NIT/RUC',
        cnpjPlaceholder: '00.000.000/0000-00',
        showInDocs: 'Mostrar en documentos',
        segment: 'Segmento de Negocio',
        segmentNotDefined: 'No definido',
        segmentHint: 'Define qué herramientas y funciones aparecen para tu segmento. Para cambiarlo, contacta a Dominex.',
      },

      contact: {
        sectionTitle: 'Contacto',
        sectionDescription: 'Teléfono y correo de la empresa',
        phone: 'Teléfono',
        phonePlaceholder: '(00) 0000-0000',
        email: 'Correo',
        emailPlaceholder: 'contacto@empresa.com',
        showInDocs: 'Mostrar en documentos',
      },

      address: {
        sectionTitle: 'Dirección',
        sectionDescription: 'Ubicación física de la empresa',
        showInDocs: 'Mostrar en documentos',
        zip: 'Código Postal',
        street: 'Dirección',
        streetPlaceholder: 'Calle, Avenida…',
        number: 'Número',
        numberPlaceholder: 'Nº',
        complement: 'Complemento',
        complementPlaceholder: 'Piso, Oficina…',
        neighborhood: 'Colonia/Barrio',
        neighborhoodPlaceholder: 'Colonia/Barrio',
        stateCity: 'Estado / Ciudad',
      },

      whiteLabelSection: {
        sectionTitle: 'White Label',
        sectionDescription: 'Personaliza el sistema con la identidad de tu marca',
        moduleUnavailable: 'Módulo no disponible en tu plan actual',
        hire: 'Contratar',
        enableLabel: 'Activar White Label',
        enableDescription: 'Reemplaza el logo y el color predeterminado del sistema',
        fullLogo: 'Logo completo',
        fullLogoConfigured: 'Logo personalizado configurado',
        fullLogoUsingCompany: 'Usando el logo de la empresa por defecto',
        fullLogoFallback: 'Se usará el logo de la empresa',
        uploadLogoPrompt: 'Subir logo (opcional)',
        icon: 'Ícono (1:1)',
        iconDescription: 'Ícono cuadrado para el menú lateral plegado. 128×128px.',
        uploadIconPrompt: 'Subir ícono',
        primaryColor: 'Color primario',
        primaryColorDescription: 'Reemplaza el color verde predeterminado del sistema',
        qrSection: {
          title: 'Código QR',
          description: 'Personaliza el aspecto de los códigos QR generados por el sistema, con el logo de tu marca en el centro',
          dotStyleLabel: 'Estilo de los puntos',
          dotStyle: {
            square: 'Cuadrado',
            rounded: 'Redondeado',
            dots: 'Puntos',
            classy: 'Elegante',
          },
          cornerStyleLabel: 'Esquinas',
          cornerStyle: {
            square: 'Cuadrado',
            rounded: 'Redondeado',
            dot: 'Punto',
          },
          colorLabel: 'Color del código QR',
          previewCaption: 'Vista previa',
        },
        reportHeader: 'Encabezado del Informe de Servicio',
        reportHeaderDescription: 'Personaliza el encabezado que aparece en el informe de la OT finalizada',
        reportBgColor: 'Color de fondo del encabezado',
        reportTextColor: 'Color del texto',
        reportStatusBarColor: 'Color de la barra de estado',
        reportLogoSize: 'Tamaño del logo ({size}px)',
        reportLogoBg: 'Fondo detrás del logo',
        reportLogoBgDescription: 'Elimina el fondo cuando está desactivado',
        reportLogoBgColor: 'Color del fondo',
        reportLogoType: 'Tipo de logo en el informe',
        reportLogoTypeFull: 'Logo Completo',
        reportLogoTypeIcon: 'Ícono',
        reportLogoTypeHintFull: 'Usa el logo completo de la empresa',
        reportLogoTypeHintIcon: 'Usa el ícono configurado en White Label',
      },

      legalDocs: {
        sectionTitle: 'Documentos Legales',
        sectionDescription: 'Consulta y descarga los términos que rigen el uso de Dominex',
        viewTerms: 'Ver términos de uso',
        acceptedAt: 'Aceptado el {date}',
      },
    },

    usability: {
      cardTitle: 'Usabilidad',
      cardDescription: 'Preferencias de comportamiento del sistema',
      preferenceSaved: '¡Preferencia guardada!',

      sections: {
        os: {
          title: 'Órdenes de Trabajo',
          description: 'Comportamientos relacionados con las órdenes de trabajo',
          autoSaveOS: {
            title: 'Guardado automático',
            description: 'Guardar automáticamente los borradores de órdenes de trabajo al editar',
          },
          showOSValues: {
            title: 'Mostrar valores',
            description: 'Mostrar valores financieros (mano de obra, piezas) en las órdenes de trabajo',
          },
          requireSignature: {
            title: 'Requerir firma',
            description: 'Hacer obligatoria la firma del cliente al cerrar una OT',
          },
          saveOSPhotosToDevice: {
            title: 'Guardar fotos en el dispositivo',
            description: 'Muestra un botón para guardar la foto en tu dispositivo. En iPhone abre "Guardar imagen"; en Android descarga directamente.',
          },
          osStockConsumptionEnabled: {
            title: 'Consumo de stock en la OT',
            description: 'Permite que el técnico registre los materiales usados durante la orden de trabajo y los descuente del stock al finalizarla',
          },
        },
        interface: {
          title: 'Interfaz',
          description: 'Preferencias visuales de listas y tablas',
          compactTables: {
            title: 'Tablas compactas',
            description: 'Reducir el espaciado de las tablas para mostrar más datos por página',
          },
          showEquipmentPhotos: {
            title: 'Fotos de equipos',
            description: 'Mostrar miniaturas de fotos de equipos en las listas',
          },
        },
        security: {
          title: 'Seguridad',
          description: 'Confirmaciones y validaciones de seguridad',
          confirmDelete: {
            title: 'Confirmar eliminaciones',
            description: 'Mostrar un diálogo de confirmación antes de eliminar registros',
          },
        },
        schedule: {
          title: 'Agenda',
          description: 'Configuración de agenda y calendario',
          showHolidays: {
            title: 'Mostrar festivos',
            description: 'Mostrar festivos nacionales y locales en la agenda',
          },
        },
      },

      origins: {
        cardTitle: 'Orígenes',
        cardDescription: 'Lista de orígenes usada en el registro de clientes y las oportunidades del CRM',
        manage: 'Gestionar orígenes',
        empty: 'No hay orígenes registrados. Crea un conjunto inicial y edítalo libremente después.',
        seedButton: 'Crear orígenes predeterminados',
        seedLoading: 'Creando…',
        inactive: 'Inactivo',
      },
    },

    regional: {
      personalCard: {
        title: 'Mi idioma',
        description: 'El idioma que TÚ ves en el sistema. Se aplica al instante, solo para ti.',
        languageLabel: 'Idioma',
        languageHint: 'Preferencia personal. Reemplaza el idioma predeterminado de la empresa solo para ti.',
      },
      companyCard: {
        title: 'Valores predeterminados de la empresa',
        description: 'Idioma, moneda y zona horaria predeterminados para toda la empresa',
        adminOnly: '(solo admin)',
        languageLabel: 'Idioma predeterminado de la empresa',
        languageHint: 'Idioma aplicado a todos los usuarios que no han elegido uno personalmente.',
        localeDefaultsPrompt: '¿Aplicar los valores predeterminados de {locale} para moneda y zona horaria?',
        keepCurrent: 'Mantener actuales',
        useDefaults: 'Usar predeterminados',
        localeDefaultsApplied: 'Valores de {locale} aplicados',
        currencyLabel: 'Moneda',
        currencyHint: 'Moneda de operación usada en los valores del sistema.',
        currencyChangedWarning: 'La moneda es la moneda operativa de la empresa. Cambiarla no convierte los valores ya registrados, se mantienen con su número original.',
        timezoneLabel: 'Zona horaria',
        timezoneHint: 'Usada para fechas y horas mostradas en el sistema.',
        timezonePlaceholder: 'Seleccionar zona horaria',
        timezoneSearchPlaceholder: 'Buscar zona horaria (ej: Mexico_City)',
        timezoneEmpty: 'No se encontró ninguna zona horaria.',
        savingStatus: 'Guardando...',
        unsavedStatus: 'Cambios sin guardar',
        savedStatus: 'Guardado',
      },
      discCard: {
        title: 'Perfil de Comportamiento',
        description: 'Cómo se comporta la evaluación de comportamiento (DISC) para los empleados.',
        visibilityLabel: 'El empleado ve su propio resultado de la evaluación de comportamiento',
        visibilityHint:
          'Activado: al terminar, el empleado ve su propio perfil. Desactivado: solo ve una pantalla de agradecimiento, y el resultado queda con RR. HH.',
        on: 'Sí',
        off: 'No',
      },
    },

    appearance: {
      cardTitle: 'Apariencia',
      cardDescription: 'Personaliza la interfaz visual del sistema',
      navigationStyle: {
        sectionTitle: 'Estilo de Navegación (Escritorio)',
        sectionDescription: 'Elige entre menú lateral o menú superior. Esta opción solo afecta la vista de escritorio.',
        sidebar: 'Menú lateral',
        sidebarDescription: 'Barra lateral tradicional a la izquierda',
        topbar: 'Menú superior',
        topbarDescription: 'Barra horizontal en la parte superior',
      },
      theme: {
        sectionTitle: 'Tema del Sistema',
        sectionDescription: 'Elige entre tema claro u oscuro para la interfaz.',
        light: 'Tema claro',
        dark: 'Tema oscuro',
      },
      navTheme: {
        sectionTitle: 'Menú (lateral y superior)',
        sectionDescription: 'Elige el color del menú de navegación, independiente del tema del sistema.',
        dark: 'Oscuro (recomendado)',
        darkDescription: 'Menú siempre oscuro, resaltado con el color de la marca',
        system: 'Seguir el tema',
        systemDescription: 'El menú sigue el tema claro u oscuro',
      },
    },

    shortcuts: {
      pageTitle: 'Atajos de Teclado',
      pageDescription: 'Usa atajos para acceder rápidamente a las funciones',
      toggleLabel: 'Atajos de Teclado',
      toggleEnabled: 'Los atajos están activados',
      toggleDisabled: 'Los atajos están desactivados',
      groupNavigation: 'Navegación',
      groupGeneral: 'General',
      tips: {
        title: 'Consejos',
        shiftNav: 'Usa {modifier} + tecla para navegación rápida',
        noTyping: 'Los atajos no funcionan cuando estás escribiendo en campos de texto',
        canDisable: 'Puedes desactivar los atajos en cualquier momento con el interruptor de arriba',
      },
      goto_dashboard: { label: 'Panel', description: 'Navegar al panel principal' },
      goto_os: { label: 'Órdenes de Trabajo', description: 'Navegar a órdenes de trabajo' },
      goto_agenda: { label: 'Agenda', description: 'Navegar a la agenda' },
      goto_clients: { label: 'Clientes', description: 'Navegar a clientes' },
      goto_equipment: { label: 'Equipos', description: 'Navegar a equipos' },
      goto_crm: { label: 'CRM', description: 'Navegar al CRM' },
      goto_finance: { label: 'Finanzas', description: 'Navegar a finanzas' },
      goto_inventory: { label: 'Inventario', description: 'Navegar al inventario' },
      goto_quotes: { label: 'Presupuestos', description: 'Navegar a presupuestos' },
      goto_contracts: { label: 'Contratos', description: 'Navegar a contratos' },
      goto_settings: { label: 'Configuración', description: 'Abrir configuración' },
      goto_profile: { label: 'Perfil', description: 'Abrir perfil de usuario' },
    },

    dangerZone: {
      title: 'Zona de Peligro',
      description: 'Acciones irreversibles. Revisa cuidadosamente antes de confirmar.',
      resetSystem: {
        label: 'Reiniciar Sistema',
        description: 'Elimina los datos operativos seleccionados. Los datos de la empresa y usuarios se conservan. Toda operación queda registrada en el log de auditoría.',
        button: 'Reiniciar Sistema',
      },
    },

    dataExport: {
      cardTitle: 'Exportar mis datos',
      cardDescription: 'Descarga una planilla de Excel con los datos de tu empresa: clientes, equipos, órdenes de servicio, presupuestos, contratos, finanzas y más. Útil para respaldo o para llevar tus datos a otro lugar.',
      button: 'Exportar datos',
      dialogTitle: 'Exportar mis datos',
      dialogDescription: 'Elige qué incluir en la planilla. El archivo se genera al instante y se descarga automáticamente.',
      selectAll: 'Marcar todo',
      clearAll: 'Limpiar selección',
      confirm: 'Exportar',
      cancel: 'Cancelar',
      running: 'Exportando...',
      progressLabel: 'Exportando {sheet}... {rows} registros',
      successToast: '¡Exportación completa! El archivo fue descargado.',
      errorToast: 'No fue posible exportar los datos. Inténtalo de nuevo.',
      emptySelection: 'Selecciona al menos un elemento para exportar.',
      heavyHint: 'Movimientos de Stock y Fichaje vienen desmarcados por defecto porque suelen tener muchos registros. Márcalos si también los necesitas.',
      fileNamePrefix: 'backup',
      groups: {
        customers: 'Clientes',
        equipment: 'Equipos',
        serviceOrders: 'Órdenes de Servicio',
        quotes: 'Presupuestos',
        contracts: 'Contratos',
        financial: 'Finanzas',
        inventory: 'Stock',
        inventoryMovements: 'Movimientos de Stock',
        employees: 'Empleados',
        leads: 'CRM / Leads',
        timeRecords: 'Fichaje',
      },
      sheets: {
        summary: 'Resumen',
        customers: 'Clientes',
        customerContacts: 'Contactos de Clientes',
        equipment: 'Equipos',
        serviceOrders: 'Órdenes de Servicio',
        serviceMaterials: 'Materiales de las OS',
        quotes: 'Presupuestos',
        quoteItems: 'Ítems de Presupuestos',
        contracts: 'Contratos',
        contractItems: 'Ítems de Contratos',
        financialTransactions: 'Finanzas',
        financialAccounts: 'Cuentas Bancarias',
        inventory: 'Stock',
        inventoryMovements: 'Movimientos de Stock',
        employees: 'Empleados',
        leads: 'CRM Leads',
        timeRecords: 'Fichaje',
      },
      summary: {
        companyLabel: 'Empresa',
        generatedAtLabel: 'Generado el',
        sheetColumn: 'Hoja',
        rowCountColumn: 'Registros',
        totalLabel: 'Total',
      },
      columns: {
        summary: {
          company: 'Empresa',
          generatedAt: 'Generado el',
          sheet: 'Hoja',
          rows: 'Registros',
        },
        customers: {
          name: 'Nombre',
          nome_fantasia: 'Nombre Comercial',
          company_name: 'Razón Social',
          customer_type: 'Tipo de Cliente',
          document: 'RFC/DNI',
          inscricao_estadual: 'Inscripción Estatal',
          inscricao_municipal: 'Inscripción Municipal',
          email: 'Email',
          phone: 'Teléfono',
          celular: 'Celular',
          address: 'Dirección',
          address_number: 'Número',
          complement: 'Complemento',
          neighborhood: 'Barrio',
          city: 'Ciudad',
          state: 'Estado',
          zip_code: 'Código Postal',
          origin: 'Origen',
          birth_date: 'Fecha de Nacimiento',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        customerContacts: {
          customer: 'Cliente',
          name: 'Nombre',
          position: 'Cargo',
          phone: 'Teléfono',
          email: 'Email',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        equipment: {
          customer: 'Cliente',
          name: 'Nombre',
          category: 'Categoría',
          brand: 'Marca',
          model: 'Modelo',
          serial_number: 'Número de Serie',
          identifier: 'Identificador',
          capacity: 'Capacidad',
          location: 'Ubicación',
          status: 'Estado',
          install_date: 'Fecha de Instalación',
          warranty_until: 'Garantía Hasta',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        serviceOrders: {
          order_number: 'Número de Orden',
          status: 'Estado',
          os_type: 'Tipo de Orden',
          service_type: 'Tipo de Servicio',
          entry_type: 'Tipo de Entrada',
          origin: 'Origen',
          customer: 'Cliente',
          equipment: 'Equipo',
          technician: 'Técnico',
          team: 'Equipo de Trabajo',
          contract: 'Contrato',
          task_title: 'Título de la Tarea',
          description: 'Descripción',
          diagnosis: 'Diagnóstico',
          solution: 'Solución',
          notes: 'Notas',
          scheduled_date: 'Fecha Programada',
          scheduled_time: 'Hora Programada',
          started_at: 'Iniciado el',
          completed_at: 'Finalizado el',
          duration_minutes: 'Duración (minutos)',
          labor_hours: 'Horas Trabajadas',
          labor_value: 'Valor de Mano de Obra',
          parts_value: 'Valor de Repuestos',
          total_value: 'Valor Total',
          pmoc_conformity_status: 'Conformidad PMOC',
          service_address: 'Dirección del Servicio',
          service_address_number: 'Número de Dirección',
          service_neighborhood: 'Barrio del Servicio',
          service_city: 'Ciudad del Servicio',
          service_state: 'Estado del Servicio',
          service_zip_code: 'Código Postal del Servicio',
          created_at: 'Creado el',
        },
        serviceMaterials: {
          order_number: 'Número de Orden',
          item_name: 'Material',
          quantity: 'Cantidad',
          unit: 'Unidad',
          purchase_price: 'Precio de Compra',
          sale_price: 'Precio de Venta',
          subtotal: 'Subtotal',
          created_at: 'Creado el',
        },
        quotes: {
          quote_number: 'Número de Presupuesto',
          status: 'Estado',
          customer: 'Cliente',
          prospect_name: 'Nombre del Prospecto',
          prospect_email: 'Email del Prospecto',
          prospect_phone: 'Teléfono del Prospecto',
          subtotal: 'Subtotal',
          discount_type: 'Tipo de Descuento',
          discount_value: 'Valor del Descuento',
          discount_amount: 'Descuento Aplicado',
          displacement_cost: 'Costo de Desplazamiento',
          distance_km: 'Distancia (km)',
          km_cost: 'Costo por Km',
          total_cost: 'Costo Total',
          total_price: 'Precio Total',
          total_value: 'Valor Total',
          bdi: 'BDI (%)',
          profit_rate: 'Margen de Ganancia (%)',
          tax_rate: 'Tasa de Impuestos (%)',
          admin_indirect_rate: 'Tasa Administrativa (%)',
          card_installments: 'Cuotas de Tarjeta',
          card_discount_rate: 'Descuento de Tarjeta (%)',
          valid_until: 'Válido Hasta',
          view_count: 'Nº de Visualizaciones',
          last_viewed_at: 'Última Visualización',
          terms: 'Condiciones',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        quoteItems: {
          quote_number: 'Número de Presupuesto',
          position: 'Posición',
          item_type: 'Tipo de Ítem',
          description: 'Descripción',
          details: 'Detalles',
          service_type: 'Tipo de Servicio',
          quantity: 'Cantidad',
          unit_price: 'Precio Unitario',
          price_override: 'Precio Ajustado',
          total_price: 'Precio Total',
          unit_total_cost: 'Costo Unitario Total',
          unit_labor_cost: 'Costo Unitario de Mano de Obra',
          unit_materials_cost: 'Costo Unitario de Materiales',
          unit_extras_cost: 'Costo Unitario de Extras',
          unit_hours: 'Horas por Unidad',
          unit_hourly_rate: 'Valor de la Hora',
          bdi: 'BDI (%)',
          profit_rate: 'Margen de Ganancia (%)',
        },
        contracts: {
          name: 'Nombre',
          customer: 'Cliente',
          status: 'Estado',
          is_pmoc: 'Es PMOC',
          frequency_type: 'Tipo de Frecuencia',
          frequency_value: 'Valor de Frecuencia',
          start_date: 'Fecha de Inicio',
          horizon_months: 'Horizonte (meses)',
          next_pmoc_generation_date: 'Próxima Generación PMOC',
          service_type: 'Tipo de Servicio',
          technician: 'Técnico',
          team: 'Equipo',
          responsible_technician: 'Técnico Responsable',
          unidade_nome: 'Nombre de la Unidad',
          unidade_endereco: 'Dirección de la Unidad',
          unidade_numero: 'Número de la Unidad',
          unidade_complemento: 'Complemento de la Unidad',
          unidade_bairro: 'Barrio de la Unidad',
          unidade_cidade: 'Ciudad de la Unidad',
          unidade_uf: 'Estado de la Unidad',
          unidade_cep: 'Código Postal de la Unidad',
          pmoc_identificacao_ambiente: 'Identificación del Ambiente (PMOC)',
          pmoc_tipo_atividade: 'Tipo de Actividad (PMOC)',
          pmoc_area_climatizada_m2: 'Área Climatizada (m²)',
          pmoc_carga_termica_tr: 'Carga Térmica (TR)',
          pmoc_ocupantes_fixos: 'Ocupantes Fijos',
          pmoc_ocupantes_flutuantes: 'Ocupantes Flotantes',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        contractItems: {
          contract: 'Contrato',
          item_name: 'Nombre del Ítem',
          item_description: 'Descripción del Ítem',
          equipment: 'Equipo',
          pmoc_scope: 'Alcance PMOC',
          pmoc_start_visit: 'Visita Inicial PMOC',
          sort_order: 'Orden',
          created_at: 'Creado el',
        },
        financialTransactions: {
          transaction_date: 'Fecha de la Transacción',
          due_date: 'Vencimiento',
          paid_date: 'Fecha de Pago',
          description: 'Descripción',
          transaction_type: 'Tipo',
          category: 'Categoría',
          amount: 'Valor',
          amount_received: 'Valor Recibido',
          is_paid: 'Pagado',
          payment_method: 'Forma de Pago',
          account: 'Cuenta',
          customer: 'Cliente',
          employee: 'Empleado',
          contract: 'Contrato',
          order_number: 'Número de Orden',
          installment_number: 'Cuota',
          installment_total: 'Total de Cuotas',
          payroll_kind: 'Tipo de Nómina',
          payroll_period: 'Período de Nómina',
          credit_card_bill_date: 'Fecha de Factura de Tarjeta',
          cancelled_at: 'Cancelado el',
          cancelled_reason: 'Motivo de Cancelación',
          notes: 'Notas',
          created_at: 'Creado el',
        },
        financialAccounts: {
          name: 'Nombre',
          type: 'Tipo',
          bank_name: 'Banco',
          institution_name: 'Institución',
          initial_balance: 'Saldo Inicial',
          credit_limit: 'Límite de Crédito',
          closing_day: 'Día de Cierre',
          due_day: 'Día de Vencimiento',
          payment_due_days: 'Plazo de Pago (días)',
          is_active: 'Activo',
          created_at: 'Creado el',
        },
        inventory: {
          name: 'Nombre',
          sku: 'Código (SKU)',
          category: 'Categoría',
          unit: 'Unidad',
          stock: 'Ubicación de Stock',
          quantity: 'Cantidad',
          min_quantity: 'Stock Mínimo',
          cost_price: 'Precio de Costo',
          sale_price: 'Precio de Venta',
          total_value: 'Valor Total en Stock',
          supplier: 'Proveedor',
          description: 'Descripción',
        },
        inventoryMovements: {
          created_at: 'Fecha',
          movement_type: 'Tipo de Movimiento',
          item: 'Material',
          sku: 'Código (SKU)',
          stock: 'Ubicación de Stock',
          quantity: 'Cantidad',
          unit_cost: 'Costo Unitario',
          stock_before: 'Saldo Anterior',
          stock_after: 'Saldo Posterior',
          supplier: 'Proveedor',
          order_number: 'Número de Orden',
          notes: 'Notas',
        },
        employees: {
          name: 'Nombre',
          matricula: 'Matrícula',
          cpf: 'DNI',
          position: 'Cargo',
          cbo: 'Código de Ocupación',
          employment_regime: 'Régimen de Contratación',
          hire_date: 'Fecha de Ingreso',
          is_active: 'Activo',
          email: 'Email',
          phone: 'Teléfono',
          address: 'Dirección',
          salary: 'Salario',
          monthly_cost: 'Costo Mensual',
          dependents_count: 'Nº de Dependientes',
          payment_frequency: 'Frecuencia de Pago',
          payment_day: 'Día de Pago',
          payment_day_2: '2º Día de Pago',
          payment_day_type: 'Tipo de Día de Pago',
          payment_weekday: 'Día de la Semana de Pago',
          vt_enabled: 'Vale de Transporte Activo',
          vt_monthly_value: 'Valor Mensual del Vale de Transporte',
          pix_key: 'Clave Pix',
          created_at: 'Creado el',
        },
        leads: {
          title: 'Título',
          status: 'Estado',
          stage: 'Etapa',
          customer: 'Cliente',
          assigned_to: 'Responsable',
          source: 'Origen',
          value: 'Valor',
          probability: 'Probabilidad (%)',
          expected_close_date: 'Fecha Estimada de Cierre',
          notes: 'Notas',
          created_at: 'Creado el',
          updated_at: 'Actualizado el',
        },
        timeRecords: {
          date: 'Fecha',
          recorded_at: 'Registrado el',
          type: 'Tipo',
          employee: 'Empleado',
          source: 'Origen',
          is_valid: 'Válido',
          address: 'Dirección',
          latitude: 'Latitud',
          longitude: 'Longitud',
          notes: 'Notas',
        },
      },
      values: {
        discountTypeValor: 'Valor',
        discountTypePercentual: 'Porcentaje',
        itemTypeServico: 'Servicio',
        itemTypeMaterial: 'Material',
        itemTypeMaoDeObra: 'Mano de Obra',
        originManual: 'Manual',
        originContract: 'Generado por Contrato',
        originPortal: 'Portal del Cliente',
        payrollKindRescission: 'Liquidación',
        timeRecordSourceApp: 'Aplicación',
        timeRecordSourceAdmin: 'Administrador',
        timeRecordSourceLinkPublico: 'Enlace Público',
      },
    },

    resetSystem: {
      modalTitle: 'Reiniciar Sistema',
      irreversibleWarning: 'Esta acción',
      deletePermanently: 'ELIMINARÁ PERMANENTEMENTE',
      warningCompany: 'los datos operativos de la empresa',
      chooseWhat: 'Elige qué deseas eliminar:',
      selectAll: 'Seleccionar todo',
      selectedCount: '{count}/{total} seleccionados',
      groupSelectAriaLabel: 'Seleccionar todo en {group}',
      forcedByMaterials: 'Necesario porque el catálogo de materiales fue seleccionado.',
      preserved: {
        title: 'Se conservarán:',
        companyData: 'Datos básicos de la empresa',
        users: 'Usuarios registrados',
        paymentHistory: 'Historial de pagos',
      },
      finalWarning: '¡Esta acción NO PUEDE deshacerse!',
      confirmLabel: 'Para confirmar, escribe el nombre de la empresa:',
      confirmPlaceholder: 'Nombre de la empresa',
      nameMismatch: 'El nombre ingresado no coincide con el nombre de la empresa.',
      offline: 'Esta acción requiere conexión a internet. Estás sin conexión.',
      progressTitle: 'Eliminando datos...',
      progressStep: 'Paso {current} de {total}:',
      cancel: 'Cancelar',
      confirmButton: 'Sí, Reiniciar Sistema',
      successTitle: 'Sistema reiniciado con éxito',
      successDesc: 'Los datos seleccionados han sido eliminados. El log de auditoría ha sido generado.',
      errorTitle: 'No fue posible reiniciar el sistema',
      errorPermission: 'No tienes permiso para reiniciar el sistema. Solo el administrador de la empresa puede hacerlo.',
      errorTimeout: 'Un paso tardó demasiado. Inténtalo de nuevo, lo que ya fue eliminado sigue eliminado.',
      errorDependency: 'No fue posible completar el paso por una dependencia inesperada. Avisa al soporte con el nombre del paso.',
      errorDuplicate: 'No fue posible recrear una configuración predeterminada. Inténtalo de nuevo en unos segundos.',
      errorGeneric: 'No fue posible reiniciar el sistema. Inténtalo de nuevo o avisa al soporte.',
      errorStepPrefix: 'Error en el paso "{step}".',

      groups: {
        operacional: {
          label: 'Operativo',
          delete_service_orders: {
            title: 'Órdenes de Trabajo',
            description: 'Elimina todas las OT, fotos, materiales consumidos, evaluaciones y formularios respondidos',
          },
          delete_equipment: {
            title: 'Equipos',
            description: 'Elimina todos los equipos registrados para los clientes',
          },
          delete_contracts: {
            title: 'Contratos y Planes de Mantenimiento',
            description: 'Elimina todos los contratos, cronogramas y documentos de mantenimiento personalizados',
          },
          delete_custom_configs: {
            title: 'Configuraciones personalizadas',
            description: 'Elimina etapas CRM personalizadas, formularios de OT y recursos de costo',
          },
        },
        comercial: {
          label: 'Ventas (CRM)',
          delete_quotes: {
            title: 'Presupuestos y Propuestas',
            description: 'Elimina todos los presupuestos y propuestas',
          },
          delete_customers: {
            title: 'Clientes y Leads',
            description: 'Elimina todos los registros de clientes, contactos, portales y leads',
          },
        },
        financeiro: {
          label: 'Finanzas',
          delete_financial_movements: {
            title: 'Transacciones',
            description: 'Elimina todas las transacciones de caja, banco, tarjeta, ingresos y gastos',
          },
          delete_financial_categories: {
            title: 'Categorías financieras',
            description: 'Elimina las categorías financieras personalizadas (las predeterminadas se mantienen)',
          },
        },
        rh_inventario: {
          label: 'RR.HH. e Inventario',
          delete_employees: {
            title: 'Empleados y Equipos',
            description: 'Elimina todos los empleados, anticipos, pagos, registros de tiempo y equipos',
          },
          delete_materials: {
            title: 'Materiales',
            description: 'Elimina todos los materiales registrados (y su inventario)',
          },
          delete_stock: {
            title: 'Inventario',
            description: 'Limpia el inventario (movimientos), mantiene los materiales registrados',
          },
        },
      },

      stepLabels: {
        service_orders: 'Órdenes de Trabajo',
        contracts: 'Contratos',
        quotes: 'Presupuestos',
        equipment: 'Equipos',
        custom_configs: 'Configuraciones Personalizadas',
        financial_movements: 'Transacciones Financieras',
        financial_categories: 'Categorías Financieras',
        employees: 'Empleados y RR.HH.',
        stock: 'Inventario de Materiales',
        materials: 'Catálogo de Materiales',
        customers: 'Clientes',
      },
    },

    users: {
      pageTitle: 'Usuarios y Permisos',
      pageSubtitle: '{count}/{max} usuarios • {active} activos',
      tabs: {
        users: 'Usuarios',
        presets: 'Cargos',
      },
      searchPlaceholderUsers: 'Buscar usuarios...',
      searchPlaceholderPresets: 'Buscar cargos...',
      searchPlaceholderDesktop: 'Buscar por nombre o teléfono...',
      viewList: 'Lista',
      viewCards: 'Tarjetas',
      btnSettings: 'Configuración',
      btnCreateUser: 'Crear Usuario',
      btnHireMore: 'Agregar más usuarios',
      limitTitle: 'Límite de usuarios alcanzado ({count}/{max})',
      limitDesc: 'Para registrar nuevos usuarios, añade plazas de usuario adicionales a tu plan.',
      badgeYou: 'Tú',
      badgeActive: 'Activo',
      badgeInactive: 'Inactivo',
      badgeAllPerms: 'Acceso Total',
      badgePermCount: '{count} permisos',
      fabUser: 'Usuario',
      fabPreset: 'Cargo',
      btnEdit: 'Editar',
      btnDeactivate: 'Desactivar',
      btnReactivate: 'Reactivar',
      btnReactivateDisabledTitle: 'Sin plaza disponible — mejora el plan o desactiva otro usuario',
      emptyFiltered: 'No se encontraron usuarios',
      emptyAll: 'No hay usuarios registrados',
      deleteUserTitle: '¿Eliminar usuario permanentemente?',
      deleteUserDesc: 'Esta acción es irreversible. El usuario {name} será eliminado permanentemente del sistema, incluyendo su acceso, permisos y vínculos.',
      deletePresetTitle: '¿Eliminar cargo?',
      deletePresetDesc: 'El cargo {name} será eliminado. Los usuarios vinculados a él conservan sus permisos individuales. Esta acción no puede deshacerse.',
      deleteLoading: 'Eliminando...',
      deletePermanently: 'Eliminar Permanentemente',
      cancel: 'Cancelar',
      delete: 'Eliminar',
      toastCreated: '¡Usuario creado con éxito!',
      toastCreateError: 'Error al crear usuario',
      toastCreateErrorDuplicate: 'Este correo ya está registrado en el sistema.',
      toastUpdated: '¡Usuario actualizado!',
      toastUpdateError: 'Error',
      toastDeactivated: 'Usuario desactivado',
      toastDeactivatedDesc: 'La plaza fue liberada. Puedes reactivarlo después.',
      toastDeactivateError: 'Error al desactivar',
      toastReactivated: '¡Usuario reactivado!',
      toastReactivateError: 'Error al reactivar',
      toastDeleted: '¡Usuario eliminado permanentemente!',
      toastDeleteError: 'Error al eliminar',
      toastPresetDuplicated: '¡Cargo duplicado!',
      toastPresetDuplicateError: 'Error al duplicar',
      toastPresetDeleted: '¡Cargo eliminado!',
      toastPresetDeleteError: 'Error al eliminar',
      blockLastAdminTitle: 'Acción bloqueada',
      blockLastAdminDesc: 'La empresa debe tener al menos un administrador activo.',
      btnDeleteTitle: 'Eliminar usuario permanentemente',

      form: {
        titleCreate: 'Crear Usuario',
        titleEdit: 'Editar Usuario',
        descCreate: 'Completa los datos del nuevo usuario',
        descEdit: 'Actualiza los datos y permisos del usuario',
        cancel: 'Cancelar',
        save: 'Actualizar',
        create: 'Crear Usuario',
        photoLabel: 'Foto del Usuario',
        photoReplace: 'Reemplazar',
        photoSelect: 'Seleccionar foto',
        photoRemove: 'Eliminar',
        photoHint: 'Foto opcional del usuario',
        labelFullName: 'Nombre Completo *',
        placeholderFullName: 'Nombre del usuario',
        labelPassword: 'Contraseña *',
        placeholderPassword: 'Crea una contraseña segura',
        labelEmail: 'Correo',
        labelEmailRequired: 'Correo *',
        labelEmailHint: 'Cambiar el correo modificará el acceso del usuario',
        labelPhone: 'Teléfono',
        labelEmployee: 'Vincular a Empleado',
        placeholderEmployee: 'Ningún empleado vinculado',
        employeeNone: 'Ninguno',
        employeeHint: 'Vincula este usuario a un empleado registrado',
        emailConflictTitle: 'Los correos son diferentes. ¿Cuál debe prevalecer?',
        emailConflictUser: 'Usuario:',
        emailConflictEmployee: 'Empleado:',
        emailConflictHint: 'El correo elegido se aplicará a ambos registros',
        labelUserType: 'Tipo de Usuario',
        typeInternal: 'Interno',
        typeTech: 'Técnico',
        descTech: 'Aparece en la lista de técnicos de la OT y puede agregarse a equipos',
        descInternal: 'Usuario interno del sistema, no aparece como técnico en las OT',
        labelProfile: 'Perfil de Acceso',
        placeholderProfile: 'Selecciona un perfil',
        profileCustom: 'Personalizado',
        profileAll: 'Acceso Total',
        sectionScreens: 'Pantallas',
        hintScreens: 'Selecciona a qué pantallas puede acceder este usuario',
        sectionFunctions: 'Funciones',
        hintFunctions: 'Selecciona qué acciones puede realizar este usuario',
        sectionPermissions: 'Permisos',
        hintPermissions: 'Elige un perfil rápido o activa pantalla por pantalla. Las acciones de cada pantalla están dentro de ella.',
      },

      // ── Editor de permisos (compartido usuario/cargo) ─────────────────────────
      permissionsEditor: {
        quickProfiles: 'Perfiles rápidos',
        fullAccess: 'Acceso Total',
        fullAccessHint: 'Habilita todas las pantallas y acciones, incluso las futuras',
        custom: 'Personalizado',
        searchPlaceholder: 'Buscar pantalla o acción...',
        clearSearch: 'Limpiar búsqueda',
        actionsTitle: 'Acciones de esta pantalla',
        screenOffHint: 'La pantalla está desactivada, pero estas acciones siguen valiendo por sí solas.',
        emptySearch: 'No se encontró ninguna pantalla o acción para "{query}".',
        generalGroup: 'General',
        ariaScreenSwitch: 'Acceso a la pantalla {screen}',
        ariaScreenActions: 'Ver acciones de {screen}',
      },

      presets: {
        dialogTitle: 'Configuración de Cargos',
        dialogDesc: 'Crea y edita perfiles de acceso con permisos predefinidos',
        btnCreate: 'Nuevo Cargo',
        btnSave: 'Guardar',
        btnSaveNew: 'Crear Cargo',
        btnCancel: 'Cancelar',
        btnBack: '← Volver',
        empty: 'No hay cargos registrados',
        permCount: '{count} permisos',
        labelName: 'Nombre *',
        placeholderName: 'Ej: Técnico de Campo',
        labelDesc: 'Descripción',
        placeholderDesc: 'Descripción opcional',
        sectionScreens: 'Pantallas',
        sectionFunctions: 'Funciones',
        emptyMobileTitle: 'No hay cargos registrados',
        emptyMobileDesc: 'Toca "Nuevo Cargo" para crear un perfil de acceso',
        noDescription: 'Sin descripción',
        actionEdit: 'Editar',
        actionDuplicate: 'Duplicar',
        actionDelete: 'Eliminar',
      },

      list: {
        ariaViewPhoto: 'Ver foto',
        emptyFilteredDesc: 'Intenta una búsqueda diferente',
        emptyAllDesc: 'Toca "Nuevo Usuario" para comenzar',
        reactivateNoSlot: 'Reactivar (sin plaza)',
      },
    },

    profile: {
      pageTitle: 'Mi Perfil',
      pageSubtitle: 'Administra tu información personal',
      labelFullName: 'Nombre Completo',
      placeholderFullName: 'Tu nombre',
      labelEmail: 'Correo',
      labelPhone: 'Teléfono',
      placeholderPhone: '(00) 00000-0000',
      labelCreatedAt: 'Cuenta creada el',
      btnSave: 'Guardar Cambios',
      btnSaving: 'Guardando...',
      cardPasswordTitle: 'Contraseña',
      cardPasswordDesc: 'Cambia la contraseña de tu cuenta',
      btnChangePassword: 'Cambiar Contraseña',
      labelNewPassword: 'Nueva Contraseña',
      placeholderNewPassword: 'Crea una contraseña segura',
      labelConfirmPassword: 'Confirmar Nueva Contraseña',
      placeholderConfirmPassword: 'Repite la nueva contraseña',
      btnSavePassword: 'Guardar Nueva Contraseña',
      btnCancel: 'Cancelar',
      toastProfileUpdated: '¡Perfil actualizado con éxito!',
      toastSaveError: 'Error al guardar',
      toastPhotoUpdated: '¡Foto actualizada!',
      toastPhotoError: 'Error al subir la foto',
      toastPasswordWeak: 'Contraseña débil',
      toastPasswordWeakDesc: 'Usa al menos 8 caracteres con mayúsculas, minúsculas, números y/o caracteres especiales.',
      toastPasswordMismatch: 'Las contraseñas no coinciden',
      toastPasswordChanged: '¡Contraseña cambiada con éxito!',
      toastPasswordError: 'Error al cambiar contraseña',
      btnBack: 'Volver',
    },

    billing: {
      pageTitle: 'Suscripción',
      pageSubtitle: 'Gestiona tu plan, módulos y pagos',
      heroTrialTitle: 'Activa tu Suscripción',
      heroTrialDesc: 'Estás en el período de prueba. Elige el plan ideal y obtén acceso completo.',
      heroTrialDaysLeft: '{days} día{s} restante{s} de prueba',
      btnChoosePlan: 'Elegir Plan y Activar',
      planLabel: 'Plan {name}',
      statusExpired: 'Vencida',
      statusExpiringSoon: 'Vence pronto',
      statusActive: 'Activa',
      statusMsgExpired: 'Vencida hace {days} día{s}',
      statusMsgExpiringSoon: 'Vence en {days} día{s}',
      statusMsgActive: '{days} días restantes',
      statusMsgActiveNoDate: 'Activa',
      labelMonthlyValue: 'Valor mensual',
      labelExpiration: 'Vencimiento',
      sectionActiveModules: 'Módulos Activos',
      sectionAvailableModules: 'Módulos Disponibles',
      btnHireModule: 'Contratar',
      cardPaymentTitle: 'Pago',
      cardPaymentDescExpired: 'Tu suscripción ha vencido. Renueva ahora.',
      cardPaymentDescActive: 'Mantén tu suscripción al día.',
      labelAmountDue: 'Monto a pagar',
      labelDueDate: 'Vencimiento',
      btnPayNow: 'Pagar Ahora',
      paymentYearlyDiscount: '20% de descuento en el plan anual · válido para Pix o Boleto',
      bannerPromoTitle: 'Precio promocional activo',
      bannerPromoDesc: 'Tras el período promocional, el valor volverá a {value}.',
      bannerPendingTitle: 'Cambio de valor programado',
      bannerPendingDesc: 'Valor actual: {current}. A partir del próximo cobro: {new}.',
      bannerRecurringTitle: 'Renovación automática activa',
      bannerRecurringDesc: 'Tu suscripción se renueva automáticamente en la tarjeta registrada.',
      bannerNfseTitle: 'Datos pendientes para emitir facturas',
      bannerNfseDesc: 'Completa en la configuración de la empresa: {fields}',
      cardSummaryTitle: 'Resumen del Plan',
      summaryPlan: 'Plan',
      summaryActiveModules: 'Módulos activos',
      summaryMaxUsers: 'Máx. Usuarios',
      summaryCycle: 'Ciclo',
      summaryStatus: 'Estado',
      cycleYearly: 'Anual',
      cycleMonthly: 'Mensual',
      cardPaymentHistoryTitle: 'Historial de pagos',
      cardPaymentHistoryDesc: 'Cobros y pagos de tu suscripción.',
      btnCancelSubscription: 'Cancelar suscripción',
      cancelSubscriptionNote: 'Al cancelar, la renovación automática se detiene y mantienes el acceso hasta el vencimiento.',
      btnManagePlan: 'Gestionar Mi Plan',
      nfseTierSectionTitle: 'Nivel de Facturas (NFS-e)',
      nfseTierCurrent: 'actual',
      nfseTierUpgradeNote: 'El nuevo nivel se activa de inmediato y el nuevo valor entra en la próxima facturación.',
      // Panel "Tu Plan" en tiempo real (ModulesManagementCard)
      planSummaryTitle: 'Tu Plan',
      planSummaryIncluded: 'Incluido',
      planSummaryEmptyCustom: 'Elige módulos a la izquierda para armar tu plan.',
      planSummaryEmptyPlans: 'Selecciona un plan a la izquierda.',
      planSummaryUsersOne: 'usuario',
      planSummaryUsersPlural: 'usuarios',
      planSummaryUsersIncluded: 'incluidos',
      planSummaryCycleLabel: 'Ciclo',
      planSummaryCycleYearlyDesc: '20% de descuento en Pix/Boleto',
      planSummaryCycleYearlyInfo: 'El descuento del 20% solo aplica para pago único (Pix o Boleto). Con tarjeta, la facturación sigue siendo mensual.',
      planSummaryYearlySavings: 'Ahorras {value}/año',
      planSummaryTotalMonthly: 'Total mensual',
      planSummaryTotalYearly: 'Total anual',
      planSummaryDiffLabel: 'vs plan actual',
      planSummaryCtaMonthly: 'Aplicar plan',
      planSummaryCtaYearly: 'Pagar y activar (anual)',
      planSummaryCtaApplying: 'Aplicando...',
      planSummaryUpgradeNote: 'Upgrade: los recursos se liberan de inmediato y el nuevo valor entra en el próximo cobro.',
      planSummaryDowngradeNote: 'Cambio programado: mantienes el plan actual hasta el fin del período pagado. El nuevo valor de {value}/mes entra en el próximo cobro.',
      noCompanyTitle: 'Ninguna empresa vinculada',
      noCompanyDesc: 'Tu cuenta aún no está vinculada a una empresa.',

      cancelSubscription: {
        titleReason: 'Cancelar suscripción',
        titleConfirm: 'Confirmar cancelación',
        titleDone: 'Cancelación completada',
        headerSubtitle: 'Ayúdanos a entender el motivo',
        labelReason: 'Motivo de cancelación',
        placeholderReason: 'Selecciona un motivo',
        labelDetailsRequired: 'Explica tu motivo *',
        labelDetailsOptional: '¿Quieres contarnos más? (opcional)',
        placeholderDetailsRequired: 'Describe el motivo de cancelación...',
        placeholderDetailsOther: '¿Alguna sugerencia o comentario...',
        retentionTalkSupport: 'Hablar con soporte',
        confirmOnConfirm: 'Al confirmar:',
        confirmBullet1: 'La renovación automática será cancelada',
        confirmBullet2Until: 'Mantienes acceso hasta',
        confirmBullet2NoDate: 'el fin del período ya pagado',
        confirmBullet3: 'Los cobros futuros pendientes se cancelarán automáticamente',
        confirmBullet4: 'Puedes reactivar cuando quieras',
        confirmReasonPrefix: 'Motivo:',
        confirmDetailsPrefix: 'Detalles:',
        doneTitle: 'Suscripción cancelada',
        doneDesc: 'La renovación automática fue cancelada. Mantienes acceso',
        doneDescUntil: 'hasta {date}',
        doneDescNoDate: 'hasta el fin del período ya pagado',
        doneDescSuffix: '. Si cambias de opinión, puedes reactivar en cualquier momento.',
        btnBack: 'Volver',
        btnContinue: 'Continuar',
        btnCanceling: 'Cancelando...',
        btnConfirm: 'Confirmar cancelación',
        btnUnderstood: 'Entendido',
        toastError: 'No fue posible cancelar ahora. Intenta de nuevo o contacta al soporte.',
        toastSelectReason: 'Selecciona un motivo',
        reasons: {
          preco_alto: 'El precio es muy alto',
          nao_uso: 'No lo estoy usando suficiente',
          funcionalidades: 'Faltan funciones que necesito',
          dificuldade_uso: 'Dificultad para usar el sistema',
          concorrente: 'Encontré otra solución',
          fechando_empresa: 'Estoy cerrando mi empresa',
          temporario: 'Pausa temporal',
          outro: 'Otro motivo',
        },
        retention: {
          preco_alto: {
            title: '¿Qué tal un descuento?',
            message: 'Habla con nosotros por WhatsApp, podemos negociar un precio especial para que sigas en Dominex.',
            waText: '¡Hola! Estoy pensando en cancelar mi suscripción de Dominex por el precio. ¿Hay alguna condición especial?',
          },
          nao_uso: {
            title: '¡Podemos ayudarte!',
            message: 'Nuestro equipo ofrece una capacitación personalizada gratuita para que aproveches el sistema al máximo.',
            waText: '¡Hola! Me gustaría agendar una capacitación de Dominex antes de decidir sobre mi suscripción.',
          },
          funcionalidades: {
            title: '¡Tu opinión es valiosa!',
            message: 'Cuéntanos qué funciones necesitas. Estamos en constante evolución y tu sugerencia podría incluirse pronto.',
            waText: '¡Hola! Echo de menos algunas funciones en Dominex y me gustaría sugerirlas antes de cancelar.',
          },
          dificuldade_uso: {
            title: 'Soporte dedicado',
            message: 'Podemos agendar una sesión de soporte dedicada para resolver tus dudas y facilitar el uso.',
            waText: '¡Hola! Tengo dificultades para usar Dominex y me gustaría una sesión de soporte antes de cancelar.',
          },
        },
        // Oferta de downgrade
        downgradeOfferTitle: '¿Qué tal un plan más económico?',
        downgradeOfferDesc: 'En vez de cancelar, cambia al plan {plan} y ahorra {savings} al mes.',
        downgradeOfferCta: 'Cambiar a {plan}',
        downgradeOfferApplying: 'Aplicando...',
        downgradeOfferToastSuccess: 'Plan cambiado a {plan} correctamente.',
        downgradeOfferToastError: 'No fue posible cambiar el plan ahora. Intenta de nuevo.',
      },

      paymentHistory: {
        empty: 'Aún no hay pagos registrados.',
        perPage: 'Por página',
        pageSummary: 'Página {current} de {total}',
        prevPage: 'Página anterior',
        nextPage: 'Página siguiente',
        viewCharge: 'Ver cobro',
        statusPaid: 'Pagado',
        statusPending: 'Pendiente',
        statusOverdue: 'Vencido',
        statusCancelled: 'Cancelado',
        typeSale: 'Venta',
        typeRenewal: 'Renovación',
        methodPix: 'PIX',
        methodBoleto: 'Boleta',
        methodCard: 'Tarjeta',
        methodDefault: 'Cobro',
      },

      accountUsage: {
        title: 'Uso de la cuenta',
        description: 'Controla el consumo de tu plan',
        users: 'Usuarios',
        nfseThisMonth: 'Facturas este mes',
        usersIncluded: '{base} incluidos + {extra} adicional',
        usersIncludedPlural: '{base} incluidos + {extra} adicionales',
        usersPlanLimit: '{max} usuario en el plan actual',
        usersPlanLimitPlural: '{max} usuarios en el plan actual',
        nfseUnlimitedLabel: '{used} emitidas',
        nfseLevelLimit: 'Limite del Nivel {level} — {limit} facturas/mes',
        nfsePlanLimit: 'Limite del plan — {limit} facturas/mes',
        paymentMethods: 'Formas de pago',
      },
    },

    changelog: {
      pageTitle: 'Novedades',
      searchPlaceholder: 'Buscar actualizaciones...',
      currentVersionBadge: 'Versión actual',
      filterAll: 'Todas',
      filterRecurso: 'Funciones',
      filterMelhoria: 'Mejoras',
      filterCorrecao: 'Correcciones',
      filterSeguranca: 'Seguridad',
      categoryRecurso: 'Nueva función',
      categoryMelhoria: 'Mejora',
      categoryCorrecao: 'Corrección',
      categorySeguranca: 'Seguridad',
      emptyTitle: 'No se encontraron actualizaciones',
      emptyDesc: 'Prueba una búsqueda diferente o cambia los filtros.',
      changesCount_one: '{count} cambio',
      changesCount_other: '{count} cambios',
      showMore: 'Ver más',
      showLess: 'Ver menos',
    },

    terms: {
      modalTitle: 'Términos de Uso — Dominex',
      pdfTitle: 'TÉRMINOS DE USO — DOMINEX',
      pdfFilename: 'TERMINOS DE USO - DOMINEX',
      checkboxLabel: 'He leído y acepto los Términos de Uso de Dominex',
      btnAccept: 'Acepto los Términos',
      btnAccepting: 'Confirmando...',
      btnDownloadPdf: 'Descargar PDF',
      btnGeneratingPdf: 'Generando PDF...',
      btnClose: 'Cerrar',
      acceptedAt: 'Aceptado el {date}',
      pdfFooterPage: 'Página {current} de {total}',
      pdfGeneratedOn: 'Documento generado el {date}',
      errorPdf: 'Error al generar el PDF. Inténtelo de nuevo.',
    },

    myData: {
      backToDashboard: '← Volver al panel',
      pageTitle: 'Centro de Privacidad',
      pageSubtitle: 'Administra tus datos personales (derechos de protección de datos)',
      cardPersonalTitle: 'Tus datos personales',
      labelName: 'Nombre',
      labelEmail: 'Correo',
      labelPhone: 'Teléfono',
      labelCreatedAt: 'Cuenta creada el',
      cardExportTitle: 'Exportar mis datos',
      cardExportDesc: 'Descarga una copia de tus datos en formato JSON (derecho de portabilidad de datos).',
      btnExport: 'Descargar mis datos',
      btnExporting: 'Exportando...',
      cardDeleteTitle: 'Solicitar eliminación de cuenta',
      cardDeleteDesc: 'Solicita la eliminación permanente de tu cuenta y datos personales. Los datos fiscales y contables pueden conservarse por el plazo legal (5 años). El procesamiento tarda hasta 15 días hábiles.',
      deletionRequested: 'Solicitud registrada. Nuestro equipo se pondrá en contacto.',
      btnRequestDeletion: 'Solicitar eliminación',
      btnRequesting: 'Registrando...',
      cardDpoTitle: 'Contacto con el DPO',
      cardDpoDesc: 'Para ejercer otros derechos (corrección, portabilidad, revocación de consentimiento) o para preguntas sobre el tratamiento de tus datos:',
      linkPrivacy: 'Política de Privacidad',
      linkTerms: 'Términos de Uso',
      toastExported: '¡Datos exportados con éxito!',
      toastExportError: 'Error al exportar datos',
      toastDeletionRequested: 'Solicitud de eliminación enviada',
      toastDeletionRequestedDesc: 'Nuestro equipo procesará tu solicitud en hasta 15 días hábiles conforme a la ley de protección de datos.',
      toastDeletionError: 'Error al registrar la solicitud',
    },

    integrations: {
      subTabs: {
        whatsapp: 'WhatsApp',
        asaas: 'Cobros',
        simulador: 'Simulador',
      },
      simulator: {
        title: 'Simulador de venta',
        description: 'Mira cuánto queda de una venta después de la tarifa de Asaas y cuándo llega el dinero a tu cuenta.',
        notConnectedTitle: 'Tu cuenta Asaas todavía no está conectada',
        notConnectedDesc: 'Los números de abajo usan la tabla de referencia de Asaas. Conecta tu cuenta en la pestaña Cobros para simular con las tarifas reales de tu plan.',
        notContractedTitle: 'Cobros no contratado',
        notContractedDesc: 'El simulador muestra cuánto queda de cada venta por Pix, boleto o tarjeta. Habla con nuestro equipo para activar los cobros en línea.',
        amountLabel: 'Valor de la venta',
        methodLabel: 'Forma de pago',
        methodPix: 'Pix',
        methodBoleto: 'Boleto',
        methodCard: 'Tarjeta',
        installmentsLabel: 'Cuotas',
        installmentsOption: '{n}x',
        feePayerLabel: 'Quién paga la tarifa',
        feePayerCompany: 'La empresa la absorbe',
        feePayerCustomer: 'Trasladar al cliente',
        anticipateLabel: 'Anticipar el cobro',
        anticipateDesc: 'Recibe todo en 1 día hábil pagando la tarifa de anticipo de Asaas.',
        anticipateUnavailable: 'Asaas no informó la tarifa de anticipo de tu cuenta. Usamos la tarifa de referencia.',
        resultTitle: 'Resultado de la simulación',
        resultGross: 'Valor bruto',
        resultCustomerPays: 'El cliente paga',
        resultFee: 'Tarifa de Asaas',
        resultNet: 'Valor neto en tu cuenta',
        resultInstallment: 'Cada cuota para el cliente',
        resultAnticipationCost: 'Costo del anticipo',
        resultNetAnticipated: 'Neto anticipado',
        resultEffectiveCost: 'Costo total de la venta',
        compareTitle: 'Con y sin anticipo',
        compareWithout: 'Sin anticipar',
        compareWith: 'Anticipando',
        compareWhen: 'cuándo llega',
        scheduleTitle: 'Cuándo llega el dinero a tu cuenta',
        scheduleInstallment: 'Cuota {n}',
        scheduleSingle: 'Pago',
        scheduleTotal: 'Total',
        settlementSameDay: 'al instante',
        settlementInDay: 'en 1 día hábil ({date})',
        settlementInDays: 'en {count} días ({date})',
        settlementBusinessDayNote: 'Los fines de semana y feriados no cuentan: si la fecha cae en sábado, domingo o feriado, el dinero llega el próximo día hábil.',
        settlementFallbackNote: 'Plazo de referencia, Asaas no informa este plazo por la API.',
        referenceWarning: 'Algunos valores vienen de la tabla de referencia de Asaas, no de tu cuenta.',
        pixFreeNote: 'Los primeros {count} cobros por Pix del mes no pagan tarifa, así que en esos casos el neto es el valor completo.',
        disclaimer: 'Estimación. El valor final lo calcula Asaas al momento del cobro.',
        emptyAmount: 'Escribe el valor de la venta para ver la simulación.',
      },
      asaas: {
        loading: 'Cargando…',
        notContracted: {
          title: 'Cobros no contratado',
          description: 'Este recurso no está incluido en tu plan actual. Habla con nuestro equipo para cobrar a tus clientes en línea por Pix, boleto y tarjeta.',
        },
        card: {
          title: 'Cobros (Asaas)',
          description: 'Cobra a tus clientes por Pix, boleto y tarjeta usando tu propia cuenta de Asaas.',
        },
        status: {
          active: 'Activo',
          pending: 'En verificación',
          disabled: 'Desactivado',
          rejected: 'Rechazado',
        },
        inactive: {
          intro: 'Pega la clave de API de tu cuenta Asaas para activar los cobros. Generas esa clave en el panel de Asaas, en Configuración, Integraciones.',
          apiKeyLabel: 'Clave de API de Asaas',
          apiKeyPlaceholder: 'Pega aquí tu clave de API',
          activate: 'Activar cobros',
          activating: 'Activando…',
          helpLink: '¿Dónde encuentro mi clave?',
          apiKeyGuide: {
            guideTitle: 'Cómo obtener tu clave de API de Asaas',
            trigger: '¿Cómo obtengo mi clave de API de Asaas?',
            step1: 'Ingresa a asaas.com e inicia sesión (o crea tu cuenta gratuita).',
            step2: 'En el menú, ve a "Configuración de cuenta" (ícono de engranaje), luego a "Integraciones".',
            step3: 'En la sección "Clave de API", haz clic en "Generar clave de API" o copia la clave existente.',
            step4: 'Copia la clave (empieza con "$aact_") y pégala en el campo de arriba. La clave es secreta, guárdala con cuidado.',
            sandboxHint: 'Para probar sin dinero real, usa la cuenta sandbox en sandbox.asaas.com siguiendo los mismos pasos.',
          },
        },
        activeState: {
          connected: 'Cobros activos. Ya puedes cobrar a tus clientes.',
          pendingDesc: 'Tu cuenta está en verificación en Asaas. Una vez aprobada, los cobros se activan.',
          feesTitle: 'Tarifas de Asaas',
          feesNote: 'Las tarifas las cobra Asaas directamente en tu cuenta. Dominex no retiene nada por transacción.',
          feePix: 'Pix',
          feePixValue: 'R$ 0,99 a R$ 1,99 por cobro recibido',
          feeBoleto: 'Boleto',
          feeBoletoValue: 'R$ 0,99 a R$ 1,99',
          feeCard: 'Tarjeta de crédito',
          feeCardValue: '1,99% a 4,29% + R$ 0,49 por cobro',
          feeAnticipation: 'Anticipo',
          feeSettlement: 'Plazo para que el dinero llegue',
          feeNotInformed: 'No informado por Asaas',
          feeReferenceTag: 'referencia',
          feeAnticipationValue: '1,25% a 1,70% al mes (tarjeta), 5,79% al mes (boleto y Pix)',
          feeSettlementValue: 'Pix al instante, boleto en 1 día, tarjeta en 32 días',
          feesFromAccount: 'Tarifas de tu cuenta Asaas, actualizadas el',
          feesFromAccountNoDate: 'Tarifas leídas de tu cuenta Asaas.',
          feesFromOverride: 'Tarifas ajustadas manualmente para tu empresa.',
          feesFromReference: 'Tarifas de referencia, no pudimos leer las de tu cuenta ahora.',
          feesRefresh: 'Actualizar tarifas',
          feesRefreshing: 'Actualizando…',
          feesRefreshOk: 'Tarifas actualizadas con Asaas.',
          feesRefreshError: 'No pudimos leer las tarifas de tu cuenta Asaas ahora.',
          feePixFreeNote: 'Los primeros {count} cobros recibidos del mes no pagan tarifa.',
          feePerCharge: 'por cobro',
          feeOneShot: 'al contado',
          feeInstallments: 'en cuotas',
          feeMonthly: 'al mes',
          settlementSameDay: 'al instante',
          settlementInDay: 'en 1 día',
          settlementInDays: 'en {count} días',
          financeAccountRequiredError: 'Elige la cuenta que va a recibir el dinero, sin eso el ingreso no entra en tus Finanzas.',
          financeAccountPlaceholder: 'Elige la cuenta…',
          financeAccountPendingTitle: 'Falta elegir la cuenta que recibe el dinero',
          financeAccountPendingDesc: 'Mientras no la elijas, los cobros pagados no entran solos en tus Finanzas.',
          financeAccountPendingCta: 'Elegir ahora',
          financeAccountEmptyTitle: 'Todavía no tienes una cuenta bancaria registrada',
          financeAccountEmptyDesc: 'Registra la cuenta en Finanzas, pestaña Movimientos, para elegir dónde cae el ingreso.',
          financeAccountEmptyCta: 'Abrir Finanzas',
          dueDaysError: 'Indica en cuántos días vence el cobro (0 o más).',
          maxInstallmentsError: 'Indica el número máximo de cuotas (1 o más).',
          discountPercentError: 'El descuento tiene que ser un número entre 0 y 100.',
          discountDaysError: 'Indica cuántos días antes del vencimiento vale el descuento (0 o más).',
          deactivate: 'Desactivar cobros',
          deactivating: 'Desactivando…',
          autoPostLabel: 'Registrar cobros en Finanzas automáticamente',
          autoPostDesc: 'Cada cobro genera una cuenta por cobrar que se liquida sola cuando el cliente paga.',
          autoPostToastOn: 'Registro automático activado.',
          autoPostToastOff: 'Registro automático desactivado.',
          autoPostToastError: 'No se pudo guardar la configuración. Inténtalo de nuevo.',
          autoFeeLabel: 'Registrar la tarifa de Asaas como gasto',
          autoFeeDesc: 'Al recibir un pago, la tarifa cobrada por Asaas se registra como gasto en tus Finanzas para que el saldo cuadre.',
          autoFeeToastOn: 'Registro de tarifa activado.',
          autoFeeToastOff: 'Registro de tarifa desactivado.',
          autoFeeError: 'No se pudo guardar la configuración. Inténtalo de nuevo.',
          lateFeesTitle: 'Mora e intereses por retraso',
          lateFeesDesc: 'Se aplica a los próximos cobros cuando el cliente paga tarde.',
          fineLabel: 'Mora por retraso (%)',
          interestLabel: 'Interés mensual (%)',
          interestCapWarning: 'El límite máximo de Asaas es el 10% mensual.',
          lateFeesSave: 'Guardar',
          lateFeesSaving: 'Guardando…',
          lateFeesToastOk: 'Mora e intereses actualizados.',
          lateFeesToastError: 'No se pudo guardar. Inténtalo de nuevo.',
          // ── Preferencias de cobro ─────────────────────────────────────────
          save: 'Guardar',
          saving: 'Guardando…',
          chargePrefsTitle: 'Preferencias de cobro',
          chargePrefsDesc: 'Configuración predeterminada aplicada en los nuevos cobros.',
          dueDaysLabel: 'Vencimiento predeterminado',
          dueDaysSuffix: 'días',
          defaultDescriptionLabel: 'Descripción/instrucción predeterminada',
          defaultDescriptionPlaceholder: 'Ej.: Relativo al servicio de mantenimiento…',
          paymentMethodsLabel: 'Métodos de pago habilitados',
          methodPix: 'Pix',
          methodBoleto: 'Boleto',
          methodCard: 'Tarjeta de crédito',
          maxInstallmentsLabel: 'Máx. cuotas (tarjeta)',
          maxInstallmentsSuffix: 'x',
          chargePrefsToastOk: 'Preferencias de cobro guardadas.',
          // ── Descuento por anticipo ────────────────────────────────────────
          discountTitle: 'Descuento por anticipo',
          discountDesc: 'Ofrece descuento a los clientes que paguen antes del vencimiento.',
          discountPercentLabel: 'Descuento (%)',
          discountPercentPlaceholder: 'Ej.: 5',
          discountDaysLabel: 'Hasta X días antes',
          discountDaysPlaceholder: 'Ej.: 10',
          discountDaysSuffix: 'días',
          discountHint: 'Deja en blanco para no aplicar descuento.',
          discountToastOk: 'Descuento por anticipo guardado.',
          // ── Registro en Finanzas ──────────────────────────────────────────
          financeConfigTitle: 'Registro en Finanzas',
          financeConfigDesc: 'Define la cuenta y las categorías usadas al liquidar automáticamente los cobros pagados.',
          financeAccountLabel: 'Cuenta bancaria de destino',
          financeAccountNone: 'Ninguna',
          incomeCategoryLabel: 'Categoría de ingreso',
          incomeCategoryNone: 'Ninguna',
          feeCategoryLabel: 'Categoría de tarifa (Asaas)',
          feeCategoryHint: 'Categoría usada para registrar la tarifa de Asaas como gasto.',
          financeConfigToastOk: 'Configuración de registro guardada.',
        },
        deactivateConfirm: {
          title: '¿Desactivar los cobros?',
          description: 'Los cobros ya generados siguen siendo válidos, pero no podrás crear nuevos hasta que vuelvas a activar.',
          confirm: 'Sí, desactivar',
          cancel: 'Cancelar',
        },
        errors: {
          activateGeneric: 'No pudimos activar los cobros. Verifica la clave e inténtalo de nuevo.',
          deactivateGeneric: 'No pudimos desactivar los cobros.',
        },
        toasts: {
          activated: '¡Cobros activados!',
          deactivated: 'Cobros desactivados.',
        },
      },
      whatsapp: {
        loading: 'Cargando…',
        comingSoon: {
          badge: 'Próximamente',
          title: 'Avisos por WhatsApp',
          description: 'Pronto podrás avisar a tus clientes automáticamente por WhatsApp. Estamos terminando los últimos ajustes.',
        },
        notContracted: {
          title: 'Avisos de WhatsApp no contratado',
          description: 'Esta función no está incluida en tu plan actual. Habla con nuestro equipo para activar los avisos automáticos por WhatsApp a tus clientes.',
        },
        masterToggle: {
          title: 'Avisos por WhatsApp',
          description: 'Activar o desactivar todos los avisos enviados al cliente por WhatsApp',
          ariaLabel: 'Activar avisos por WhatsApp',
        },
        connection: {
          sectionTitle: 'Conexión',
          sectionDescription: 'Conecta el WhatsApp de tu empresa para enviar avisos a los clientes',
          statusConnected: 'Conectado',
          statusQr: 'Esperando QR',
          statusDisconnected: 'Desconectado',
          statusBanned: 'Bloqueado',
          btnConnect: 'Conectar WhatsApp',
          connecting: 'Conectando…',
          qrInstructions: 'Abre WhatsApp en tu celular, ve a Menú > Dispositivos vinculados > Vincular dispositivo y escanea el código QR.',
          connectedDesc: 'WhatsApp conectado y listo para enviar avisos.',
          bannedDesc: 'Este número fue bloqueado por WhatsApp. Contacta al soporte.',
          errorTitle: 'No se pudo conectar',
          errorHint: 'Verifica tu conexión o inténtalo de nuevo en unos instantes. El servidor de integración puede estar configurándose.',
        },
        triggers: {
          sectionTitle: 'Disparadores de aviso',
          sectionDescription: 'Elige en qué momentos de la OT el cliente recibe un aviso automático',
          aCaminho: {
            label: 'Técnico en camino',
            description: 'Avisa al cliente cuando el técnico sale para atender la OT',
          },
          emAndamento: {
            label: 'Servicio iniciado',
            description: 'Avisa al cliente cuando el técnico comienza el servicio',
          },
          concluida: {
            label: 'OT finalizada',
            description: 'Avisa al cliente cuando la orden de trabajo es completada',
          },
        },
        quota: {
          sectionTitle: 'Plan y cuota',
          sectionDescription: 'Tu nivel actual y el consumo de avisos en el mes corriente',
          currentPlan: 'Plan actual',
          usageLabel: 'Avisos enviados este mes',
          unlimited: 'Ilimitado',
          nearLimitWarning: 'Estás cerca del límite mensual.',
          resetHint: 'La cuota se renueva automáticamente al inicio de cada mes.',
        },
      },
    },

    help: {
      title: 'Centro de Ayuda',
      intro: 'Preguntas frecuentes sobre el sistema. Si no encuentras tu respuesta, contacta al Soporte por WhatsApp.',
      faqs: [
        {
          q: '¿Cómo creo una Orden de Trabajo?',
          a: 'Ve a "Órdenes de Trabajo" en el menú lateral, haz clic en "+ Nueva OT", completa los datos del cliente, tipo de servicio y técnico asignado, luego guarda.',
        },
        {
          q: '¿Cómo registro un nuevo cliente?',
          a: 'Ve a "Clientes" en el menú, haz clic en "Nuevo Cliente" y completa los datos. Puedes registrar persona física o jurídica.',
        },
        {
          q: '¿Cómo funciona el control financiero?',
          a: 'En el menú "Finanzas" tienes resumen, movimientos, cuentas a pagar/cobrar y estado de resultados. Las OT pueden generar asientos financieros automáticamente.',
        },
        {
          q: '¿Cómo funcionan los pagos de empleados?',
          a: 'En "Empleados", haz clic en el ícono de pago en la tarjeta del empleado. El sistema calcula automáticamente salario + bonos - anticipos - faltas y permite seleccionar la cuenta de débito.',
        },
        {
          q: '¿Cómo configuro el control de asistencia?',
          a: 'Ve a Empleados > Control de Asistencia. Define horarios estándar, tolerancia de retraso y requisitos de selfie/geolocalización en la configuración.',
        },
        {
          q: '¿Cómo creo contratos recurrentes?',
          a: 'En "Contratos", crea un nuevo contrato con cliente, frecuencia (mensual, bimestral, etc.) y período. El sistema genera automáticamente las ocurrencias programadas.',
        },
        {
          q: '¿Qué es el CRM y cómo se usa?',
          a: 'El CRM es tu embudo de ventas. Crea leads, muévelos entre etapas (Nuevo, Contacto, Propuesta, Ganado/Perdido) y sigue el valor del pipeline en tiempo real.',
        },
        {
          q: '¿Cómo funciona el módulo de Inventario?',
          a: 'En "Inventario", registra artículos con precio de costo/venta, cantidad mínima y unidad. El sistema alerta cuando el stock cae por debajo del mínimo.',
        },
        {
          q: '¿Cómo exporto informes?',
          a: 'Varios módulos tienen botones de exportación (PDF, HTML). En extractos de empleados, presupuestos y estado de resultados hay opciones de exportación e impresión.',
        },
        {
          q: '¿Cómo cambio mi contraseña?',
          a: 'Ve a "Perfil" desde el menú de usuario (parte inferior del menú lateral) y haz clic en "Cambiar Contraseña". Recibirás un correo para restablecerla.',
        },
      ],
    },
  },

  // ═══════════════════════════════════════════════════════════════════════════
  // FRANÇAIS
  // ═══════════════════════════════════════════════════════════════════════════
  fr: {
    page: {
      title: 'Paramètres',
      subtitle: 'Gérez les paramètres du système',
      tabs: {
        empresa: 'Entreprise',
        regional: 'Régional',
        usuarios: 'Utilisateurs et permissions',
        usabilidade: 'Ergonomie',
        atalhos: 'Raccourcis',
        aparencia: 'Apparence',
        integracoes: 'Intégrations',
      },
    },

    saveStatus: {
      saving: 'Enregistrement…',
      unsaved: 'Modifications non enregistrées',
      saved: 'Enregistré',
    },

    company: {
      cardTitle: "Informations de l'entreprise",
      cardDescription: "Informations de l'entreprise affichées sur les étiquettes et documents",

      visualIdentity: {
        sectionTitle: 'Identité visuelle',
        sectionDescription: "Logo de l'entreprise pour les documents et le système",
        replace: 'Remplacer',
        remove: 'Supprimer',
        removeLogoTitle: 'Supprimer le logo ?',
        removeLogoDesc: 'Le logo actuel sera supprimé définitivement.',
        uploadPrompt: 'Cliquez pour télécharger le logo',
        uploadHint: 'PNG, JPG jusqu’à 5Mo',
        errorFileSize: 'Fichier trop volumineux (max 5Mo)',
        errorFileType: 'Seules les images sont autorisées',
        errorUpload: 'Erreur lors du téléchargement du logo',
        errorIconUpload: "Erreur lors du téléchargement de l'icône",
      },

      registrationData: {
        sectionTitle: "Données d'enregistrement",
        sectionDescription: 'Raison sociale, numéro de document et informations officielles',
        companyName: "Nom de l'entreprise",
        companyNamePlaceholder: 'Nom de votre entreprise',
        cnpj: 'Numéro fiscal',
        cnpjPlaceholder: '00.000.000/0000-00',
        showInDocs: 'Afficher dans les documents',
        segment: "Secteur d'activité",
        segmentNotDefined: 'Non défini',
        segmentHint: "Définit quels outils et fonctions apparaissent pour votre secteur. Pour le modifier, contactez Dominex.",
      },

      contact: {
        sectionTitle: 'Contact',
        sectionDescription: "Téléphone et e-mail de l'entreprise",
        phone: 'Téléphone',
        phonePlaceholder: '(00) 0000-0000',
        email: 'E-mail',
        emailPlaceholder: 'contact@entreprise.com',
        showInDocs: 'Afficher dans les documents',
      },

      address: {
        sectionTitle: 'Adresse',
        sectionDescription: "Localisation physique de l'entreprise",
        showInDocs: 'Afficher dans les documents',
        zip: 'Code postal',
        street: 'Adresse',
        streetPlaceholder: 'Rue, Avenue…',
        number: 'Numéro',
        numberPlaceholder: 'N°',
        complement: 'Complément',
        complementPlaceholder: 'Étage, Bureau…',
        neighborhood: 'Quartier',
        neighborhoodPlaceholder: 'Quartier',
        stateCity: 'Région / Ville',
      },

      whiteLabelSection: {
        sectionTitle: 'White Label',
        sectionDescription: "Personnalisez le système avec l'identité de votre marque",
        moduleUnavailable: "Module non disponible dans votre plan actuel",
        hire: 'Souscrire',
        enableLabel: 'Activer le White Label',
        enableDescription: 'Remplace le logo et la couleur par défaut du système',
        fullLogo: 'Logo complet',
        fullLogoConfigured: 'Logo personnalisé configuré',
        fullLogoUsingCompany: "Logo de l'entreprise utilisé par défaut",
        fullLogoFallback: "Le logo de l'entreprise sera utilisé",
        uploadLogoPrompt: 'Télécharger le logo (facultatif)',
        icon: 'Icône (1:1)',
        iconDescription: 'Icône carrée pour le menu latéral réduit. 128×128px.',
        uploadIconPrompt: "Télécharger l'icône",
        primaryColor: 'Couleur principale',
        primaryColorDescription: 'Remplace la couleur verte par défaut du système',
        qrSection: {
          title: 'Code QR',
          description: 'Personnalisez l`apparence des codes QR générés par le système, avec le logo de votre marque au centre',
          dotStyleLabel: 'Style des points',
          dotStyle: {
            square: 'Carré',
            rounded: 'Arrondi',
            dots: 'Points',
            classy: 'Élégant',
          },
          cornerStyleLabel: 'Coins',
          cornerStyle: {
            square: 'Carré',
            rounded: 'Arrondi',
            dot: 'Point',
          },
          colorLabel: 'Couleur du code QR',
          previewCaption: 'Aperçu',
        },
        reportHeader: 'En-tête du rapport de service',
        reportHeaderDescription: "Personnalisez l'en-tête affiché dans le rapport du bon de travail terminé",
        reportBgColor: "Couleur d'arrière-plan de l'en-tête",
        reportTextColor: 'Couleur du texte',
        reportStatusBarColor: 'Couleur de la barre de statut',
        reportLogoSize: 'Taille du logo ({size}px)',
        reportLogoBg: "Fond derrière le logo",
        reportLogoBgDescription: "Supprime le fond quand désactivé",
        reportLogoBgColor: "Couleur du fond",
        reportLogoType: 'Type de logo dans le rapport',
        reportLogoTypeFull: 'Logo complet',
        reportLogoTypeIcon: 'Icône',
        reportLogoTypeHintFull: "Utilise le logo complet de l'entreprise",
        reportLogoTypeHintIcon: 'Utilise l’icône configurée dans le White Label',
      },

      legalDocs: {
        sectionTitle: 'Documents légaux',
        sectionDescription: "Consultez et téléchargez les conditions régissant l'utilisation de Dominex",
        viewTerms: "Voir les conditions d'utilisation",
        acceptedAt: 'Accepté le {date}',
      },
    },

    usability: {
      cardTitle: 'Ergonomie',
      cardDescription: 'Préférences de comportement du système',
      preferenceSaved: 'Préférence enregistrée !',

      sections: {
        os: {
          title: 'Bons de travail',
          description: 'Comportements liés aux bons de travail',
          autoSaveOS: {
            title: 'Sauvegarde automatique',
            description: 'Enregistrer automatiquement les brouillons de bons de travail lors de l’édition',
          },
          showOSValues: {
            title: 'Afficher les montants',
            description: 'Afficher les valeurs financières (main-d’œuvre, pièces) sur les bons de travail',
          },
          requireSignature: {
            title: 'Exiger une signature',
            description: 'Rendre obligatoire la signature du client à la clôture d’un bon de travail',
          },
          saveOSPhotosToDevice: {
            title: "Enregistrer les photos sur l'appareil",
            description: "Affiche un bouton pour enregistrer la photo sur votre appareil. Sur iPhone, ouvre \"Enregistrer l'image\"; sur Android, télécharge directement.",
          },
          osStockConsumptionEnabled: {
            title: 'Consommation de stock sur le bon de travail',
            description: "Permet au technicien d'enregistrer les matériaux utilisés pendant le bon de travail et de les déduire du stock à la clôture",
          },
        },
        interface: {
          title: 'Interface',
          description: 'Préférences visuelles pour les listes et tableaux',
          compactTables: {
            title: 'Tableaux compacts',
            description: 'Réduire l’espacement des tableaux pour afficher plus de données par page',
          },
          showEquipmentPhotos: {
            title: 'Photos des équipements',
            description: 'Afficher des miniatures des photos d’équipements dans les listes',
          },
        },
        security: {
          title: 'Sécurité',
          description: 'Confirmations et validations de sécurité',
          confirmDelete: {
            title: 'Confirmer les suppressions',
            description: 'Afficher une boîte de dialogue de confirmation avant de supprimer des enregistrements',
          },
        },
        schedule: {
          title: 'Agenda',
          description: "Paramètres de l'agenda et du calendrier",
          showHolidays: {
            title: 'Afficher les jours fériés',
            description: 'Afficher les jours fériés nationaux et locaux dans l’agenda',
          },
        },
      },

      origins: {
        cardTitle: 'Sources',
        cardDescription: 'Liste de sources utilisée dans l’inscription des clients et les opportunités CRM',
        manage: 'Gérer les sources',
        empty: 'Aucune source enregistrée. Créez un ensemble initial et modifiez-le librement ensuite.',
        seedButton: 'Créer les sources par défaut',
        seedLoading: 'Création…',
        inactive: 'Inactif',
      },
    },

    regional: {
      personalCard: {
        title: 'Ma langue',
        description: 'La langue que VOUS voyez dans le système. S’applique immédiatement, pour vous seulement.',
        languageLabel: 'Langue',
        languageHint: 'Préférence personnelle. Remplace la langue par défaut de l’entreprise uniquement pour vous.',
      },
      companyCard: {
        title: "Paramètres par défaut de l'entreprise",
        description: "Langue, devise et fuseau horaire par défaut pour toute l'entreprise",
        adminOnly: '(admin uniquement)',
        languageLabel: "Langue par défaut de l'entreprise",
        languageHint: "Langue appliquée à tous les utilisateurs n'ayant pas choisi la leur.",
        localeDefaultsPrompt: 'Appliquer les paramètres par défaut de {locale} pour la devise et le fuseau horaire ?',
        keepCurrent: 'Conserver actuels',
        useDefaults: 'Utiliser les défauts',
        localeDefaultsApplied: 'Paramètres de {locale} appliqués',
        currencyLabel: 'Devise',
        currencyHint: "Devise d'exploitation utilisée pour les valeurs du système.",
        currencyChangedWarning: "La devise est la devise d'exploitation de l'entreprise. La changer ne convertit pas les valeurs déjà enregistrées, elles conservent leur nombre d'origine.",
        timezoneLabel: 'Fuseau horaire',
        timezoneHint: 'Utilisé pour les dates et heures affichées dans le système.',
        timezonePlaceholder: 'Sélectionner le fuseau horaire',
        timezoneSearchPlaceholder: 'Rechercher un fuseau (ex: Paris)',
        timezoneEmpty: 'Aucun fuseau trouvé.',
        savingStatus: 'Enregistrement...',
        unsavedStatus: 'Modifications non enregistrées',
        savedStatus: 'Enregistré',
      },
      discCard: {
        title: 'Profil Comportemental',
        description: 'Comment l`évaluation comportementale (DISC) se comporte pour les salariés.',
        visibilityLabel: 'Le salarié voit son propre résultat de l`évaluation comportementale',
        visibilityHint:
          'Activé : à la fin, le salarié voit son propre profil. Désactivé : il voit seulement un écran de remerciement, et le résultat reste chez les RH.',
        on: 'Oui',
        off: 'Non',
      },
    },

    appearance: {
      cardTitle: 'Apparence',
      cardDescription: "Personnalisez l'interface visuelle du système",
      navigationStyle: {
        sectionTitle: 'Style de navigation (Bureau)',
        sectionDescription: 'Choisissez entre un menu latéral ou une barre de navigation supérieure. Cette option n’affecte que la vue bureau.',
        sidebar: 'Menu latéral',
        sidebarDescription: 'Barre latérale traditionnelle à gauche',
        topbar: 'Barre supérieure',
        topbarDescription: 'Barre horizontale en haut',
      },
      theme: {
        sectionTitle: 'Thème du système',
        sectionDescription: "Choisissez entre le thème clair ou sombre pour l'interface.",
        light: 'Thème clair',
        dark: 'Thème sombre',
      },
      navTheme: {
        sectionTitle: 'Menu (latéral et supérieur)',
        sectionDescription: 'Choisissez la couleur du menu de navigation, indépendamment du thème du système.',
        dark: 'Sombre (recommandé)',
        darkDescription: 'Menu toujours sombre, mis en valeur par la couleur de la marque',
        system: 'Suivre le thème',
        systemDescription: 'Le menu suit le thème clair ou sombre',
      },
    },

    shortcuts: {
      pageTitle: 'Raccourcis clavier',
      pageDescription: 'Utilisez des raccourcis pour accéder rapidement aux fonctionnalités',
      toggleLabel: 'Raccourcis clavier',
      toggleEnabled: 'Les raccourcis sont activés',
      toggleDisabled: 'Les raccourcis sont désactivés',
      groupNavigation: 'Navigation',
      groupGeneral: 'Général',
      tips: {
        title: 'Astuces',
        shiftNav: 'Utilisez {modifier} + touche pour la navigation rapide',
        noTyping: 'Les raccourcis ne fonctionnent pas lorsque vous saisissez dans des champs de texte',
        canDisable: "Vous pouvez désactiver les raccourcis à tout moment via le bouton ci-dessus",
      },
      goto_dashboard: { label: 'Tableau de bord', description: 'Aller au tableau de bord principal' },
      goto_os: { label: 'Bons de travail', description: 'Aller aux bons de travail' },
      goto_agenda: { label: 'Agenda', description: "Aller à l'agenda" },
      goto_clients: { label: 'Clients', description: 'Aller aux clients' },
      goto_equipment: { label: 'Équipements', description: 'Aller aux équipements' },
      goto_crm: { label: 'CRM', description: 'Aller au CRM' },
      goto_finance: { label: 'Finances', description: 'Aller aux finances' },
      goto_inventory: { label: 'Stock', description: 'Aller au stock' },
      goto_quotes: { label: 'Devis', description: 'Aller aux devis' },
      goto_contracts: { label: 'Contrats', description: 'Aller aux contrats' },
      goto_settings: { label: 'Paramètres', description: 'Ouvrir les paramètres' },
      goto_profile: { label: 'Profil', description: "Ouvrir le profil utilisateur" },
    },

    dangerZone: {
      title: 'Zone dangereuse',
      description: 'Actions irréversibles. Vérifiez attentivement avant de confirmer.',
      resetSystem: {
        label: 'Réinitialiser le système',
        description: "Supprime les données opérationnelles sélectionnées. Les données de l'entreprise et les utilisateurs sont conservés. Toute opération est enregistrée dans un journal d'audit.",
        button: 'Réinitialiser le système',
      },
    },

    dataExport: {
      cardTitle: 'Exporter mes données',
      cardDescription: `Téléchargez une feuille Excel avec les données de votre entreprise : clients, équipements, ordres de service, devis, contrats, finances et plus. Utile pour une sauvegarde ou pour emporter vos données ailleurs.`,
      button: 'Exporter les données',
      dialogTitle: 'Exporter mes données',
      dialogDescription: `Choisissez ce qui doit figurer dans la feuille. Le fichier est généré instantanément et téléchargé automatiquement.`,
      selectAll: 'Tout sélectionner',
      clearAll: 'Effacer la sélection',
      confirm: 'Exporter',
      cancel: 'Annuler',
      running: 'Exportation en cours...',
      progressLabel: 'Exportation de {sheet}... {rows} enregistrements',
      successToast: `Exportation terminée ! Le fichier a été téléchargé.`,
      errorToast: `Impossible d'exporter les données. Réessayez.`,
      emptySelection: `Sélectionnez au moins un élément à exporter.`,
      heavyHint: `Mouvements de Stock et Pointage sont décochés par défaut car ils contiennent souvent beaucoup d'enregistrements. Cochez-les si vous en avez aussi besoin.`,
      fileNamePrefix: 'backup',
      groups: {
        customers: 'Clients',
        equipment: 'Équipements',
        serviceOrders: 'Ordres de Service',
        quotes: 'Devis',
        contracts: 'Contrats',
        financial: 'Finances',
        inventory: 'Stock',
        inventoryMovements: 'Mouvements de Stock',
        employees: 'Employés',
        leads: 'CRM / Prospects',
        timeRecords: 'Pointage',
      },
      sheets: {
        summary: 'Résumé',
        customers: 'Clients',
        customerContacts: 'Contacts Clients',
        equipment: 'Équipements',
        serviceOrders: 'Ordres de Service',
        serviceMaterials: 'Matériaux des OS',
        quotes: 'Devis',
        quoteItems: 'Articles des Devis',
        contracts: 'Contrats',
        contractItems: 'Articles des Contrats',
        financialTransactions: 'Finances',
        financialAccounts: 'Comptes Bancaires',
        inventory: 'Stock',
        inventoryMovements: 'Mouvements de Stock',
        employees: 'Employés',
        leads: 'CRM Prospects',
        timeRecords: 'Pointage',
      },
      summary: {
        companyLabel: 'Entreprise',
        generatedAtLabel: 'Généré le',
        sheetColumn: 'Feuille',
        rowCountColumn: 'Enregistrements',
        totalLabel: 'Total',
      },
      columns: {
        summary: {
          company: 'Entreprise',
          generatedAt: 'Généré le',
          sheet: 'Feuille',
          rows: 'Enregistrements',
        },
        customers: {
          name: 'Nom',
          nome_fantasia: 'Nom Commercial',
          company_name: 'Raison Sociale',
          customer_type: 'Type de Client',
          document: 'CPF/CNPJ',
          inscricao_estadual: `Immatriculation d'État`,
          inscricao_municipal: 'Immatriculation Municipale',
          email: 'E-mail',
          phone: 'Téléphone',
          celular: 'Mobile',
          address: 'Adresse',
          address_number: 'Numéro',
          complement: 'Complément',
          neighborhood: 'Quartier',
          city: 'Ville',
          state: 'État',
          zip_code: 'Code Postal',
          origin: 'Origine',
          birth_date: 'Date de Naissance',
          notes: 'Notes',
          created_at: 'Créé le',
        },
        customerContacts: {
          customer: 'Client',
          name: 'Nom',
          position: 'Fonction',
          phone: 'Téléphone',
          email: 'E-mail',
          notes: 'Notes',
          created_at: 'Créé le',
        },
        equipment: {
          customer: 'Client',
          name: 'Nom',
          category: 'Catégorie',
          brand: 'Marque',
          model: 'Modèle',
          serial_number: 'Numéro de Série',
          identifier: 'Identifiant',
          capacity: 'Capacité',
          location: 'Emplacement',
          status: 'Statut',
          install_date: `Date d'Installation`,
          warranty_until: `Garantie jusqu'à`,
          notes: 'Notes',
          created_at: 'Créé le',
        },
        serviceOrders: {
          order_number: `Numéro d'Ordre`,
          status: 'Statut',
          os_type: `Type d'Ordre`,
          service_type: 'Type de Service',
          entry_type: `Type d'Entrée`,
          origin: 'Origine',
          customer: 'Client',
          equipment: 'Équipement',
          technician: 'Technicien',
          team: 'Équipe',
          contract: 'Contrat',
          task_title: 'Titre de la Tâche',
          description: 'Description',
          diagnosis: 'Diagnostic',
          solution: 'Solution',
          notes: 'Notes',
          scheduled_date: 'Date Prévue',
          scheduled_time: 'Heure Prévue',
          started_at: 'Débuté le',
          completed_at: 'Terminé le',
          duration_minutes: 'Durée (minutes)',
          labor_hours: 'Heures de Travail',
          labor_value: `Valeur de la Main-d'Œuvre`,
          parts_value: 'Valeur des Pièces',
          total_value: 'Valeur Totale',
          pmoc_conformity_status: 'Conformité PMOC',
          service_address: 'Adresse du Service',
          service_address_number: `Numéro de l'Adresse`,
          service_neighborhood: 'Quartier du Service',
          service_city: 'Ville du Service',
          service_state: 'État du Service',
          service_zip_code: 'Code Postal du Service',
          created_at: 'Créé le',
        },
        serviceMaterials: {
          order_number: `Numéro d'Ordre`,
          item_name: 'Matériau',
          quantity: 'Quantité',
          unit: 'Unité',
          purchase_price: `Prix d'Achat`,
          sale_price: 'Prix de Vente',
          subtotal: 'Sous-total',
          created_at: 'Créé le',
        },
        quotes: {
          quote_number: 'Numéro de Devis',
          status: 'Statut',
          customer: 'Client',
          prospect_name: 'Nom du Prospect',
          prospect_email: `E-mail du Prospect`,
          prospect_phone: 'Téléphone du Prospect',
          subtotal: 'Sous-total',
          discount_type: 'Type de Remise',
          discount_value: 'Valeur de la Remise',
          discount_amount: 'Remise Appliquée',
          displacement_cost: 'Frais de Déplacement',
          distance_km: 'Distance (km)',
          km_cost: 'Coût par Km',
          total_cost: 'Coût Total',
          total_price: 'Prix Total',
          total_value: 'Valeur Totale',
          bdi: 'Frais Généraux (%)',
          profit_rate: 'Marge Bénéficiaire (%)',
          tax_rate: 'Taux de Taxe (%)',
          admin_indirect_rate: 'Frais Administratifs (%)',
          card_installments: 'Versements par Carte',
          card_discount_rate: 'Remise Carte (%)',
          valid_until: `Valable Jusqu'à`,
          view_count: 'Nombre de Vues',
          last_viewed_at: 'Dernière Consultation',
          terms: 'Conditions',
          notes: 'Notes',
          created_at: 'Créé le',
        },
        quoteItems: {
          quote_number: 'Numéro de Devis',
          position: 'Position',
          item_type: `Type d'Article`,
          description: 'Description',
          details: 'Détails',
          service_type: 'Type de Service',
          quantity: 'Quantité',
          unit_price: 'Prix Unitaire',
          price_override: 'Prix Ajusté',
          total_price: 'Prix Total',
          unit_total_cost: 'Coût Unitaire Total',
          unit_labor_cost: `Coût Unitaire de Main-d'Œuvre`,
          unit_materials_cost: 'Coût Unitaire de Matériaux',
          unit_extras_cost: 'Coût Unitaire des Extras',
          unit_hours: 'Heures par Unité',
          unit_hourly_rate: 'Taux Horaire',
          bdi: 'Frais Généraux (%)',
          profit_rate: 'Marge Bénéficiaire (%)',
        },
        contracts: {
          name: 'Nom',
          customer: 'Client',
          status: 'Statut',
          is_pmoc: 'Est PMOC',
          frequency_type: 'Type de Fréquence',
          frequency_value: 'Valeur de Fréquence',
          start_date: 'Date de Début',
          horizon_months: 'Horizon (mois)',
          next_pmoc_generation_date: 'Prochaine Génération PMOC',
          service_type: 'Type de Service',
          technician: 'Technicien',
          team: 'Équipe',
          responsible_technician: 'Technicien Responsable',
          unidade_nome: 'Nom du Site',
          unidade_endereco: 'Adresse du Site',
          unidade_numero: 'Numéro du Site',
          unidade_complemento: 'Complément du Site',
          unidade_bairro: 'Quartier du Site',
          unidade_cidade: 'Ville du Site',
          unidade_uf: 'État du Site',
          unidade_cep: 'Code Postal du Site',
          pmoc_identificacao_ambiente: `Identification de l'Environnement (PMOC)`,
          pmoc_tipo_atividade: `Type d'Activité (PMOC)`,
          pmoc_area_climatizada_m2: 'Surface Climatisée (m²)',
          pmoc_carga_termica_tr: 'Charge Thermique (TR)',
          pmoc_ocupantes_fixos: 'Occupants Permanents',
          pmoc_ocupantes_flutuantes: 'Occupants Flottants',
          notes: 'Notes',
          created_at: 'Créé le',
        },
        contractItems: {
          contract: 'Contrat',
          item_name: `Nom de l'Article`,
          item_description: `Description de l'Article`,
          equipment: 'Équipement',
          pmoc_scope: 'Portée PMOC',
          pmoc_start_visit: 'Première Visite PMOC',
          sort_order: 'Ordre',
          created_at: 'Créé le',
        },
        financialTransactions: {
          transaction_date: 'Date de la Transaction',
          due_date: 'Échéance',
          paid_date: 'Date de Paiement',
          description: 'Description',
          transaction_type: 'Type',
          category: 'Catégorie',
          amount: 'Montant',
          amount_received: 'Montant Reçu',
          is_paid: 'Payé',
          payment_method: 'Mode de Paiement',
          account: 'Compte',
          customer: 'Client',
          employee: 'Employé',
          contract: 'Contrat',
          order_number: `Numéro d'Ordre`,
          installment_number: 'Échéance N°',
          installment_total: `Total d'Échéances`,
          payroll_kind: 'Type de Paie',
          payroll_period: 'Période de Paie',
          credit_card_bill_date: 'Date de la Facture de Carte',
          cancelled_at: 'Annulé le',
          cancelled_reason: `Motif d'Annulation`,
          notes: 'Notes',
          created_at: 'Créé le',
        },
        financialAccounts: {
          name: 'Nom',
          type: 'Type',
          bank_name: 'Banque',
          institution_name: 'Institution',
          initial_balance: 'Solde Initial',
          credit_limit: 'Limite de Crédit',
          closing_day: 'Jour de Clôture',
          due_day: `Jour d'Échéance`,
          payment_due_days: 'Délai de Paiement (jours)',
          is_active: 'Actif',
          created_at: 'Créé le',
        },
        inventory: {
          name: 'Nom',
          sku: 'Code (SKU)',
          category: 'Catégorie',
          unit: 'Unité',
          stock: 'Emplacement de Stock',
          quantity: 'Quantité',
          min_quantity: 'Stock Minimum',
          cost_price: 'Prix de Revient',
          sale_price: 'Prix de Vente',
          total_value: 'Valeur Totale en Stock',
          supplier: 'Fournisseur',
          description: 'Description',
        },
        inventoryMovements: {
          created_at: 'Date',
          movement_type: 'Type de Mouvement',
          item: 'Matériau',
          sku: 'Code (SKU)',
          stock: 'Emplacement de Stock',
          quantity: 'Quantité',
          unit_cost: 'Coût Unitaire',
          stock_before: 'Solde Avant',
          stock_after: 'Solde Après',
          supplier: 'Fournisseur',
          order_number: `Numéro d'Ordre`,
          notes: 'Notes',
        },
        employees: {
          name: 'Nom',
          matricula: 'Matricule',
          cpf: 'CPF',
          position: 'Fonction',
          cbo: `Code d'Occupation`,
          employment_regime: `Régime d'Embauche`,
          hire_date: `Date d'Embauche`,
          is_active: 'Actif',
          email: 'E-mail',
          phone: 'Téléphone',
          address: 'Adresse',
          salary: 'Salaire',
          monthly_cost: 'Coût Mensuel',
          dependents_count: 'Nombre de Personnes à Charge',
          payment_frequency: 'Fréquence de Paiement',
          payment_day: 'Jour de Paiement',
          payment_day_2: '2e Jour de Paiement',
          payment_day_type: 'Type de Jour de Paiement',
          payment_weekday: 'Jour de la Semaine de Paiement',
          vt_enabled: 'Allocation Transport Active',
          vt_monthly_value: 'Valeur Mensuelle du Transport',
          pix_key: 'Clé Pix',
          created_at: 'Créé le',
        },
        leads: {
          title: 'Titre',
          status: 'Statut',
          stage: 'Étape',
          customer: 'Client',
          assigned_to: 'Responsable',
          source: 'Origine',
          value: 'Valeur',
          probability: 'Probabilité (%)',
          expected_close_date: 'Date de Clôture Prévue',
          notes: 'Notes',
          created_at: 'Créé le',
          updated_at: 'Mis à jour le',
        },
        timeRecords: {
          date: 'Date',
          recorded_at: 'Enregistré le',
          type: 'Type',
          employee: 'Employé',
          source: 'Origine',
          is_valid: 'Valide',
          address: 'Adresse',
          latitude: 'Latitude',
          longitude: 'Longitude',
          notes: 'Notes',
        },
      },
      values: {
        discountTypeValor: 'Montant',
        discountTypePercentual: 'Pourcentage',
        itemTypeServico: 'Service',
        itemTypeMaterial: 'Matériel',
        itemTypeMaoDeObra: `Main-d'œuvre`,
        originManual: 'Manuel',
        originContract: 'Généré par Contrat',
        originPortal: 'Portail Client',
        payrollKindRescission: 'Indemnité de Rupture',
        timeRecordSourceApp: 'Application',
        timeRecordSourceAdmin: 'Administrateur',
        timeRecordSourceLinkPublico: 'Lien Public',
      },
    },

    resetSystem: {
      modalTitle: 'Réinitialiser le système',
      irreversibleWarning: 'Cette action va',
      deletePermanently: 'SUPPRIMER DÉFINITIVEMENT',
      warningCompany: "les données opérationnelles de l'entreprise",
      chooseWhat: 'Choisissez ce que vous souhaitez supprimer :',
      selectAll: 'Tout sélectionner',
      selectedCount: '{count}/{total} sélectionnés',
      groupSelectAriaLabel: 'Tout sélectionner dans {group}',
      forcedByMaterials: 'Requis car le catalogue de matériaux a été sélectionné.',
      preserved: {
        title: 'Seront conservés :',
        companyData: "Données de base de l'entreprise",
        users: 'Utilisateurs enregistrés',
        paymentHistory: 'Historique des paiements',
      },
      finalWarning: 'Cette action NE PEUT PAS être annulée !',
      confirmLabel: "Pour confirmer, saisissez le nom de l'entreprise :",
      confirmPlaceholder: "Nom de l'entreprise",
      nameMismatch: "Le nom saisi ne correspond pas au nom de l'entreprise.",
      offline: 'Cette action nécessite une connexion Internet. Vous êtes hors ligne.',
      progressTitle: 'Suppression des données...',
      progressStep: 'Étape {current} sur {total} :',
      cancel: 'Annuler',
      confirmButton: 'Oui, réinitialiser le système',
      successTitle: 'Système réinitialisé avec succès',
      successDesc: "Les données sélectionnées ont été supprimées. Le journal d'audit a été généré.",
      errorTitle: "Impossible de réinitialiser le système",
      errorPermission: "Vous n'avez pas la permission de réinitialiser le système. Seul l'administrateur de l'entreprise peut le faire.",
      errorTimeout: "Une étape a pris trop de temps. Réessayez, les données déjà supprimées restent supprimées.",
      errorDependency: "Impossible de terminer l'étape en raison d'une dépendance inattendue. Prévenez le support avec le nom de l'étape.",
      errorDuplicate: "Impossible de recréer une configuration par défaut. Réessayez dans quelques secondes.",
      errorGeneric: "Impossible de réinitialiser le système. Réessayez ou contactez le support.",
      errorStepPrefix: 'Erreur à l’étape "{step}".',

      groups: {
        operacional: {
          label: 'Opérationnel',
          delete_service_orders: {
            title: 'Bons de travail',
            description: 'Supprime tous les bons de travail, photos, matériaux consommés, évaluations et formulaires remplis',
          },
          delete_equipment: {
            title: 'Équipements',
            description: 'Supprime tous les équipements enregistrés pour les clients',
          },
          delete_contracts: {
            title: 'Contrats et plans de maintenance',
            description: 'Supprime tous les contrats, plannings et documents de maintenance personnalisés',
          },
          delete_custom_configs: {
            title: 'Configurations personnalisées',
            description: 'Supprime les étapes CRM personnalisées, les formulaires de bons de travail et les ressources de coût',
          },
        },
        comercial: {
          label: 'Ventes (CRM)',
          delete_quotes: {
            title: 'Devis et propositions',
            description: 'Supprime tous les devis et propositions',
          },
          delete_customers: {
            title: 'Clients et leads',
            description: 'Supprime tous les dossiers clients, contacts, portails et leads',
          },
        },
        financeiro: {
          label: 'Finances',
          delete_financial_movements: {
            title: 'Transactions',
            description: 'Supprime toutes les transactions de caisse, banque, carte, recettes et dépenses',
          },
          delete_financial_categories: {
            title: 'Catégories financières',
            description: 'Supprime les catégories financières personnalisées (les catégories par défaut sont conservées)',
          },
        },
        rh_inventario: {
          label: 'RH et Stock',
          delete_employees: {
            title: 'Employés et équipes',
            description: 'Supprime tous les employés, avances, paiements, pointages et équipes',
          },
          delete_materials: {
            title: 'Matériaux',
            description: 'Supprime tous les matériaux enregistrés (et leur stock)',
          },
          delete_stock: {
            title: 'Stock',
            description: 'Vide le stock (mouvements), conserve les matériaux enregistrés',
          },
        },
      },

      stepLabels: {
        service_orders: 'Bons de travail',
        contracts: 'Contrats',
        quotes: 'Devis',
        equipment: 'Équipements',
        custom_configs: 'Configurations personnalisées',
        financial_movements: 'Transactions financières',
        financial_categories: 'Catégories financières',
        employees: 'Employés et RH',
        stock: 'Stock de matériaux',
        materials: 'Catalogue de matériaux',
        customers: 'Clients',
      },
    },

    users: {
      pageTitle: 'Utilisateurs et permissions',
      pageSubtitle: '{count}/{max} utilisateurs • {active} actifs',
      tabs: {
        users: 'Utilisateurs',
        presets: 'Rôles',
      },
      searchPlaceholderUsers: 'Rechercher des utilisateurs...',
      searchPlaceholderPresets: 'Rechercher des rôles...',
      searchPlaceholderDesktop: 'Rechercher par nom ou téléphone...',
      viewList: 'Liste',
      viewCards: 'Cartes',
      btnSettings: 'Paramètres',
      btnCreateUser: 'Créer un utilisateur',
      btnHireMore: 'Ajouter des utilisateurs',
      limitTitle: "Limite d’utilisateurs atteinte ({count}/{max})",
      limitDesc: 'Pour enregistrer de nouveaux utilisateurs, ajoutez des places supplémentaires à votre plan.',
      badgeYou: 'Vous',
      badgeActive: 'Actif',
      badgeInactive: 'Inactif',
      badgeAllPerms: 'Accès total',
      badgePermCount: '{count} permissions',
      fabUser: 'Utilisateur',
      fabPreset: 'Rôle',
      btnEdit: 'Modifier',
      btnDeactivate: 'Désactiver',
      btnReactivate: 'Réactiver',
      btnReactivateDisabledTitle: 'Aucune place disponible — améliorez votre plan ou désactivez un autre utilisateur',
      emptyFiltered: 'Aucun utilisateur trouvé',
      emptyAll: 'Aucun utilisateur enregistré',
      deleteUserTitle: "Supprimer l'utilisateur définitivement ?",
      deleteUserDesc: "Cette action est irréversible. L'utilisateur {name} sera supprimé définitivement du système, y compris son accès, ses permissions et ses liens.",
      deletePresetTitle: 'Supprimer le rôle ?',
      deletePresetDesc: 'Le rôle {name} sera supprimé. Les utilisateurs qui y étaient liés conservent leurs permissions individuelles. Cette action ne peut pas être annulée.',
      deleteLoading: 'Suppression...',
      deletePermanently: 'Supprimer définitivement',
      cancel: 'Annuler',
      delete: 'Supprimer',
      toastCreated: 'Utilisateur créé avec succès !',
      toastCreateError: 'Erreur lors de la création',
      toastCreateErrorDuplicate: 'Cet e-mail est déjà enregistré dans le système.',
      toastUpdated: 'Utilisateur mis à jour !',
      toastUpdateError: 'Erreur',
      toastDeactivated: 'Utilisateur désactivé',
      toastDeactivatedDesc: 'La place a été libérée. Vous pouvez le réactiver plus tard.',
      toastDeactivateError: 'Erreur lors de la désactivation',
      toastReactivated: 'Utilisateur réactivé !',
      toastReactivateError: 'Erreur lors de la réactivation',
      toastDeleted: 'Utilisateur supprimé définitivement !',
      toastDeleteError: 'Erreur lors de la suppression',
      toastPresetDuplicated: 'Rôle dupliqué !',
      toastPresetDuplicateError: 'Erreur lors de la duplication',
      toastPresetDeleted: 'Rôle supprimé !',
      toastPresetDeleteError: 'Erreur lors de la suppression',
      blockLastAdminTitle: 'Action bloquée',
      blockLastAdminDesc: "L'entreprise doit avoir au moins un administrateur actif.",
      btnDeleteTitle: "Supprimer l'utilisateur définitivement",

      form: {
        titleCreate: 'Créer un utilisateur',
        titleEdit: "Modifier l'utilisateur",
        descCreate: 'Remplissez les informations du nouvel utilisateur',
        descEdit: "Mettez à jour les informations et permissions de l'utilisateur",
        cancel: 'Annuler',
        save: 'Mettre à jour',
        create: "Créer l'utilisateur",
        photoLabel: "Photo de l'utilisateur",
        photoReplace: 'Remplacer',
        photoSelect: 'Choisir une photo',
        photoRemove: 'Supprimer',
        photoHint: "Photo facultative de l'utilisateur",
        labelFullName: 'Nom complet *',
        placeholderFullName: "Nom de l'utilisateur",
        labelPassword: 'Mot de passe *',
        placeholderPassword: 'Créez un mot de passe sécurisé',
        labelEmail: 'E-mail',
        labelEmailRequired: 'E-mail *',
        labelEmailHint: "Modifier l'e-mail changera l'accès de connexion de l'utilisateur",
        labelPhone: 'Téléphone',
        labelEmployee: 'Lier à un employé',
        placeholderEmployee: 'Aucun employé lié',
        employeeNone: 'Aucun',
        employeeHint: 'Lie cet utilisateur à un employé enregistré',
        emailConflictTitle: 'Les e-mails sont différents. Lequel doit prévaloir ?',
        emailConflictUser: 'Utilisateur :',
        emailConflictEmployee: 'Employé :',
        emailConflictHint: "L'e-mail choisi sera appliqué aux deux enregistrements",
        labelUserType: "Type d'utilisateur",
        typeInternal: 'Interne',
        typeTech: 'Technicien',
        descTech: "Apparaît dans la liste des techniciens du bon de travail et peut être ajouté à des équipes",
        descInternal: "Utilisateur interne du système, n'apparaît pas comme technicien sur les bons de travail",
        labelProfile: "Profil d'accès",
        placeholderProfile: 'Sélectionnez un profil',
        profileCustom: 'Personnalisé',
        profileAll: 'Accès total',
        sectionScreens: 'Écrans',
        hintScreens: "Sélectionnez les écrans auxquels cet utilisateur peut accéder",
        sectionFunctions: 'Fonctions',
        hintFunctions: "Sélectionnez les actions que cet utilisateur peut effectuer",
        sectionPermissions: 'Permissions',
        hintPermissions: "Choisissez un profil rapide ou activez les écrans un par un. Les actions de chaque écran sont à l'intérieur.",
      },

      // ── Éditeur de permissions (partagé utilisateur/rôle) ─────────────────────
      permissionsEditor: {
        quickProfiles: 'Profils rapides',
        fullAccess: 'Accès total',
        fullAccessHint: 'Débloque tous les écrans et actions, y compris ceux à venir',
        custom: 'Personnalisé',
        searchPlaceholder: 'Rechercher un écran ou une action...',
        clearSearch: 'Effacer la recherche',
        actionsTitle: 'Actions de cet écran',
        screenOffHint: "L'écran est désactivé, mais ces actions restent valables d'elles-mêmes.",
        emptySearch: 'Aucun écran ou action trouvé pour "{query}".',
        generalGroup: 'Général',
        ariaScreenSwitch: "Accès à l'écran {screen}",
        ariaScreenActions: 'Voir les actions de {screen}',
      },

      presets: {
        dialogTitle: 'Paramètres des rôles',
        dialogDesc: "Créez et modifiez des profils d'accès avec des permissions prédéfinies",
        btnCreate: 'Nouveau rôle',
        btnSave: 'Enregistrer',
        btnSaveNew: 'Créer le rôle',
        btnCancel: 'Annuler',
        btnBack: '← Retour',
        empty: 'Aucun rôle enregistré',
        permCount: '{count} permissions',
        labelName: 'Nom *',
        placeholderName: 'Ex : Technicien de terrain',
        labelDesc: 'Description',
        placeholderDesc: 'Description facultative',
        sectionScreens: 'Écrans',
        sectionFunctions: 'Fonctions',
        emptyMobileTitle: 'Aucun rôle enregistré',
        emptyMobileDesc: "Touchez \"Nouveau rôle\" pour créer un profil d'accès",
        noDescription: 'Aucune description',
        actionEdit: 'Modifier',
        actionDuplicate: 'Dupliquer',
        actionDelete: 'Supprimer',
      },

      list: {
        ariaViewPhoto: 'Voir la photo',
        emptyFilteredDesc: 'Essayez une autre recherche',
        emptyAllDesc: 'Touchez "Nouvel utilisateur" pour commencer',
        reactivateNoSlot: 'Réactiver (aucune place)',
      },
    },

    profile: {
      pageTitle: 'Mon profil',
      pageSubtitle: 'Gérez vos informations personnelles',
      labelFullName: 'Nom complet',
      placeholderFullName: 'Votre nom',
      labelEmail: 'E-mail',
      labelPhone: 'Téléphone',
      placeholderPhone: '(00) 00000-0000',
      labelCreatedAt: 'Compte créé le',
      btnSave: 'Enregistrer les modifications',
      btnSaving: 'Enregistrement...',
      cardPasswordTitle: 'Mot de passe',
      cardPasswordDesc: 'Modifiez le mot de passe de votre compte',
      btnChangePassword: 'Changer le mot de passe',
      labelNewPassword: 'Nouveau mot de passe',
      placeholderNewPassword: 'Créez un mot de passe sécurisé',
      labelConfirmPassword: 'Confirmer le nouveau mot de passe',
      placeholderConfirmPassword: 'Répétez le nouveau mot de passe',
      btnSavePassword: 'Enregistrer le nouveau mot de passe',
      btnCancel: 'Annuler',
      toastProfileUpdated: 'Profil mis à jour avec succès !',
      toastSaveError: "Erreur lors de l'enregistrement",
      toastPhotoUpdated: 'Photo mise à jour !',
      toastPhotoError: 'Erreur lors du téléchargement de la photo',
      toastPasswordWeak: 'Mot de passe trop faible',
      toastPasswordWeakDesc: 'Utilisez au moins 8 caractères avec des majuscules, minuscules, chiffres et/ou caractères spéciaux.',
      toastPasswordMismatch: 'Les mots de passe ne correspondent pas',
      toastPasswordChanged: 'Mot de passe modifié avec succès !',
      toastPasswordError: 'Erreur lors du changement de mot de passe',
      btnBack: 'Retour',
    },

    billing: {
      pageTitle: 'Abonnement',
      pageSubtitle: 'Gérez votre forfait, vos modules et vos paiements',
      heroTrialTitle: 'Activez votre abonnement',
      heroTrialDesc: "Vous êtes en période d'essai. Choisissez le plan idéal et obtenez un accès complet.",
      heroTrialDaysLeft: "{days} jour{s} restant{s} d'essai",
      btnChoosePlan: 'Choisir un plan et activer',
      planLabel: 'Plan {name}',
      statusExpired: 'Expiré',
      statusExpiringSoon: 'Expire bientôt',
      statusActive: 'Actif',
      statusMsgExpired: 'Expiré depuis {days} jour{s}',
      statusMsgExpiringSoon: 'Expire dans {days} jour{s}',
      statusMsgActive: '{days} jours restants',
      statusMsgActiveNoDate: 'Actif',
      labelMonthlyValue: 'Montant mensuel',
      labelExpiration: 'Expiration',
      sectionActiveModules: 'Modules actifs',
      sectionAvailableModules: 'Modules disponibles',
      btnHireModule: 'Souscrire',
      cardPaymentTitle: 'Paiement',
      cardPaymentDescExpired: 'Votre abonnement a expiré. Renouvelez maintenant.',
      cardPaymentDescActive: 'Maintenez votre abonnement à jour.',
      labelAmountDue: 'Montant à payer',
      labelDueDate: 'Échéance',
      btnPayNow: 'Payer maintenant',
      paymentYearlyDiscount: "20% de remise sur le plan annuel, valable pour Pix ou virement",
      bannerPromoTitle: 'Prix promotionnel actif',
      bannerPromoDesc: "Apres la periode promotionnelle, le montant reviendra a {value}.",
      bannerPendingTitle: 'Changement de montant programme',
      bannerPendingDesc: 'Montant actuel : {current}. A partir du prochain prelevement : {new}.',
      bannerRecurringTitle: 'Renouvellement automatique actif',
      bannerRecurringDesc: "Votre abonnement se renouvelle automatiquement sur la carte enregistree.",
      bannerNfseTitle: "Donnees manquantes pour l'emission de factures",
      bannerNfseDesc: "Completez dans les parametres de l'entreprise : {fields}",
      cardSummaryTitle: "Résumé du plan",
      summaryPlan: 'Plan',
      summaryActiveModules: 'Modules actifs',
      summaryMaxUsers: 'Max. utilisateurs',
      summaryCycle: 'Cycle',
      summaryStatus: 'Statut',
      cycleYearly: 'Annuel',
      cycleMonthly: 'Mensuel',
      cardPaymentHistoryTitle: 'Historique des paiements',
      cardPaymentHistoryDesc: 'Factures et paiements de votre abonnement.',
      btnCancelSubscription: "Annuler l'abonnement",
      cancelSubscriptionNote: "En annulant, le renouvellement automatique s'arrête et vous conservez l'accès jusqu'à la date d'expiration.",
      btnManagePlan: 'Gerer mon abonnement',
      nfseTierSectionTitle: 'Niveau de facturation (NFS-e)',
      nfseTierCurrent: 'actuel',
      nfseTierUpgradeNote: 'Le nouveau niveau est active immediatement et le nouveau tarif entre en vigueur au prochain prelevement.',
      // Panneau "Votre Forfait" en temps reel (ModulesManagementCard)
      planSummaryTitle: 'Votre Forfait',
      planSummaryIncluded: 'Inclus',
      planSummaryEmptyCustom: 'Choisissez des modules a gauche pour composer votre forfait.',
      planSummaryEmptyPlans: 'Selectionnez un forfait a gauche.',
      planSummaryUsersOne: 'utilisateur',
      planSummaryUsersPlural: 'utilisateurs',
      planSummaryUsersIncluded: 'inclus',
      planSummaryCycleLabel: 'Cycle',
      planSummaryCycleYearlyDesc: '20% de reduction Pix/Virement',
      planSummaryCycleYearlyInfo: 'La reduction de 20% est valable uniquement pour un paiement comptant (Pix ou Virement). Par carte, la facturation reste mensuelle.',
      planSummaryYearlySavings: 'Vous economisez {value}/an',
      planSummaryTotalMonthly: 'Total mensuel',
      planSummaryTotalYearly: 'Total annuel',
      planSummaryDiffLabel: 'vs forfait actuel',
      planSummaryCtaMonthly: 'Appliquer le forfait',
      planSummaryCtaYearly: 'Payer et activer (annuel)',
      planSummaryCtaApplying: 'Application...',
      planSummaryUpgradeNote: 'Mise a niveau : les fonctionnalites sont activees immediatement et le nouveau tarif entre en vigueur au prochain prelevement.',
      planSummaryDowngradeNote: 'Retrogradation planifiee : vous conservez le forfait actuel jusqu`a la fin de la periode payee. Le nouveau montant de {value}/mois entre en vigueur au prochain prelevement.',
      noCompanyTitle: 'Aucune entreprise liée',
      noCompanyDesc: "Votre compte n'est pas encore lié à une entreprise.",

      cancelSubscription: {
        titleReason: "Annuler l'abonnement",
        titleConfirm: "Confirmer l'annulation",
        titleDone: 'Annulation effectuee',
        headerSubtitle: 'Aidez-nous a comprendre pourquoi',
        labelReason: "Motif d'annulation",
        placeholderReason: 'Selectionnez un motif',
        labelDetailsRequired: 'Expliquez votre motif *',
        labelDetailsOptional: 'Voulez-vous nous en dire plus ? (facultatif)',
        placeholderDetailsRequired: "Decrivez le motif d'annulation...",
        placeholderDetailsOther: 'Des suggestions ou commentaires...',
        retentionTalkSupport: 'Parler au support',
        confirmOnConfirm: 'En confirmant :',
        confirmBullet1: 'Le renouvellement automatique sera annule',
        confirmBullet2Until: "Vous conservez l'acces jusqu'au",
        confirmBullet2NoDate: 'la fin de la periode deja payee',
        confirmBullet3: 'Les futures factures en attente seront automatiquement annulees',
        confirmBullet4: 'Vous pouvez reabonner a tout moment',
        confirmReasonPrefix: 'Motif :',
        confirmDetailsPrefix: 'Details :',
        doneTitle: 'Abonnement annule',
        doneDesc: "Le renouvellement automatique a ete annule. Vous conservez l'acces",
        doneDescUntil: "jusqu'au {date}",
        doneDescNoDate: 'jusqu\'a la fin de la periode deja payee',
        doneDescSuffix: '. Si vous changez d\'avis, vous pouvez reabonner a tout moment.',
        btnBack: 'Retour',
        btnContinue: 'Continuer',
        btnCanceling: 'Annulation...',
        btnConfirm: "Confirmer l'annulation",
        btnUnderstood: 'Compris',
        toastError: "Impossible d'annuler maintenant. Reessayez ou contactez le support.",
        toastSelectReason: 'Selectionnez un motif',
        reasons: {
          preco_alto: 'Le prix est trop eleve',
          nao_uso: "Je ne l'utilise pas assez",
          funcionalidades: 'Il manque des fonctionnalites dont j\'ai besoin',
          dificuldade_uso: "Difficulte d'utilisation du systeme",
          concorrente: "J'ai trouve une autre solution",
          fechando_empresa: 'Je ferme mon entreprise',
          temporario: 'Pause temporaire',
          outro: 'Autre raison',
        },
        retention: {
          preco_alto: {
            title: 'Et si nous vous faisions une remise ?',
            message: 'Contactez-nous sur WhatsApp, nous pouvons negocier un tarif special pour que vous restiez sur Dominex.',
            waText: "Bonjour ! Je pense a annuler mon abonnement Dominex a cause du prix. Y a-t-il une offre speciale ?",
          },
          nao_uso: {
            title: 'Nous pouvons vous aider !',
            message: 'Notre equipe propose une formation personnalisee gratuite pour que vous profitiez pleinement du systeme.',
            waText: "Bonjour ! Je voudrais planifier une formation Dominex avant de decider pour mon abonnement.",
          },
          funcionalidades: {
            title: 'Votre avis compte !',
            message: 'Dites-nous quelles fonctionnalites vous manquent. Nous evoluons constamment et votre suggestion pourrait bientot etre integree !',
            waText: "Bonjour ! Il me manque certaines fonctionnalites dans Dominex et je souhaite les suggerer avant d'annuler.",
          },
          dificuldade_uso: {
            title: 'Support dedie',
            message: 'Nous pouvons planifier une session de support dediee pour repondre a vos questions et faciliter l\'utilisation.',
            waText: "Bonjour ! J'ai du mal a utiliser Dominex et je souhaite une session de support avant d'annuler.",
          },
        },
        // Offre de passage a un plan inferieur
        downgradeOfferTitle: 'Et si vous passiez a un plan moins cher ?',
        downgradeOfferDesc: 'Au lieu d\'annuler, passez au plan {plan} et economisez {savings} par mois.',
        downgradeOfferCta: 'Passer au plan {plan}',
        downgradeOfferApplying: 'Application...',
        downgradeOfferToastSuccess: 'Plan change vers {plan} avec succes.',
        downgradeOfferToastError: 'Impossible de changer le plan maintenant. Reessayez.',
      },

      paymentHistory: {
        empty: 'Aucun paiement enregistre pour le moment.',
        perPage: 'Par page',
        pageSummary: 'Page {current} sur {total}',
        prevPage: 'Page precedente',
        nextPage: 'Page suivante',
        viewCharge: 'Voir la facture',
        statusPaid: 'Paye',
        statusPending: 'En attente',
        statusOverdue: 'En retard',
        statusCancelled: 'Annule',
        typeSale: 'Vente',
        typeRenewal: 'Renouvellement',
        methodPix: 'PIX',
        methodBoleto: 'Boleto',
        methodCard: 'Carte',
        methodDefault: 'Facturation',
      },

      accountUsage: {
        title: 'Utilisation du compte',
        description: 'Suivez la consommation de votre plan',
        users: 'Utilisateurs',
        nfseThisMonth: 'Factures ce mois',
        usersIncluded: '{base} inclus + {extra} supplementaire',
        usersIncludedPlural: '{base} inclus + {extra} supplementaires',
        usersPlanLimit: '{max} utilisateur dans le plan actuel',
        usersPlanLimitPlural: '{max} utilisateurs dans le plan actuel',
        nfseUnlimitedLabel: '{used} emises',
        nfseLevelLimit: 'Limite du Niveau {level} — {limit} factures/mois',
        nfsePlanLimit: 'Limite du plan — {limit} factures/mois',
        paymentMethods: 'Moyens de paiement',
      },
    },

    changelog: {
      pageTitle: 'Nouveautes',
      searchPlaceholder: 'Rechercher des mises a jour...',
      currentVersionBadge: 'Version actuelle',
      filterAll: 'Toutes',
      filterRecurso: 'Fonctionnalites',
      filterMelhoria: 'Ameliorations',
      filterCorrecao: 'Correctifs',
      filterSeguranca: 'Securite',
      categoryRecurso: 'Nouvelle fonctionnalite',
      categoryMelhoria: 'Amelioration',
      categoryCorrecao: 'Correctif',
      categorySeguranca: 'Securite',
      emptyTitle: 'Aucune mise a jour trouvee',
      emptyDesc: 'Essayez une recherche differente ou changez les filtres.',
      changesCount_one: '{count} changement',
      changesCount_other: '{count} changements',
      showMore: 'Voir plus',
      showLess: 'Voir moins',
    },

    terms: {
      modalTitle: "Conditions d'utilisation — Dominex",
      pdfTitle: "CONDITIONS D'UTILISATION — DOMINEX",
      pdfFilename: "CONDITIONS D'UTILISATION - DOMINEX",
      checkboxLabel: "J'ai lu et j'accepte les Conditions d'utilisation de Dominex",
      btnAccept: "J'accepte les conditions",
      btnAccepting: 'Confirmation...',
      btnDownloadPdf: 'Télécharger le PDF',
      btnGeneratingPdf: 'Génération du PDF...',
      btnClose: 'Fermer',
      acceptedAt: 'Accepté le {date}',
      pdfFooterPage: 'Page {current} sur {total}',
      pdfGeneratedOn: 'Document généré le {date}',
      errorPdf: 'Erreur lors de la génération du PDF. Veuillez réessayer.',
    },

    myData: {
      backToDashboard: '← Retour au tableau de bord',
      pageTitle: 'Centre de confidentialité',
      pageSubtitle: 'Gérez vos données personnelles (droits sur les données personnelles)',
      cardPersonalTitle: 'Vos données personnelles',
      labelName: 'Nom',
      labelEmail: 'E-mail',
      labelPhone: 'Téléphone',
      labelCreatedAt: 'Compte créé le',
      cardExportTitle: 'Exporter mes données',
      cardExportDesc: 'Téléchargez une copie de vos données au format JSON (droit à la portabilité des données).',
      btnExport: 'Télécharger mes données',
      btnExporting: 'Exportation...',
      cardDeleteTitle: 'Demander la suppression du compte',
      cardDeleteDesc: "Demandez la suppression définitive de votre compte et de vos données personnelles. Les données fiscales et comptables peuvent être conservées pendant la durée légale (5 ans). Le traitement prend jusqu'à 15 jours ouvrables.",
      deletionRequested: 'Demande enregistrée. Notre équipe vous contactera.',
      btnRequestDeletion: 'Demander la suppression',
      btnRequesting: 'Enregistrement...',
      cardDpoTitle: 'Contacter le DPO',
      cardDpoDesc: "Pour exercer d'autres droits (rectification, portabilité, retrait du consentement) ou pour toute question sur le traitement de vos données :",
      linkPrivacy: 'Politique de confidentialité',
      linkTerms: "Conditions d'utilisation",
      toastExported: 'Données exportées avec succès !',
      toastExportError: "Erreur lors de l'exportation",
      toastDeletionRequested: 'Demande de suppression soumise',
      toastDeletionRequestedDesc: 'Notre équipe traitera votre demande dans un délai de 15 jours ouvrables conformément à la loi sur la protection des données.',
      toastDeletionError: "Erreur lors de l'enregistrement de la demande",
    },

    integrations: {
      subTabs: {
        whatsapp: 'WhatsApp',
        asaas: 'Encaissements',
        simulador: 'Simulateur',
      },
      simulator: {
        title: 'Simulateur de vente',
        description: 'Voyez ce qu\'il reste d\'une vente après les frais Asaas et quand l\'argent arrive sur votre compte.',
        notConnectedTitle: 'Votre compte Asaas n\'est pas encore connecté',
        notConnectedDesc: 'Les chiffres ci-dessous utilisent la grille de référence Asaas. Connectez votre compte dans l\'onglet Encaissements pour simuler avec les frais réels de votre offre.',
        notContractedTitle: 'Encaissements non inclus',
        notContractedDesc: 'Le simulateur montre ce qu\'il reste de chaque vente par Pix, boleto ou carte. Parlez à notre équipe pour activer les encaissements en ligne.',
        amountLabel: 'Montant de la vente',
        methodLabel: 'Moyen de paiement',
        methodPix: 'Pix',
        methodBoleto: 'Boleto',
        methodCard: 'Carte',
        installmentsLabel: 'Échéances',
        installmentsOption: '{n}x',
        feePayerLabel: 'Qui paie les frais',
        feePayerCompany: 'L\'entreprise les absorbe',
        feePayerCustomer: 'Les répercuter sur le client',
        anticipateLabel: 'Anticiper l\'encaissement',
        anticipateDesc: 'Recevez tout en 1 jour ouvré en payant les frais d\'anticipation Asaas.',
        anticipateUnavailable: 'Asaas n\'a pas communiqué les frais d\'anticipation de votre compte. Nous utilisons le taux de référence.',
        resultTitle: 'Résultat de la simulation',
        resultGross: 'Montant brut',
        resultCustomerPays: 'Le client paie',
        resultFee: 'Frais Asaas',
        resultNet: 'Montant net sur votre compte',
        resultInstallment: 'Chaque échéance pour le client',
        resultAnticipationCost: 'Coût de l\'anticipation',
        resultNetAnticipated: 'Net anticipé',
        resultEffectiveCost: 'Coût total de la vente',
        compareTitle: 'Avec et sans anticipation',
        compareWithout: 'Sans anticiper',
        compareWith: 'En anticipant',
        compareWhen: 'arrivée',
        scheduleTitle: 'Quand l\'argent arrive sur votre compte',
        scheduleInstallment: 'Échéance {n}',
        scheduleSingle: 'Paiement',
        scheduleTotal: 'Total',
        settlementSameDay: 'immédiat',
        settlementInDay: 'en 1 jour ouvré ({date})',
        settlementInDays: 'en {count} jours ({date})',
        settlementBusinessDayNote: 'Le week-end et les jours fériés ne comptent pas : si la date tombe un samedi, un dimanche ou un jour férié, l\'argent arrive le jour ouvré suivant.',
        settlementFallbackNote: 'Délai de référence, Asaas ne le communique pas via l\'API.',
        referenceWarning: 'Certaines valeurs viennent de la grille de référence Asaas, pas de votre compte.',
        pixFreeNote: 'Les {count} premiers encaissements Pix du mois sont sans frais, dans ce cas le net correspond au montant complet.',
        disclaimer: 'Estimation. Le montant final est calculé par Asaas au moment de la facturation.',
        emptyAmount: 'Saisissez le montant de la vente pour voir la simulation.',
      },
      asaas: {
        loading: 'Chargement…',
        notContracted: {
          title: 'Encaissements non inclus',
          description: `Cette fonctionnalité ne fait pas partie de votre offre actuelle. Contactez notre équipe pour encaisser vos clients en ligne par Pix, boleto et carte.`,
        },
        card: {
          title: 'Encaissements (Asaas)',
          description: 'Encaissez vos clients par Pix, boleto et carte avec votre propre compte Asaas.',
        },
        status: {
          active: 'Actif',
          pending: `En vérification`,
          disabled: `Désactivé`,
          rejected: `Refusé`,
        },
        inactive: {
          intro: `Collez la clé d'API de votre compte Asaas pour activer les encaissements. Vous générez cette clé dans le panneau Asaas, sous Paramètres, Intégrations.`,
          apiKeyLabel: `Clé d'API Asaas`,
          apiKeyPlaceholder: `Collez votre clé d'API ici`,
          activate: 'Activer les encaissements',
          activating: 'Activation…',
          helpLink: `Où trouver ma clé ?`,
          apiKeyGuide: {
            guideTitle: `Comment obtenir votre clé d'API Asaas`,
            trigger: `Comment obtenir ma clé d'API Asaas ?`,
            step1: `Accédez à asaas.com et connectez-vous (ou créez votre compte gratuit).`,
            step2: `Dans le menu, allez dans "Paramètres du compte" (icône d'engrenage), puis "Intégrations".`,
            step3: `Dans la section "Clé d'API", cliquez sur "Générer une clé d'API" ou copiez la clé existante.`,
            step4: `Copiez la clé (commence par "$aact_") et collez-la dans le champ ci-dessus. La clé est secrète, conservez-la précieusement.`,
            sandboxHint: `Pour tester sans argent réel, utilisez le compte sandbox sur sandbox.asaas.com en suivant les mêmes étapes.`,
          },
        },
        activeState: {
          connected: 'Encaissements actifs. Vous pouvez maintenant encaisser vos clients.',
          pendingDesc: `Votre compte est en vérification chez Asaas. Une fois approuvé, les encaissements deviennent actifs.`,
          feesTitle: 'Frais Asaas',
          feesNote: 'Les frais sont prélevés par Asaas directement sur votre compte. Dominex ne retient rien par transaction.',
          feePix: 'Pix',
          feePixValue: 'R$ 0,99 à R$ 1,99 par encaissement',
          feeBoleto: 'Boleto',
          feeBoletoValue: 'R$ 0,99 à R$ 1,99',
          feeCard: 'Carte de crédit',
          feeCardValue: '1,99% à 4,29% + R$ 0,49 par facture',
          feeAnticipation: 'Anticipation',
          feeSettlement: 'Délai avant réception',
          feeNotInformed: 'Non communiqué par Asaas',
          feeReferenceTag: 'référence',
          feeAnticipationValue: '1,25% à 1,70% par mois (carte), 5,79% par mois (boleto et Pix)',
          feeSettlementValue: 'Pix immédiat, boleto en 1 jour, carte en 32 jours',
          feesFromAccount: 'Frais de votre compte Asaas, mis à jour le',
          feesFromAccountNoDate: 'Frais lus depuis votre compte Asaas.',
          feesFromOverride: 'Frais ajustés manuellement pour votre entreprise.',
          feesFromReference: 'Frais de référence, nous n\'avons pas pu lire ceux de votre compte pour le moment.',
          feesRefresh: 'Actualiser les frais',
          feesRefreshing: 'Actualisation…',
          feesRefreshOk: 'Frais actualisés avec Asaas.',
          feesRefreshError: 'Nous n\'avons pas pu lire les frais de votre compte Asaas pour le moment.',
          feePixFreeNote: 'Les {count} premiers encaissements du mois sont sans frais.',
          feePerCharge: 'par facture',
          feeOneShot: 'comptant',
          feeInstallments: 'échelonné',
          feeMonthly: 'par mois',
          settlementSameDay: 'immédiat',
          settlementInDay: 'en 1 jour',
          settlementInDays: 'en {count} jours',
          financeAccountRequiredError: 'Choisissez le compte qui recevra l\'argent, sans cela la recette n\'entre pas dans vos Finances.',
          financeAccountPlaceholder: 'Choisissez le compte…',
          financeAccountPendingTitle: 'Il reste à choisir le compte qui reçoit l\'argent',
          financeAccountPendingDesc: 'Tant que vous ne l\'aurez pas choisi, les factures payées n\'entrent pas seules dans vos Finances.',
          financeAccountPendingCta: 'Choisir maintenant',
          financeAccountEmptyTitle: 'Vous n\'avez encore aucun compte bancaire enregistré',
          financeAccountEmptyDesc: 'Créez le compte dans Finances, onglet Mouvements, pour choisir où la recette arrive.',
          financeAccountEmptyCta: 'Ouvrir Finances',
          dueDaysError: 'Indiquez dans combien de jours la facture est due (0 ou plus).',
          maxInstallmentsError: 'Indiquez le nombre maximum d\'échéances (1 ou plus).',
          discountPercentError: 'La remise doit être un nombre entre 0 et 100.',
          discountDaysError: 'Indiquez combien de jours avant l\'échéance la remise s\'applique (0 ou plus).',
          deactivate: 'Désactiver les encaissements',
          deactivating: 'Désactivation…',
          autoPostLabel: 'Enregistrer automatiquement les encaissements en comptabilité',
          autoPostDesc: `Chaque encaissement crée une créance qui est soldée automatiquement dès que le client paie.`,
          autoPostToastOn: `Enregistrement automatique activé.`,
          autoPostToastOff: `Enregistrement automatique désactivé.`,
          autoPostToastError: `Impossible d'enregistrer le paramètre. Veuillez réessayer.`,
          autoFeeLabel: `Enregistrer les frais Asaas comme dépense`,
          autoFeeDesc: `Lors d'un encaissement, les frais Asaas sont enregistrés comme dépense dans votre comptabilité afin que la trésorerie soit exacte.`,
          autoFeeToastOn: `Enregistrement des frais activé.`,
          autoFeeToastOff: `Enregistrement des frais désactivé.`,
          autoFeeError: `Impossible d'enregistrer le paramètre. Veuillez réessayer.`,
          lateFeesTitle: `Pénalités et intérêts de retard`,
          lateFeesDesc: `Appliqué aux prochaines factures lorsque le client paie en retard.`,
          fineLabel: `Pénalité de retard (%)`,
          interestLabel: `Intérêt mensuel (%)`,
          interestCapWarning: `Le plafond Asaas est de 10 % par mois.`,
          lateFeesSave: `Enregistrer`,
          lateFeesSaving: `Enregistrement…`,
          lateFeesToastOk: `Pénalités et intérêts mis à jour.`,
          lateFeesToastError: `Impossible d'enregistrer. Veuillez réessayer.`,
          // ── Préférences de facturation ────────────────────────────────────
          save: `Enregistrer`,
          saving: `Enregistrement…`,
          chargePrefsTitle: `Préférences de facturation`,
          chargePrefsDesc: `Paramètres par défaut appliqués aux nouvelles factures.`,
          dueDaysLabel: `Échéance par défaut`,
          dueDaysSuffix: `jours`,
          defaultDescriptionLabel: `Description/instructions par défaut`,
          defaultDescriptionPlaceholder: `Ex. : Relatif au service de maintenance…`,
          paymentMethodsLabel: `Moyens de paiement activés`,
          methodPix: `Pix`,
          methodBoleto: `Boleto`,
          methodCard: `Carte de crédit`,
          maxInstallmentsLabel: `Nb. max. de versements (carte)`,
          maxInstallmentsSuffix: `x`,
          chargePrefsToastOk: `Préférences de facturation enregistrées.`,
          // ── Escompte pour paiement anticipé ──────────────────────────────
          discountTitle: `Escompte pour paiement anticipé`,
          discountDesc: `Proposez une remise aux clients qui paient avant l'échéance.`,
          discountPercentLabel: `Remise (%)`,
          discountPercentPlaceholder: `Ex. : 5`,
          discountDaysLabel: `Jusqu'à X jours avant`,
          discountDaysPlaceholder: `Ex. : 10`,
          discountDaysSuffix: `jours`,
          discountHint: `Laissez vide pour ne pas appliquer de remise.`,
          discountToastOk: `Escompte pour paiement anticipé enregistré.`,
          // ── Enregistrement en comptabilité ────────────────────────────────
          financeConfigTitle: `Enregistrement en comptabilité`,
          financeConfigDesc: `Définissez le compte et les catégories utilisés lors du solde automatique des factures payées.`,
          financeAccountLabel: `Compte bancaire cible`,
          financeAccountNone: `Aucun`,
          incomeCategoryLabel: `Catégorie de recette`,
          incomeCategoryNone: `Aucune`,
          feeCategoryLabel: `Catégorie de frais (Asaas)`,
          feeCategoryHint: `Catégorie utilisée pour enregistrer les frais Asaas comme dépense.`,
          financeConfigToastOk: `Paramètres d'enregistrement comptable sauvegardés.`,
        },
        deactivateConfirm: {
          title: 'Désactiver les encaissements ?',
          description: `Les factures déjà générées restent valides, mais vous ne pourrez pas en créer de nouvelles avant de réactiver.`,
          confirm: 'Oui, désactiver',
          cancel: 'Annuler',
        },
        errors: {
          activateGeneric: `Impossible d'activer les encaissements. Vérifiez la clé et réessayez.`,
          deactivateGeneric: `Impossible de désactiver les encaissements.`,
        },
        toasts: {
          activated: 'Encaissements activés !',
          deactivated: 'Encaissements désactivés.',
        },
      },
      whatsapp: {
        loading: 'Chargement…',
        comingSoon: {
          badge: 'Bientôt disponible',
          title: 'Notifications WhatsApp',
          description: `Bientôt vous pourrez prévenir vos clients automatiquement par WhatsApp. Nous finalisons les derniers ajustements.`,
        },
        notContracted: {
          title: `Alertes WhatsApp non incluses`,
          description: `Cette fonctionnalité ne fait pas partie de votre forfait actuel. Contactez notre équipe pour activer les alertes WhatsApp automatiques envoyées à vos clients.`,
        },
        masterToggle: {
          title: 'Notifications WhatsApp',
          description: 'Activer ou désactiver toutes les notifications envoyées au client via WhatsApp',
          ariaLabel: 'Activer les notifications WhatsApp',
        },
        connection: {
          sectionTitle: 'Connexion',
          sectionDescription: 'Connectez le WhatsApp de votre entreprise pour envoyer des notifications aux clients',
          statusConnected: 'Connecté',
          statusQr: 'En attente du QR',
          statusDisconnected: 'Déconnecté',
          statusBanned: 'Banni',
          btnConnect: 'Connecter WhatsApp',
          connecting: 'Connexion…',
          qrInstructions: 'Ouvrez WhatsApp sur votre téléphone, accédez à Menu > Appareils connectés > Connecter un appareil et scannez le code QR ci-dessous.',
          connectedDesc: 'WhatsApp connecté et prêt à envoyer des notifications.',
          bannedDesc: 'Ce numéro a été banni par WhatsApp. Veuillez contacter le support.',
          errorTitle: 'Impossible de se connecter',
          errorHint: 'Vérifiez votre connexion ou réessayez dans quelques instants. Le serveur d`intégration est peut-être en cours de configuration.',
        },
        triggers: {
          sectionTitle: 'Déclencheurs de notification',
          sectionDescription: 'Choisissez à quel moment le client reçoit une notification automatique lors d`un bon de travail',
          aCaminho: {
            label: 'Technicien en route',
            description: 'Notifie le client quand le technicien part pour l`intervention',
          },
          emAndamento: {
            label: 'Service démarré',
            description: 'Notifie le client quand le technicien commence le travail',
          },
          concluida: {
            label: 'Bon de travail terminé',
            description: 'Notifie le client quand le bon de travail est finalisé',
          },
        },
        quota: {
          sectionTitle: 'Plan et quota',
          sectionDescription: 'Votre niveau actuel et la consommation de notifications du mois en cours',
          currentPlan: 'Plan actuel',
          usageLabel: 'Notifications envoyées ce mois',
          unlimited: 'Illimité',
          nearLimitWarning: 'Vous approchez de votre limite mensuelle.',
          resetHint: 'Le quota est renouvelé automatiquement au début de chaque mois.',
        },
      },
    },

    help: {
      title: "Centre d'aide",
      intro: "Questions fréquentes sur le système. Si vous ne trouvez pas votre réponse, contactez le support via WhatsApp.",
      faqs: [
        {
          q: 'Comment créer un bon de travail ?',
          a: 'Allez dans "Bons de travail" dans le menu latéral, cliquez sur "+ Nouveau bon", remplissez les informations du client, le type de service et le technicien assigné, puis enregistrez.',
        },
        {
          q: 'Comment enregistrer un nouveau client ?',
          a: 'Allez dans "Clients" dans le menu, cliquez sur "Nouveau client" et remplissez les informations. Vous pouvez enregistrer un particulier ou une entreprise.',
        },
        {
          q: 'Comment fonctionne la gestion financière ?',
          a: 'Dans le menu "Finances", vous avez une vue d’ensemble, les transactions, les comptes à payer/recevoir et le compte de résultat. Les bons de travail peuvent générer automatiquement des écritures financières.',
        },
        {
          q: 'Comment fonctionnent les paiements des employés ?',
          a: "Dans \"Employés\", cliquez sur l'icône de paiement sur la fiche de l'employé. Le système calcule automatiquement salaire + primes - avances - absences et vous permet de sélectionner le compte à débiter.",
        },
        {
          q: 'Comment configurer le suivi des présences ?',
          a: 'Allez dans Employés > Suivi des présences. Définissez les horaires standard, la tolérance de retard et les exigences de selfie/géolocalisation dans les paramètres.',
        },
        {
          q: 'Comment créer des contrats récurrents ?',
          a: "Dans \"Contrats\", créez un nouveau contrat en définissant le client, la fréquence (mensuelle, bimestrielle, etc.) et la période. Le système génère automatiquement les occurrences planifiées.",
        },
        {
          q: "Qu'est-ce que le CRM et comment l'utiliser ?",
          a: "Le CRM est votre entonnoir de vente. Créez des leads, déplacez-les entre les étapes (Nouveau, Contact, Proposition, Gagné/Perdu) et suivez la valeur du pipeline en temps réel.",
        },
        {
          q: 'Comment fonctionne le module de Stock ?',
          a: 'Dans "Stock", enregistrez des articles avec leur prix de revient/vente, la quantité minimale et l’unité. Le système vous alerte quand le stock passe sous le minimum.',
        },
        {
          q: 'Comment exporter des rapports ?',
          a: 'Plusieurs modules disposent de boutons d’exportation (PDF, HTML). Les relevés d’employés, devis et comptes de résultat proposent des options d’exportation et d’impression.',
        },
        {
          q: 'Comment changer mon mot de passe ?',
          a: 'Allez dans "Profil" depuis le menu utilisateur (bas du menu latéral) et cliquez sur "Changer le mot de passe". Vous recevrez un e-mail pour le réinitialiser.',
        },
      ],
    },
  },
};
