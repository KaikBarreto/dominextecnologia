      ...
      {activeTab === 'geral' && (
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {(() => {
            ...
            if ((customer as any).celular) {
              const celular = (customer as any).celular as string;
              const whatsappNumber = celular.replace(/\D/g, '');
              const whatsappUrl = `https://wa.me/55${whatsappNumber}`;
              rows.push({
                label: t.fieldCelular,
                node: (
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium flex items-center gap-1 leading-tight">
            ...
            if ((customer as any).nome_fantasia) {
              rows.push({
                label: t.fieldFantasia,
                node: <span className="text-sm font-medium leading-tight">{(customer as any).nome_fantasia}</span>,
              });
            }
