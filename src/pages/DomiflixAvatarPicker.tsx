...
function buildSections(t: typeof MESSAGES['pt-br']['app']['domiflix']['avatar']): AvatarSection[] {
  return [
    {
      label: t.sectionClassics,
      images: PADRÃO_NAMES.map((n) => ({ url: padrao(n), name: n })),
    },
    {
      label: t.sectionChars1,
      images: Array.from({ length: 85 }, (_, i) => ({
        url: outras(i + 1),
        name: t.characterName.replace('{{number}}', String(i + 1)),
      })),
    },
    {
      label: t.sectionChars2,
      images: Array.from({ length: 85 }, (_, i) => ({
        url: outras(i + 86),
        name: t.characterName.replace('{{number}}', String(i + 86)),
      })),
    },
    {
      label: t.sectionChars3,
      images: Array.from({ length: 85 }, (_, i) => ({
        url: outras(i + 171),
        name: t.characterName.replace('{{number}}', String(i + 171)),
      })),
    },
  ];
}
...
// ── "My photo" — original account photo ──────────────────────────
  ...
  return (
    ...
    >
      {hasPhoto ? (
        ...
        // No account photo: show initials (same fallback as header)
      ...
      {/* Badge distinguishing account photo from character avatars */}
      {!selected && (
...
export default function DomiflixAvatarPicker() {
  ...
  const t = MESSAGES[locale].app.domiflix.avatar;

  // `selected` = active Domiflix avatar (string url) or null = "My photo"
  // (no Domiflix avatar → Domiflix falls back to the account photo).
  ...
  // Original account photo (same source as the Domiflix header fallback).
  const accountPhoto = profile?.avatar_url || null;

  // Account initials (fallback when no photo), mirroring the header.
  ...
  // "My photo" is active when there is no Domiflix avatar selected.
  const myPhotoSelected = !selected;

  // Keep `selected` in sync with the Domiflix avatar from the DB (may resolve
  // after mount). Null = "My photo" active.
  useEffect(() => {
  ...
  const headerName = (displayName?.trim() || user?.user_metadata?.full_name?.split(" ")[0]) ?? systemFirstName;

  // Build locale-aware sections (re-computed when locale changes)
  ...
  // "My photo": clear Domiflix avatar → Domiflix reverts to account photo
  // (header fallback) or initials when no photo exists.
  ...
  return (
    <div className="domiflix-app min-h-screen bg-[#141414] text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 md:px-16 py-10">
        <h1 className="text-2xl sm:text-3xl font-bold mb-1">{t.pageTitle}</h1>
        <p className="text-white/50 text-sm mb-8">{t.pageSubtitle}</p>

        {/* ── Display name editor ───────────────────────────── */}
        <section className="mb-10 rounded-lg border border-white/10 bg-white/[0.03] p-5 sm:p-6">
          <h2 className="text-base sm:text-lg font-semibold text-white/90 mb-1">{t.displayNameSection}</h2>
          <p className="text-white/50 text-xs mb-4 flex items-start gap-1.5">
            <Info className="w-3.5 h-3.5 mt-0.5 shrink-0 text-white/40" />
            <span>{t.displayNameHint}</span>
          </p>
        ...
        {/* ── "My photo" — reverts to original account photo ──────── */}
        <section className="mb-12">
          ...
          <div className="flex flex-wrap items-start gap-3 sm:gap-4">
            ...
            <p className="flex-1 min-w-[180px] text-white/45 text-xs leading-relaxed pt-1">
              {t.myPhotoDesc}
              {accountPhoto ? "" : t.myPhotoDescNoPhoto}.
            </p>
          </div>
        </section>
