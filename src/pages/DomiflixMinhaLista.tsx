...
export default function DomiflixMinhaLista() {
  ...
  const isLoading = titlesLoading || wlLoading;
      ...
      <div className="px-4 md:px-12">
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <Skeleton key={i} className="h-[140px] w-full bg-[#222] rounded-lg" />
            ))}
          </div>
        ) : myTitles.length === 0 ? (
          ...
          <div className="space-y-3">
            {myTitles.map((title) => {
              ...
              return (
                <div
                  key={title.id}
                  onClick={() => navigate(`/domiflix/${slugify(title.title)}`)}
                  className="flex gap-4 rounded-lg overflow-hidden bg-[#1a1a1a] hover:bg-[#252525] transition-colors cursor-pointer group"
                >
                    ...
                    {bgImage ? (
                      ...
                      <div className="w-full h-full bg-[#2a2a2a] flex items-center justify-center">
                        {title.type === "series" ? (
                          ...
                          <Film className="w-10 h-10 text-[#444]" />
                        )}
                      </div>
                    )}
                  ...
                  <div className="flex-1 min-w-0 py-3 pr-4 flex flex-col justify-between">
                    <div>
                      ...
                      {truncatedDesc && (
                        <p className="text-white/55 text-sm mt-1.5 line-clamp-2 leading-snug">
                          {truncatedDesc}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleWatchlist.mutate({ titleId: title.id, isInList: true });
                        }}
